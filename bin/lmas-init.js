...
/**
 * LMAS-FullStack Installation Wizard v5 (LEGACY)
 ...
 *
 * Migration path:
 * - Use `npx lmas-core` which routes through bin/lmas.js to the new wizard
...
/**
 * Smart path resolution for LMAS Core modules
 */
function resolveLmasCoreModule(modulePath) {
  const lmasCoreModule = path.join(__dirname, '..', '.lmas-core', modulePath);

  const moduleExists =
    fs.existsSync(lmasCoreModule + '.js') ||
    fs.existsSync(lmasCoreModule + '/index.js') ||
    fs.existsSync(lmasCoreModule);

  if (!moduleExists) {
    throw new Error(
      `Cannot find LMAS Core module: ${modulePath}\n` +
        `Searched: ${lmasCoreModule}\n` +
        'Please ensure lmas-core is installed correctly.'
    );
  }

  return require(lmasCoreModule);
}

// Load LMAS Core modules
const { detectRepositoryContext } = resolveLmasCoreModule(
  'infrastructure/scripts/repository-detector'
);
// PM adapters imported but not used directly (loaded dynamically)
// const { ClickUpAdapter } = resolveLmasCoreModule('utils/pm-adapters/clickup-adapter');
// const { GitHubProjectsAdapter } = resolveLmasCoreModule('utils/pm-adapters/github-adapter');
// const { JiraAdapter } = resolveLmasCoreModule('utils/pm-adapters/jira-adapter');
...
async function main() {
  ...
  if (!context) {
    ...
    if (!fs.existsSync(packageJsonPath)) {
      ...
      const defaultPackage = {
        ...
        version: '1.0.0',
        description: 'LMAS-FullStack project',
        main: 'index.js',
  ...
  const installedManifestPath = path.join(projectRoot, '.lmas-core', '.installed-manifest.yaml');
  const hasExistingInstall = fs.existsSync(installedManifestPath);

  if (hasExistingInstall && brownfieldUpgrader) {
    console.log(chalk.yellow('🔄 Existing LMAS installation detected!'));
    console.log('');

    const sourceDir = path.join(context.frameworkLocation, '.lmas-core');
  ...
  const { installMode } = await inquirer.prompt([
    {
      type: 'list',
      name: 'installMode',
      message: chalk.white('How are you using LMAS-FullStack?'),
      choices: [
        {
          name: '  Using LMAS in a project ' + chalk.gray('(Framework files added to .gitignore)'),
          value: 'project-development',
        },
        {
          name:
            '  Developing LMAS framework itself ' + chalk.gray('(Framework files are source code)'),
  ...
  const configPath = path.join(context.projectRoot, '.lmas-installation-config.yaml');
  // INS-2 Performance: Use async write instead of sync
  ...
  // Step 4b: Copy LMAS Core files
  console.log('');

  const sourceCoreDir = path.join(context.frameworkLocation, '.lmas-core');
  const targetCoreDir = path.join(context.projectRoot, '.lmas-core');

  if (fs.existsSync(sourceCoreDir)) {
    // INS-2 Performance: Add spinner for file copy (AC9)
    const copySpinner = ora('Installing LMAS Core files...').start();
    await fse.copy(sourceCoreDir, targetCoreDir);
    copySpinner.succeed(
      'LMAS Core files installed ' +
    ...
    console.error(chalk.red('✗') + ' LMAS Core files not found');
    process.exit(1);
  }
  ...
  if (!ides.includes('none')) {
    ...
    const ideRulesMap = {
      ...
      'github-copilot': { source: 'copilot-rules.md', target: '.github/chatmodes/lmas-agent.md' },
    ...
    // Step 2: Install LMAS CORE agents and tasks for Claude Code
    ...
    if (ides.includes('claude')) {
      ...
      // Create LMAS README for Claude Code
      const lmassReadme = path.join(
        context.projectRoot,
        '.claude',
        'commands',
        'LMAS',
      ...
      await fse.ensureDir(path.dirname(lmassReadme));
      await fse.writeFile(
        lmassReadme,
        `# LMAS Core Commands

This directory contains the core LMAS-FullStack agents and tasks.
...
## Documentation
See .lmas-core/user-guide.md for complete documentation.
`
      );
    ...
    // Step 3: Install LMAS CORE agents for Cursor
    ...
    if (ides.includes('cursor')) {
      ...
      // Create LMAS README for Cursor
      const cursorReadme = path.join(context.projectRoot, '.cursor', 'rules', 'LMAS', 'README.md');
      await fse.ensureDir(path.dirname(cursorReadme));
      await fse.writeFile(
        cursorReadme,
        `# LMAS Core Rules

This directory contains the core LMAS-FullStack agent rules for Cursor.
...
`
    ...
    // Step 4: Install LMAS CORE agents for other IDEs (Gemini, AntiGravity)
    // v4: Agents are in development/ module
    // INS-2 Performance: Uses cached agent files list
    ...
    if (ides.includes('github-copilot')) {
      ...
      if (cachedAgentFiles.length > 0) {
        ...
        for (const agentFile of cachedAgentFiles) {
          ...
          const targetPath = path.join(copilotModesDir, `lmas-${agentName}.md`);
          await fse.copy(sourcePath, targetPath);
        }
  ...
  const possibleSquadsDirs = [
    ...
    path.join(context.projectRoot, 'node_modules', 'lmas-core', 'squads'),
    path.join(context.projectRoot, 'node_modules', '@lmas', 'fullstack', 'squads'),
  ];
  ...
  try {
    ...
    const validator = new PostInstallValidator(context.projectRoot, context.frameworkLocation, {
      ...
      // For production integrity checks, users should run `lmas validate` which
    ...
    if (
      ...
      console.log(
        chalk.yellow('   Run ') +
          chalk.cyan('lmas validate --repair') +
    ...
    if (process.env.DEBUG || process.env.LMAS_DEBUG) {
      console.log(chalk.dim(`   Stack: ${validationError.stack}`));
    }
    console.log(chalk.dim('   Run `lmas validate` to check installation integrity'));
  }
  ...
  console.log(chalk.gray('═'.repeat(80)));
  console.log('');
  console.log(chalk.green.bold('✓ LMAS-FullStack installation complete! 🎉'));
  console.log('');
  ...
  console.log('  ' + chalk.dim('.lmas-core/') + '           - Framework core files');

  if (ides.includes('claude')) {
    console.log('  ' + chalk.dim('.claude/'));
    console.log('    ' + chalk.dim('├─ CLAUDE.md') + '        - Main configuration');
    console.log('    ' + chalk.dim('└─ commands/'));
    console.log('      ' + chalk.dim('  ├─ LMAS/') + '         - Core agents & tasks');
  ...
  if (ides.includes('cursor')) {
    ...
    console.log('      ' + chalk.dim('  ├─ LMAS/') + '         - Core agent rules');
    if (selectedSquads && selectedSquads.length > 0) {
      selectedSquads.forEach((squad) => {
  ...
  if (ides.includes('claude')) {
    ...
    console.log('    • Browse: .claude/commands/LMAS/agents/ for all available agents');
  }
  ...
  if (ides.includes('antigravity')) {
    ...
    console.log('    • Browse: .antigravity/rules/LMAS/agents/ for all available agents');
  }

  console.log('  ' + chalk.yellow('General:'));
  console.log('    • Run ' + chalk.yellow('lmas validate') + ' to verify installation integrity');
  console.log('    • Run ' + chalk.yellow('lmas validate --repair') + ' to fix any missing files');
  console.log('    • Check .lmas-core/user-guide.md for complete documentation');
...
async function updateGitIgnore(mode, projectRoot) {
  ...
  if (mode === 'project-development') {
    const frameworkRules = [
      '',
      '# LMAS-FullStack Framework Files (auto-managed - do not edit)',
      '.lmas-core/',
      'node_modules/@lmas/',
      'outputs/minds/',
      '.lmas-installation-config.yaml',
      '# End LMAS-FullStack auto-managed section',
      '',
    ];

    const hasFrameworkSection = gitignore.includes('# LMAS-FullStack Framework Files');
...
async function savePMConfig(pmTool, config, projectRoot) {
  ...
  const configPath = path.join(projectRoot, '.lmas-pm-config.yaml');
  // INS-2 Performance: Use async write
 ...
 * @param {string} sourceCoreDir - Path to installed .lmas-core directory
 */
