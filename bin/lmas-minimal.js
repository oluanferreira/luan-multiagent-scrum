...
/**
 * LMAS-FullStack Minimal Installation
 *
...
console.log('\n⚠️  DEPRECATION WARNING: lmas-minimal is deprecated.');
console.log('   The --minimal mode (squads) was replaced by Squads.');
console.log('   Running standard installation wizard instead.\n');

// Get the path to the main router (lmas.js)
const routerPath = path.join(__dirname, 'lmas.js');
...
child.on('error', (error) => {
  console.error('❌ Failed to start LMAS:', error.message);
  process.exit(1);
});
