...
/**
 * LMAS IDS CLI — Incremental Development System
 * Standalone (no external dependencies for npx compatibility)
 *
 * Commands:
 *   lmas ids:query {intent}     — Query registry for matching artifacts
 *   lmas ids:create-review      — Review CREATE decisions for promotion/deprecation
 *   lmas ids:health             — Run registry health check (self-healing)
 *   lmas ids:check {intent}     — Pre-check registry (FrameworkGovernor advisory)
 *   lmas ids:impact {entity-id} — Impact analysis for modifications
 *   lmas ids:stats              — Registry statistics
 *   lmas ids:register {path}    — Register entity after creation
 *
 * Flags:
...
const { RegistryLoader } = require(path.resolve(__dirname, '..', '.lmas-core', 'core', 'ids', 'registry-loader'));
const { IncrementalDecisionEngine } = require(path.resolve(__dirname, '..', '.lmas-core', 'core', 'ids', 'incremental-decision-engine'));
const { RegistryUpdater } = require(path.resolve(__dirname, '..', '.lmas-core', 'core', 'ids', 'registry-updater'));
const { FrameworkGovernor } = require(path.resolve(__dirname, '..', '.lmas-core', 'core', 'ids', 'framework-governor'));

// Optional: RegistryHealer (IDS-4a — may not exist yet)
let RegistryHealer = null;
try {
  RegistryHealer = require(path.resolve(__dirname, '..', '.lmas-core', 'core', 'ids', 'registry-healer')).RegistryHealer;
} catch (_err) {
...
function showHelp() {
  console.log(`
LMAS IDS — Incremental Development System
...
Examples:
  lmas ids:query "validate story drafts"
  lmas ids:query "template rendering engine" --json
  lmas ids:query "database migration" --type script
  lmas ids:create-review
  lmas ids:create-review --json
  lmas ids:health
  lmas ids:health --fix
  lmas ids:check "validate yaml schema" --type task
  lmas ids:impact create-doc
  lmas ids:stats
  lmas ids:stats --json
  lmas ids:register .lmas-core/development/tasks/my-new-task.md
`);
}
...
function runQuery() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: lmas ids:query "your intent here"');
    process.exit(1);
  }
...
async function runCheck() {
  ...
  if (!intent) {
    console.error('Error: Intent is required. Usage: lmas ids:check "your intent here"');
    process.exit(1);
  }
...
async function runImpact() {
  ...
  if (!entityId) {
    console.error('Error: Entity ID is required. Usage: lmas ids:impact {entity-id}');
    process.exit(1);
  }
...
async function runRegister() {
  ...
  if (!filePath) {
    console.error('Error: File path is required. Usage: lmas ids:register {file-path}');
    process.exit(1);
  }
