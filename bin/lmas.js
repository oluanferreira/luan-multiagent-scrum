...
async function runWizard(options = {}) {
  ...
  if (!fs.existsSync(wizardPath)) {
    ...
    const legacyScript = path.join(__dirname, 'lmas-init.js');
    if (fs.existsSync(legacyScript)) {
      if (!options.quiet) {
        console.log('⚠️  Using legacy wizard (src/wizard not found)');
      }
      // Legacy wizard doesn't support options, pass via env vars
      process.env.LMAS_INSTALL_FORCE = options.force ? '1' : '';
      process.env.LMAS_INSTALL_QUIET = options.quiet ? '1' : '';
      process.env.LMAS_INSTALL_DRY_RUN = options.dryRun ? '1' : '';
...
UPDATE:
  ...
  lmas update --check            # Check for updates without applying
  lmas update --dry-run          # Preview what would be updated
  lmas update --force            # Force update even if up-to-date
  lmas update --verbose          # Show detailed output

VALIDATION:
  lmas validate                    # Validate installation integrity
  lmas validate --repair           # Repair missing/corrupted files
  lmas validate --repair --dry-run # Preview repairs
  lmas validate --detailed         # Show detailed file list

CONFIGURATION:
  lmas config show                       # Show resolved configuration
  lmas config show --debug               # Show with source annotations
  lmas config diff --levels L1,L2        # Compare config levels
  lmas config migrate                    # Migrate monolithic to layered
  lmas config validate                   # Validate config files
  lmas config init-local                 # Create local-config.yaml

SERVICE DISCOVERY:
  lmas workers search <query>            # Search for workers
  lmas workers search "json" --category=data
  lmas workers search "transform" --tags=etl,data
  lmas workers search "api" --format=json
...
function showInfo() {
  ...
  // Check if .lmas-core exists
  const lmasCoreDir = path.join(__dirname, '..', '.lmas-core');
  if (fs.existsSync(lmasCoreDir)) {
    console.log('\n✓ LMAS Core installed');
    ...
    const devDir = path.join(lmasCoreDir, 'development');
    const componentBase = fs.existsSync(devDir) ? devDir : lmasCoreDir;
...
async function runValidate() {
  ...
  try {
    ...
    const { createValidateCommand } = require('../.lmas-core/cli/commands/validate/index.js');
    ...
    await validateCmd.parseAsync(['node', 'lmas', ...validateArgs]);
  } catch (_error) {
...
async function runUpdate() {
  ...
  try {
    ...
    const { LMASUpdater, formatCheckResult, formatUpdateResult } = require(updaterPath);

    const updater = new LMASUpdater(process.cwd(), {
      verbose: isVerbose,
      force: isForce,
    });
...
async function runDoctor(options = {}) {
  const { runDoctorChecks } = require(path.join(__dirname, '..', '.lmas-core', 'core', 'doctor'));
...
// Helper: Remove LMAS sections from .gitignore
function cleanGitignore(gitignorePath) {
  ...
  const newLines = [];
  let inLmasSection = false;
  let removedLines = 0;

  for (const line of lines) {
    if (line.includes('# LMAS') || line.includes('# Added by LMAS')) {
      inLmasSection = true;
      removedLines++;
      continue;
    }
    if (inLmasSection && line.trim() === '') {
      inLmasSection = false;
      continue;
    }
    if (inLmasSection) {
...
Options:
  ...
  --keep-data  Keep .lmas/ directory (settings and history)
  --dry-run    Show what would be removed without removing
  -h, --help   Show this help message

What gets removed:
  - .lmas-core/     Framework core files
  - docs/stories/   Story files (if created by LMAS)
  - squads/         Squad definitions
  - .gitignore      LMAS-added entries only

What is preserved (with --keep-data):
  - .lmas/          Project settings and agent history
...
Checks performed:
  • Required directories exist (.lmas-core/, .lmas/)
...
async function runUninstall(options = {}) {
  ...
  // Items to remove
  const itemsToRemove = [
    { path: '.lmas-core', description: 'Framework core' },
    { path: 'squads', description: 'Squad definitions' },
  ];

  // Optionally remove .lmas
  if (!keepData) {
    itemsToRemove.push({ path: '.lmas', description: 'Project data and settings' });
  }
  ...
  if (existingItems.length === 0) {
    console.log('ℹ️  No LMAS installation found in this directory.');
    return;
  }
  ...
  if (!quiet) {
    ...
    if (fs.existsSync(gitignorePath)) {
      const content = fs.readFileSync(gitignorePath, 'utf8');
      if (content.includes('# LMAS') || content.includes('# Added by LMAS')) {
        console.log('  • .gitignore LMAS entries will be cleaned');
      }
    }
    console.log('');
  }
  ...
  if (!force) {
    ...
    const answer = await new Promise(resolve => {
      rl.question('⚠️  Are you sure you want to uninstall LMAS? (y/N): ', resolve);
    });
    rl.close();
  ...
  if (gitignoreResult.removed && !quiet) {
    console.log(`  ✓ Cleaned ${gitignoreResult.lines} LMAS entries from .gitignore`);
  }

  // Summary
  if (!quiet) {
    console.log('\n✅ LMAS has been uninstalled.');
    if (keepData) {
...
Options:
  --force      Overwrite existing LMAS installation
...
Smart Merge (Brownfield):
  ...
  LMAS can merge new settings while preserving your customizations.

  - .env files: Adds new variables, preserves existing values
  - CLAUDE.md: Updates LMAS sections, keeps your custom rules
...
async function initProject() {
  ...
  console.log(`Creating new LMAS project: ${displayName}`);
  if (template !== 'default') {
