 ...
 * 4. (INS-4.12) .lmas-core/package.json dependency completeness
 *
...
// Check 4 (INS-4.12): .lmas-core dependency completeness
console.log('');
console.log('--- Dependency Completeness (INS-4.12) ---\n');
try {
  const depValidatorPath = path.join(PROJECT_ROOT, 'scripts', 'validate-lmas-core-deps.js');
  if (fs.existsSync(depValidatorPath)) {
    ...
    console.log('PASS: .lmas-core dependency completeness validated');
  } else {
    console.log('SKIP: scripts/validate-lmas-core-deps.js not found');
  }
} catch (_depErr) {
  console.error('FAIL: .lmas-core dependency completeness check failed');
  console.error('  Fix: Run "node scripts/validate-lmas-core-deps.js" to see details');
  passed = false;
}
