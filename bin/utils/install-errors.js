...
const ERROR_MESSAGES = {
  ...
  UNKNOWN_ERROR: {
    ...
    recovery: [
      'Check the installation log: .lmas-install.log',
...
function formatErrorMessage(error, errorCode = null) {
  ...
  lines.push('');
  lines.push(chalk.gray('Installation log: .lmas-install.log'));
  lines.push('');
...
function formatRollbackMessage(success, failedFiles = []) {
  ...
  if (success) {
    ...
    lines.push(chalk.cyan('  1. Check backup directory: .lmas-backup/'));
    lines.push(chalk.cyan('  2. Manually restore failed files from backups'));
    lines.push(chalk.cyan('  3. Review installation log: .lmas-install.log'));
  }
...
function formatSuccessMessage() {
  ...
  lines.push(chalk.green('═══════════════════════════════════════════════════════'));
  lines.push('');
  lines.push(chalk.white('LMAS has been installed and configured.'));
  lines.push('');
  lines.push(chalk.cyan.bold('Next Steps:'));
  lines.push(chalk.cyan('  1. Review installation log: .lmas-install.log'));
  lines.push(chalk.cyan('  2. Verify configuration: lmas config --check'));
  lines.push(chalk.cyan('  3. Run validation: lmas validate'));
  lines.push('');
  return lines.join('\n');
}
