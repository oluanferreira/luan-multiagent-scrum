...
IDE-FILE-RESOLUTION:
  ...
  - Example: create-agent.md → .lmas-core/development/tasks/create-agent.md
...
activation-instructions:
  ...
  - STEP 3: |
      ...
      # FALLBACK: If native greeting fails, run: node .lmas-core/development/scripts/unified-activation-pipeline.js swarm-orchestrator
...
knowledge_base:
  ...
  # ── LAYER 4: LMAS Integration ──────────────────────

  lmas_subagent_patterns:
    description: |
      Within the LMAS framework, the Agent tool (formerly Task tool) can spawn
      subagents using the subagent_type parameter. LMAS agents can delegate to
    ...
    worktree_integration:
      description: |
        ...
        changes are made. Combine with LMAS *worktree-create for story-level isolation.
      pattern: |
...
- **@architect (Architect):** Validates multi-agent topology decisions and system design alignment
- **@dev (Neo):** Receives subagent definitions and team configurations for implementation use
- **@qa (Oracle):** Reviews agent team findings through QA gate validation

**I delegate to:**

- **@devops (Operator):** For git push, PR creation, and CI/CD integration of agent configurations
