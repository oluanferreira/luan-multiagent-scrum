...
Se é sua primeira vez no LMAS, siga este caminho linear:

1. Instale em um projeto novo ou existente:
```bash
# novo projeto
npx lmas-core init meu-projeto
...
## Compatibilidade de Hooks por IDE (Realidade LMAS 4.2)

Muitos recursos avançados do LMAS dependem de eventos de ciclo de vida (hooks). A tabela abaixo mostra a paridade real entre IDEs/plataformas:
...
### Entendendo o Fluxo de Trabalho LMAS

**Antes de mergulhar, revise estes diagramas críticos de fluxo de trabalho que explicam como o LMAS funciona:**
...
- **[Aprender como usar o LMAS](docs/guides/user-guide.md)** → Guia completo do usuário e passo a passo
...
**Mantenha-se atualizado sem esforço!** Para atualizar sua instalação LMAS existente:
...
npx lmas-core@latest init meu-projeto
```
...
- ✅ Configuração de todos os agentes e fluxos de trabalho LMAS
...
Se você já tem o LMAS instalado:
...
- ✅ Integração de primeira classe no LMAS 4.2 (pipeline de ativação e greeting compartilhado)
...
- Arquivos gerados em `.gemini/rules.md`, `.gemini/rules/LMAS/agents/` e `.gemini/commands/*.toml`
- ✅ Hooks e settings locais no fluxo de instalacao (`.gemini/hooks/` + `.gemini/settings.json`)
- ✅ Ativacao rapida por slash commands (`/lmas-menu`, `/lmas-dev`, `/lmas-architect`, etc.)
...
- 📝 Padrões de código específicos do LMAS

### Início Mais Rápido com Interface Web (2 minutos)

1. **Instale o LMAS**: Execute `npx lmas-core init meu-projeto`
...
4. **Use comandos LMAS**: Digite `*help` para ver comandos disponíveis
...
npx lmas-core init <nome-projeto> [opções]
...
# Comandos do Sistema
npx lmas-core --version   Exibir versão instalada
npx lmas-core --help      Exibir ajuda detalhada
npx lmas-core info        Exibir informações do sistema
npx lmas-core doctor      Executar diagnósticos do sistema
npx lmas-core doctor --fix Corrigir problemas detectados automaticamente
...
```bash
# Instalar LMAS no projeto
npx lmas-core@latest install

# Isto vai:
# 1. Detectar instalação existente (se houver)
# 2. Instalar/atualizar framework LMAS
...
- **lmas-master** - Agente mestre de orquestração (inclui capacidades de desenvolvimento de framework)
- **lmas-orchestrator** - Orquestrador de fluxo de trabalho e coordenação de equipe
...
- 🏗️ **[Arquitetura Principal](docs/architecture/LMAS-VISUAL-OVERVIEW.md)** - Mergulho técnico profundo e design do sistema
- 🚀 **[Guia de Squads](docs/guides/squads-guide.md)** - Estenda o LMAS para qualquer domínio além do desenvolvimento de software
...
- 🎯 **[Princípios Orientadores](docs/GUIDING-PRINCIPLES.md)** - Filosofia e melhores práticas do LMAS
- 🏛️ **[Visão Geral da Arquitetura](docs/architecture/LMAS-VISUAL-OVERVIEW.md)** - Visão detalhada da arquitetura do sistema
- ⚙️ **[Guia de Ajuste de Performance](docs/performance-tuning-guide.md)** - Otimize seu fluxo de trabalho LMAS
...
Squads permitem estender o LMAS para qualquer domínio. Estrutura básica:
...
- **Licenciamento** - Gerenciamento de licença via `lmas pro activate --key <KEY>`

Para mais informações, execute `npx lmas-core pro --help` após a instalação.
...
# Validador LMAS
node .lmas-core/utils/lmas-validator.js pre-commit   # Validação pre-commit
node .lmas-core/utils/lmas-validator.js pre-push     # Validação pre-push
node .lmas-core/utils/lmas-validator.js stories      # Validar todas stories
```
