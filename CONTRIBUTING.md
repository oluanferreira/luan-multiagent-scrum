...
Welcome to LMAS! Thank you for your interest in contributing. This guide will help you understand our development workflow, contribution process, and how to submit your changes.
...
```
.lmas-core/development/agents/your-agent.md
```
...
```
.lmas-core/development/tasks/your-task.md
```
...
- Security concerns
- LMAS-specific patterns (agents, tasks, workflows)
- Performance issues
...
LMAS implements a **Defense in Depth** strategy with 3 validation layers:
...
Critical paths require approval from `@oluanferreira` (maintainer):

| Path | Why |
|------|-----|
| `.lmas-core/core/orchestration/` | Orchestration layer (MasterOrchestrator, GateEvaluator) |
| `.lmas-core/core/execution/` | Execution engine (WaveExecutor, ParallelExecutor) |
| `packages/` | Installer, CLI, shared libraries |
| `.github/` | CI/CD workflows, branch protection |
| `.lmas-core/core-config.yaml` | Framework configuration |

All other paths require review from any maintainer (`@oluanferreira`).
...
LMAS uses an Open Core model with a private `pro/` git submodule (see [ADR-PRO-001](docs/architecture/adr/adr-pro-001-repository-strategy.md)).
...
git clone https://github.com/<your-fork>/lmas-core.git
cd lmas-core
...
**Push order:** Always push `pro/` changes first, then `lmas-core`.
