...
- Pro wizard (`npx lmas-core install`) now auto-installs `@lmas-fullstack/pro` package during Step 2, fixing "Pro package not found" error in greenfield and brownfield projects.
...
- `docs/glossary.md` with official LMAS taxonomy terms:
  - `squad`
  - `flow-state`
  - `confidence gate`
  - `execution profile`
...
This release marks the **Open-Source Community Readiness** milestone, preparing LMAS for public contribution while introducing the **Squad System** for extensibility.
...
- **Rebranding to LMAS** (OSR-9)
  - Brand investigation complete
  - Namespace updated to LMAS
...
- **Scripts Path Consolidation** (6.16)
  - Standardized script locations under `.lmas-core/scripts/`
...
- **Agent Command Rationalization** (Story 6.1.2.3)
  - Command consolidation: `lmas-master` 44→30 commands (32% reduction)
...
- **Scripts Location** - Consolidated under `.lmas-core/scripts/` for consistency
...
### Removed
- **Private squads** - Moved to separate private repository (`lmas-squads`)
  - Removed `squads/creator/` (CreatorOS)
  - Removed `squads/innerlens/`
  - Removed `squads/mmos-mapper/`
  - Removed `squads/lmas-infrastructure-devops/`
  - Removed `squads/meeting-notes/`
  - Repository: https://github.com/oluanferreira/luan-multiagent-scrum (PRIVATE)
- **Internal development tools** - Moved to separate private repository (`lmas-dev-tools`)
...
- **hybrid-ops squad** - Moved to separate repository for independent maintenance
  ...
  - Repository: https://github.com/oluanferreira/luan-multiagent-scrum
...
- **Squad naming convention** - Applied consistent `{agent-id}-` prefix to agent-specific tasks across all 6 squads
  ...
  - Luan - MultiAgentes Scrum-infrastructure-devops pack: 2 tasks already renamed (pre-existing)
...
- Breaking Change: hybrid-ops no longer bundled with lmas-core
...
- PRIMARY detection layer in `tools/lmas-npx-wrapper.js` using `__dirname`
- SECONDARY fallback detection in `tools/installer/bin/lmas.js` using `process.cwd()`
...
- Prevents confusion when running `npx lmas-core install` from home directory

### Changed
- Early exit with `process.exit(1)` when NPX temporary context detected
- Help message provides actionable solution: `cd /path/to/your/project && npx lmas-core install`
