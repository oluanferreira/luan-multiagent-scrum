...
const { IncrementalDecisionEngine } = require('../../../.lmas-core/core/ids/incremental-decision-engine');
const { FrameworkGovernor, TIMEOUT_MS, RISK_THRESHOLDS } = require('../../../.lmas-core/core/ids/framework-governor');
  ...
  describe('postRegister()', () => {
    it('should register file via RegistryUpdater.onAgentTaskComplete', async () => {
      const result = await governor.postRegister(
        ...
        { type: 'task', purpose: 'Test task', agent: 'lmas-master' },
      );
      expect(result.registered).toBeDefined();
      expect(result.filePath).toBe('.lmas-core/development/tasks/test-task.md');
      expect(updater.onAgentTaskCompleteCalls.length).toBe(1);

      const call = updater.onAgentTaskCompleteCalls[0];
      expect(call.task.agent).toBe('lmas-master');
      expect(call.artifacts).toContain('.lmas-core/development/tasks/test-task.md');
    });

    it('should use onAgentTaskComplete (not processChanges) per SF-1', async () => {
      await governor.postRegister('.lmas-core/development/tasks/new-task.md', {});
    ...
    it('should default agent to lmas-master', async () => {
      ...
      it('should include recommendations when matches found', () => {
        const result = {
          ...
          recommendations: [{
            ...
            decision: 'ADAPT',
            relevanceScore: 0.75,
            entityPath: '.lmas-core/development/tasks/create-doc.md',
          }],
        };
        const output = FrameworkGovernor.formatPreCheckOutput(result);
    ...
    describe('formatImpactOutput()', () => {
      ...
      it('should show safe-to-modify for zero consumers', () => {
        const result = {
          entityId: 'po',
          found: true,
          entityPath: '.lmas-core/development/agents/po.md',
          entityType: 'agent',
          riskLevel: 'NONE',
          directConsumers: [],
          indirectConsumers: [],
