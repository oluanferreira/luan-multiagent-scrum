...
// Module under test
const TerminalSpawner = require('../../.lmas-core/core/orchestration/terminal-spawner');
...
describe('TerminalSpawner', () => {
  ...
  describe('createContextFile', () => {
    ...
    test('should create valid JSON file', async () => {
      ...
      try {
        expect(contextPath).toContain('lmas-context-');
  ...
  describe('pollForOutput', () => {
    test('should return output when lock file is missing', async () => {
      ...
      const outputPath = path.join(tmpDir, `lmas-output-${timestamp}.md`);
    ...
    test('should return "No output captured" if file does not exist and no lock', async () => {
      const fakePath = path.join(tmpDir, `lmas-output-nonexistent-${Date.now()}.md`);
    ...
    test('should timeout when lock file persists', async () => {
      ...
      const lockPath = path.join(tmpDir, `lmas-lock-${timestamp}.md`);
    ...
    test('should return output after lock is removed', async () => {
      const timestamp = Date.now();
      const outputPath = path.join(tmpDir, `lmas-output-${timestamp}.md`);
      const lockPath = path.join(tmpDir, `lmas-lock-${timestamp}.lock`);
  ...
  describe('cleanupOldFiles', () => {
    test('should clean old LMAS temp files', async () => {
      // Create old files (mock old timestamp by naming)
      const oldTimestamp = Date.now() - 7200000; // 2 hours ago
      const oldOutputPath = path.join(tmpDir, `lmas-output-${oldTimestamp}.md`);
      const oldLockPath = path.join(tmpDir, `lmas-lock-${oldTimestamp}.lock`);
      const oldContextPath = path.join(tmpDir, `lmas-context-${oldTimestamp}.json`);
    ...
    test('should not clean recent files', async () => {
      ...
      const recentPath = path.join(tmpDir, `lmas-output-${recentTimestamp}.md`);
  ...
  describe('Index Integration', () => {
    test('should be exported from orchestration index', () => {
      const orchestration = require('../../.lmas-core/core/orchestration');
...
describe('pm.sh Script', () => {
  ...
  test('should display help with --help flag', () => {
    ...
    expect(result).toContain('LMAS Multi-Modal Orchestration Script');
