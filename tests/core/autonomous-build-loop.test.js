...
jest.mock('../../.lmas-core/core/execution/build-state-manager', () => ({
  BuildStateManager: jest.fn().mockImplementation(() => ({ ...mockStateInstance })),
}));

// Mock optional dependencies to not load
jest.mock('../../.lmas-core/infrastructure/scripts/recovery-tracker', () => {
  throw new Error('not available');
});
jest.mock('../../.lmas-core/infrastructure/scripts/worktree-manager', () => {
  throw new Error('not available');
});
...
const {
  ...
  SubtaskResult,
  DEFAULT_CONFIG,
} = require('../../.lmas-core/core/execution/autonomous-build-loop');

const { BuildStateManager } = require('../../.lmas-core/core/execution/build-state-manager');
