...
} = require('../../../.lmas-core/core/orchestration/bob-status-writer');
...
describe('BobStatusWriter', () => {
  ...
  describe('constructor', () => {
    ...
    it('should set correct paths', () => {
      ...
      expect(writer.dashboardDir).toBe(path.join(tempDir, '.lmas', 'dashboard'));
      expect(writer.statusPath).toBe(path.join(tempDir, '.lmas', 'dashboard', 'bob-status.json'));
    });
  });
  ...
  describe('updateAgent', () => {
    it('should update current_agent fields', async () => {
      await writer.initialize();
      await writer.updateAgent('@dev', 'Neo', 'development', 'Story type: code_general');

      const status = await fs.readJson(writer.statusPath);
      expect(status.current_agent.id).toBe('@dev');
      expect(status.current_agent.name).toBe('Neo');
  ...
  describe('clearAgent', () => {
    it('should clear current_agent fields', async () => {
      await writer.initialize();
      await writer.updateAgent('@dev', 'Neo', 'development', 'reason');
