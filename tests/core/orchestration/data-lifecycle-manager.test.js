...
const {
  ...
  STALE_SESSION_DAYS,
  STALE_SNAPSHOT_DAYS,
} = require('../../../.lmas-core/core/orchestration/data-lifecycle-manager');
...
describe('DataLifecycleManager', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(path.join(TEST_PROJECT_ROOT, '.lmas/snapshots'), { recursive: true });
  ...
  describe('cleanupStaleSessions', () => {
    ...
    it('should archive session if last_updated > 30 days (AC8)', async () => {
      ...
      const archiveDir = path.join(TEST_PROJECT_ROOT, '.lmas/archive/sessions');
  ...
  describe('cleanupStaleSnapshots', () => {
    it('should return 0 when no snapshots directory exists', async () => {
      ...
      await fs.rm(path.join(TEST_PROJECT_ROOT, '.lmas/snapshots'), { recursive: true });
    ...
    it('should not remove snapshot if age < 90 days', async () => {
      ...
      const snapshotPath = path.join(TEST_PROJECT_ROOT, '.lmas/snapshots/snapshot-recent.json');
    ...
    it('should remove snapshot if age > 90 days and update index.json (AC10)', async () => {
      ...
      const snapshotPath = path.join(TEST_PROJECT_ROOT, '.lmas/snapshots/snapshot-old.json');
      await fs.writeFile(snapshotPath, JSON.stringify({ epic_id: 'test', story_id: '1.0' }));
      ...
      const indexPath = path.join(TEST_PROJECT_ROOT, '.lmas/snapshots/index.json');
