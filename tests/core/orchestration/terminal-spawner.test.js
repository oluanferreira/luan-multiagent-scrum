...
const {
  ...
  POLL_INTERVAL_MS,
  MAX_RETRIES,
} = require('../../../.lmas-core/core/orchestration/terminal-spawner');

// Test fixtures
const TEST_OUTPUT_DIR = path.join(os.tmpdir(), 'lmas-terminal-spawner-test');
...
describe('Terminal Spawner (Story 12.10)', () => {
  ...
  describe('Timeout handling (Task 3.2)', () => {
    it('should timeout if lock file persists', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'lmas-output-timeout-test.md');
    ...
    it('should return output when lock is removed', async () => {
      // Given
      const outputFile = path.join(TEST_OUTPUT_DIR, 'lmas-output-success-test.md');
      const lockFile = outputFile.replace('output', 'lock');
  ...
  describe('Lock cleanup (Task 3.3, 3.4)', () => {
    ...
    it('should cleanup old files', async () => {
      ...
      const oldFile = path.join(TEST_OUTPUT_DIR, 'lmas-output-old.md');
