...
const { MessageFormatter, createMessageFormatter } = require('../../../.lmas-core/core/orchestration/message-formatter');
...
describe('MessageFormatter', () => {
    ...
    describe('educational mode ON', () => {
      ...
      it('should include agents when provided', () => {
        const result = formatter.formatActionResult('Database Migration', {
          agents: [
            { id: '@data-engineer', name: 'Dozer', task: 'Create migration' },
            { id: '@dev', name: 'Neo', task: 'Update models' },
          ],
        });
        expect(result).toContain('👥 Agentes envolvidos:');
        expect(result).toContain('@data-engineer (Dozer): Create migration');
        expect(result).toContain('@dev (Neo): Update models');
      });
  ...
  describe('formatAgentAssignment', () => {
    it('should return empty string when educational mode is OFF', () => {
      const formatter = new MessageFormatter({ educationalMode: false });
      const result = formatter.formatAgentAssignment('@dev', 'Neo', 'Implement feature');
      expect(result).toBe('');
    });

    it('should return explanation when educational mode is ON', () => {
      const formatter = new MessageFormatter({ educationalMode: true });
      const result = formatter.formatAgentAssignment('@dev', 'Neo', 'Implement feature', 'Best for code implementation');
      expect(result).toContain('🤖 @dev (Neo) assumindo: Implement feature');
      expect(result).toContain('Por quê: Best for code implementation');
    });

    it('should work without reason', () => {
      const formatter = new MessageFormatter({ educationalMode: true });
      const result = formatter.formatAgentAssignment('@qa', 'Oracle', 'Run tests');
      expect(result).toContain('🤖 @qa (Oracle) assumindo: Run tests');
