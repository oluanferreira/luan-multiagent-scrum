 ...
 * - lmas-master has *update-source-tree command
 */
...
describe('Story ACT-8: Agent Config Enrichment', () => {
  ...
  beforeAll(() => {
    agentConfig = loadYaml('.lmas-core/data/agent-config-requirements.yaml');
  });
  ...
  describe('YAML Validity', () => {
    ...
    test('all expected agents have entries', () => {
      const expectedAgents = [
        'lmas-master', 'dev', 'qa', 'devops', 'github-devops',
        'architect', 'po', 'sm', 'data-engineer', 'db-sage',
        'pm', 'analyst', 'ux-design-expert', 'squad-creator',
        'lmas-developer', 'lmas-orchestrator', 'default',
      ];
  ...
  describe('7 Required Data Files Exist on Disk', () => {
    const requiredFiles = [
      ...
      { path: '.lmas-core/data/technical-preferences.md', owner: '@architect' },
      { path: '.lmas-core/product/data/test-levels-framework.md', owner: '@qa' },
      { path: '.lmas-core/product/data/test-priorities-matrix.md', owner: '@qa' },
      { path: '.lmas-core/product/data/brainstorming-techniques.md', owner: '@analyst' },
      { path: '.lmas-core/product/data/elicitation-methods.md', owner: '@po' },
    ];
  ...
  describe('Enriched Agent: @analyst', () => {
    test('analyst now loads brainstorming-techniques, tech-stack, and source-tree', () => {
      ...
      expect(paths).toContain('.lmas-core/product/data/brainstorming-techniques.md');
      expect(paths).toContain('docs/framework/tech-stack.md');
  ...
  describe('Enriched Agent: @sm', () => {
    test('sm now loads mode-selection, workflow-patterns, and coding-standards', () => {
      ...
      expect(paths).toContain('.lmas-core/product/data/mode-selection-best-practices.md');
      expect(paths).toContain('.lmas-core/data/workflow-patterns.yaml');
      expect(paths).toContain('docs/framework/coding-standards.md');
    });
...
describe('Story ACT-8: Document Governance', () => {
  ...
  describe('Governance Task', () => {
    test('update-source-tree.md task file exists', () => {
      expect(fileExists('.lmas-core/development/tasks/update-source-tree.md')).toBe(true);
    });

    test('task file contains expected sections', () => {
      const content = readFile('.lmas-core/development/tasks/update-source-tree.md');
  ...
  describe('lmas-master Command', () => {
    test('lmas-master.md contains *update-source-tree command', () => {
      const content = readFile('.lmas-core/development/agents/lmas-master.md');
      expect(content).toContain('update-source-tree');
      expect(content).toContain('Validate data file governance');
    });

    test('lmas-master dependencies include update-source-tree.md', () => {
      const content = readFile('.lmas-core/development/agents/lmas-master.md');
