...
const {
  ...
  SUPPORTED_AGENTS,
  InvocationStatus,
} = require('../../.lmas-core/core/orchestration/agent-invoker');
...
describe('Agent Invoker (Story 0.7)', () => {
  ...
  beforeEach(async () => {
    ...
    const agentsDir = path.join(tempDir, '.lmas-core', 'development', 'agents');
    ...
    const tasksDir = path.join(tempDir, '.lmas-core', 'development', 'tasks');
...
describe('Integration with MasterOrchestrator', () => {
  ...
  it('should integrate AgentInvoker with MasterOrchestrator', async () => {
    const { MasterOrchestrator } = require('../../.lmas-core/core/orchestration');
