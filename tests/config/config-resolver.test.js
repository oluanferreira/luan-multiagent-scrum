...
const {
  ...
  LEVELS,
  VALID_USER_PROFILES,
} = require('../../.lmas-core/core/config/config-resolver');
const { globalConfigCache } = require('../../.lmas-core/core/config/config-cache');
...
function createTempProject(files = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-config-test-'));
...
describe('config-resolver', () => {
  ...
  describe('constants', () => {
    ...
    test('CONFIG_FILES.user points to ~/.lmas/user-config.yaml', () => {
      const expected = path.join(os.homedir(), '.lmas', 'user-config.yaml');
  ...
  describe('isLegacyMode', () => {
    test('returns true when core-config.yaml exists but framework-config.yaml does not', () => {
      const tmpDir = createTempProject({
        '.lmas-core/core-config.yaml': 'project:\n  name: legacy\n',
      });
    ...
    test('returns false when framework-config.yaml exists', () => {
      const tmpDir = createTempProject({
        '.lmas-core/core-config.yaml': 'project:\n  name: legacy\n',
        '.lmas-core/framework-config.yaml': 'metadata:\n  version: "1.0"\n',
      });
  ...
  describe('getConfigAtLevel', () => {
    test('loads framework config at L1', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
      });

      try {
        const config = getConfigAtLevel(tmpDir, 'L1');
        expect(config).toBeTruthy();
        expect(config.metadata.framework_name).toBe('LMAS-FullStack');
      } finally {
        cleanupTempDir(tmpDir);
      }
    });
    ...
    test('loads project config at L2', () => {
      const tmpDir = createTempProject({
        '.lmas-core/project-config.yaml': { fixture: 'project-config.yaml' },
      });
  ...
  describe('loadLegacyConfig', () => {
    test('loads monolithic core-config.yaml', () => {
      const tmpDir = createTempProject({
        '.lmas-core/core-config.yaml': { fixture: 'legacy-core-config.yaml' },
      });
    ...
    test('includes deprecation warning', () => {
      ...
      const origEnv = process.env.LMAS_SUPPRESS_DEPRECATION;

      try {
        delete process.env.LMAS_SUPPRESS_DEPRECATION;
        ...
        if (origEnv === undefined) {
          ...
          process.env.LMAS_SUPPRESS_DEPRECATION = origEnv;
        }
        cleanupTempDir(tmpDir);
      }
    });

    test('suppresses deprecation when LMAS_SUPPRESS_DEPRECATION=true', () => {
      const tmpDir = createTempProject({
        '.lmas-core/core-config.yaml': { fixture: 'legacy-core-config.yaml' },
      });
      const origEnv = process.env.LMAS_SUPPRESS_DEPRECATION;
      ...
      try {
        process.env.LMAS_SUPPRESS_DEPRECATION = 'true';
  ...
  describe('loadLayeredConfig — 4-level resolution', () => {
    test('merges L1 and L2 correctly', () => {
      ...
      try {
        ...
        // L1 values present
        expect(result.config.metadata.framework_name).toBe('LMAS-FullStack');
        // L2 overrides L1
    ...
    test('merges L1 + L2 + L4 correctly', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.lmas-core/project-config.yaml': { fixture: 'project-config.yaml' },
        '.lmas-core/local-config.yaml': { fixture: 'local-config.yaml' },
      });
    ...
    test('lints L1 and L2 for env patterns', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': 'url: "${API_URL}"\n',
        '.lmas-core/project-config.yaml': 'name: "clean"\n',
      });
  ...
  describe('resolveConfig — main entry point', () => {
    ...
    test('interpolates env vars after merge', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': 'metadata:\n  name: "test"\n',
        '.lmas-core/local-config.yaml': 'api_url: "${TEST_API_URL:-http://localhost}"\n',
      });
  ...
  describe('L5 User layer', () => {
    ...
    beforeEach(() => {
      ...
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-user-config-test-'));
    ...
    test('L5 overrides L4 values', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.lmas-core/local-config.yaml': 'user_profile: "advanced"\n',
      });
  ...
  describe('setUserConfigValue', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-user-write-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
  ...
  describe('toggleUserProfile', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-toggle-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
  ...
  describe('integration — L5 with full hierarchy', () => {
    ...
    beforeEach(() => {
      originalUserConfigPath = CONFIG_FILES.user;
      tempUserDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-l5-integration-test-'));
      CONFIG_FILES.user = path.join(tempUserDir, 'user-config.yaml');
      globalConfigCache.clear();
    });
    ...
    test('resolveConfig returns user_profile from L5 overriding lower levels', () => {
      const tmpDir = createTempProject({
        '.lmas-core/framework-config.yaml': { fixture: 'framework-config.yaml' },
        '.lmas-core/project-config.yaml': 'user_profile: "advanced"\nproject:\n  name: test\n',
      });
