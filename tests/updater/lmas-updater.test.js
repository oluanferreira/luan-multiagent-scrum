...
const { LMASUpdater, UpdateStatus, formatCheckResult, formatUpdateResult } = require('../../packages/installer/src/updater');

describe('LMASUpdater', () => {
  let tempDir;
  let updater;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'lmas-updater-test-'));

    // Create minimal .lmas-core structure
    await fs.ensureDir(path.join(tempDir, '.lmas-core'));
    await fs.ensureDir(path.join(tempDir, '.lmas'));

    // Create version.json
    await fs.writeJson(path.join(tempDir, '.lmas-core', 'version.json'), {
      version: '1.0.0',
    ...
    updater = new LMASUpdater(tempDir, { verbose: false });
  });
  ...
  describe('constructor', () => {
    it('should initialize with default options', () => {
      const u = new LMASUpdater(tempDir);
  ...
  describe('getInstalledVersion', () => {
    ...
    it('should return null if version.json not found', async () => {
      await fs.remove(path.join(tempDir, '.lmas-core', 'version.json'));
      const version = await updater.getInstalledVersion();
      expect(version).toBeNull();
    });

    it('should handle corrupted version.json', async () => {
      await fs.writeFile(path.join(tempDir, '.lmas-core', 'version.json'), 'invalid json');
      const version = await updater.getInstalledVersion();
      expect(version).toBeNull();
    });
  });
  ...
  describe('detectCustomizations', () => {
    it('should detect unchanged files', async () => {
      ...
      const testFile = path.join(tempDir, '.lmas-core', 'test-file.md');
  ...
  describe('rollback', () => {
    ...
    it('should restore files from backup', async () => {
      ...
      // Verify restored
      const restored = await fs.readJson(path.join(tempDir, '.lmas-core', 'version.json'));
  ...
  describe('updateVersionInfo', () => {
    it('should update version.json with new version', async () => {
      ...
      const versionJson = await fs.readJson(path.join(tempDir, '.lmas-core', 'version.json'));
