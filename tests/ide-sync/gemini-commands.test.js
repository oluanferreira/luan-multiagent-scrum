...
const {
  ...
  syncGeminiCommands,
} = require('../../.lmas-core/infrastructure/scripts/ide-sync/gemini-commands');
...
describe('gemini command sync', () => {
  ...
  it('normalizes command slugs and menu names', () => {
    expect(commandSlugForAgent('lmas-master')).toBe('master');
    expect(commandSlugForAgent('dev')).toBe('dev');
    expect(menuCommandName('lmas-master')).toBe('/lmas-master');
    expect(menuCommandName('dev')).toBe('/lmas-dev');
  });
  ...
  it('builds menu + one command per agent', () => {
    ...
    expect(files.find((f) => f.filename === 'lmas-menu.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'lmas-dev.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'lmas-architect.toml')).toBeDefined();
    expect(files.find((f) => f.filename === 'lmas-qa.toml')).toBeDefined();
  ...
  it('derives command description from agent title and whenToUse', () => {
    ...
    const dev = files.find((f) => f.filename === 'lmas-dev.toml');
  ...
  it('falls back to generic description when metadata is missing', () => {
    ...
    const dev = files.find((f) => f.filename === 'lmas-dev.toml');
    expect(dev.content).toContain('description = "Ativar agente LMAS dev"');
  });
  ...
  it('writes command files to .gemini/commands', () => {
    ...
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'lmas-menu.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'lmas-dev.toml'))).toBe(true);
    expect(fs.existsSync(path.join(tmpRoot, '.gemini', 'commands', 'lmas-qa.toml'))).toBe(true);
  });
});
