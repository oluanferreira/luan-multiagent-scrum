...
const { syncSkills } = require('../../.lmas-core/infrastructure/scripts/codex-skills-sync/index');
const { validateCodexSkills } = require('../../.lmas-core/infrastructure/scripts/codex-skills-sync/validate');
...
describe('Codex Skills Validator', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-codex-validate-'));
  ...
  it('fails when a generated skill is missing', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    fs.rmSync(path.join(skillsDir, 'lmas-architect', 'SKILL.md'), { force: true });
  ...
  it('fails when greeting command is removed from a skill', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const target = path.join(skillsDir, 'lmas-dev', 'SKILL.md');
  ...
  it('fails in strict mode when orphaned lmas-* skill dir exists', () => {
    syncSkills({ sourceDir, localSkillsDir: skillsDir, dryRun: false });
    const orphanPath = path.join(skillsDir, 'lmas-legacy');
    ...
    expect(result.ok).toBe(false);
    expect(result.orphaned).toContain('lmas-legacy');
  });
});
