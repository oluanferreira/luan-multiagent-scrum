...
`;
  ...
  describe('Core Formatting', () => {
    test('generates valid output for Builder agent', () => {
      ...
      expect(output).toContain('**Agent:** Neo (Builder)');
    ...
    test('places Metrics section last', () => {
      ...
      expect(lines[lastSectionIndex]).toContain('— Neo');
    });
  });
  ...
  describe('Personality Injection', () => {
    ...
    test('injects signature closing correctly', () => {
      ...
      expect(signature).toBe('— Neo, sempre construindo 🔨');
    });
...
`;
    ...
    test('handles missing agent file gracefully', () => {
      ...
      expect(output).toContain('## 📊 Task Execution Report');
      expect(output).toContain('**Agent:** Neo (Agent)'); // Neutral fallback
    });
  ...
  describe('All 11 Agents', () => {
    const agents = [
      { id: 'dev', name: 'Neo', archetype: 'Builder', tone: 'pragmatic' },
      { id: 'qa', name: 'Oracle', archetype: 'Guardian', tone: 'analytical' },
      { id: 'po', name: 'Keymaker', archetype: 'Balancer', tone: 'collaborative' },
