...
} = require('../../.lmas-core/infrastructure/scripts/validate-parity');

describe('validate-parity', () => {
  function createMockProjectRoot() {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-parity-'));
  ...
  it('fails when docs matrix claim diverges from contract', () => {
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-parity-mismatch-'));
  ...
  it('includes contractDiff in parity result when --diff path is provided', () => {
    ...
    const ok = { ok: true, errors: [], warnings: [] };
    const result = runParityValidation(
      { projectRoot, diffPath: '.lmas-core/infrastructure/contracts/compatibility/lmas-4.0.3.yaml' },
      {
        runSyncValidate: () => ok,
        validateClaudeIntegration: () => ok,
        validateCodexIntegration: () => ok,
        ...
        loadCompatibilityContract: (contractPath) => {
          if (contractPath.endsWith('lmas-4.0.3.yaml')) {
            return {
              release: 'LMAS 4.0.3',
              global_required_checks: ['paths'],
              ide_matrix: [
    ...
    expect(result.contractDiff.from_release).toBe('LMAS 4.0.3');
    expect(result.contractDiff.to_release).toBe('LMAS 4.0.4');
