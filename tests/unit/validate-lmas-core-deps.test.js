...
const VALIDATOR_PATH = path.join(PROJECT_ROOT, 'scripts', 'validate-lmas-core-deps.js');

describe('validate-lmas-core-deps.js (INS-4.12)', () => {
  ...
  test('.lmas-core/package.json includes fast-glob', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.lmas-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fast-glob');
  });

  test('.lmas-core/package.json includes fs-extra', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.lmas-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('fs-extra');
  });

  test('.lmas-core/package.json includes semver', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.lmas-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('semver');
  });

  test('.lmas-core/package.json includes ajv', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.lmas-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    expect(pkg.dependencies).toHaveProperty('ajv');
  });

  test('.lmas-core/package.json includes tar', () => {
    const pkgPath = path.join(PROJECT_ROOT, '.lmas-core', 'package.json');
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
describe('validate-publish.js dependency check (INS-4.12)', () => {
  test('validate-publish includes dependency completeness check', () => {
    ...
    expect(content).toContain('validate-lmas-core-deps');
  });
});
...
describe('wizard NODE_PATH fix (INS-4.12)', () => {
  test('wizard/index.js sets NODE_PATH for entity registry bootstrap', () => {
    ...
    expect(content).toContain('lmasCoreNodeModules');
  });
...
describe('doctor npm-packages check (INS-4.12)', () => {
  test('doctor check validates .lmas-core/node_modules/', () => {
    const checkPath = path.join(PROJECT_ROOT, '.lmas-core', 'core', 'doctor', 'checks', 'npm-packages.js');
    const content = fs.readFileSync(checkPath, 'utf8');
    expect(content).toContain('lmasCoreNodeModules');
    expect(content).toContain('.lmas-core');
    expect(content).toContain('INS-4.12');
  });
});
