...
describe('SessionContextLoader', () => {
  ...
  describe('loadContext() - Session Detection', () => {
    ...
    test('detects existing session after agent activation', () => {
      ...
      loader.updateSession('po', 'Keymaker', 'validate-story-draft');
  ...
  describe('loadContext() - Agent Transition Tracking', () => {
    test('tracks agent transitions correctly', () => {
      ...
      loader.updateSession('po', 'Keymaker', 'validate-story-draft');
      loader.updateSession('dev', 'Neo', 'develop');

      // Load context for @qa
      const context = loader.loadContext('qa');

      expect(context.previousAgent.agentId).toBe('dev');
      expect(context.previousAgent.agentName).toBe('Neo');
    ...
    test('skips same agent in previous agent detection', () => {
      ...
      loader.updateSession('po', 'Keymaker', 'create-story');
      loader.updateSession('dev', 'Neo', 'develop');
      loader.updateSession('dev', 'Neo', 'run-tests');
    ...
    test('limits command history to 10 entries', () => {
      // Add 15 commands
      for (let i = 1; i <= 15; i++) {
        loader.updateSession('dev', 'Neo', `command-${i}`);
      }
  ...
  describe('updateSession() - Session State Management', () => {
    ...
    test('limits agent sequence to last 20 activations', () => {
      ...
      for (let i = 1; i <= 25; i++) {
        loader.updateSession('dev', 'Neo', `develop-${i}`);
      }
  ...
  describe('generateContextMessage() - Message Generation', () => {
    test('generates message with previous agent info', () => {
      ...
      expect(context.message).toContain('Keymaker');
  ...
  describe('getPreviousAgent() - Agent History', () => {
    ...
    test('returns most recent different agent', () => {
      const sessionState = {
        agentSequence: [
          { agentId: 'po', agentName: 'Keymaker', activatedAt: Date.now() - 10000, lastCommand: 'create-story' },
          { agentId: 'dev', agentName: 'Neo', activatedAt: Date.now() - 5000, lastCommand: 'develop' },
          { agentId: 'qa', agentName: 'Oracle', activatedAt: Date.now(), lastCommand: 'review' },
        ],
      };
      ...
      expect(previous.agentName).toBe('Neo');
    });
  });
