...
describe('IDE Config Generator', () => {
  ...
  describe('generateTemplateVariables', () => {
    it('should generate default variables', () => {
      ...
      expect(variables).toHaveProperty('lmasVersion');
    });
    ...
    it('should include LMAS version', () => {
      const wizardState = {};
      const variables = generateTemplateVariables(wizardState);

      expect(variables.lmasVersion).toBeDefined();
      expect(typeof variables.lmasVersion).toBe('string');
    });
  });
  ...
  describe('generateIDEConfigs', () => {
    ...
    it('should render template with variables', async () => {
      ...
      // v2.1 templates use static content from .lmas-core/templates/ide-rules/
    ...
    it('should create github-copilot config in .github directory', async () => {
      ...
      const content = await fs.readFile(configPath, 'utf8');
      // Should contain LMAS rules content
      expect(content).toContain('LMAS');
    });
    ...
    it('should configure Gemini hooks and settings with active LMAS hooks', async () => {
      ...
      const beforeAgentWrapper = settings.hooks.BeforeAgent.find(
        (w) => Array.isArray(w.hooks) && w.hooks.some((h) => h.name === 'lmas-context-inject'),
      );
      expect(beforeAgentWrapper).toBeDefined();
      const hook = beforeAgentWrapper.hooks.find((h) => h.name === 'lmas-context-inject');
