...
const {
  ...
  formatNotFoundError,
  wrapText,
} = require('../../.lmas-core/cli/commands/workers/formatters/info-formatter');
const { findSuggestions, findRelatedWorkers } = require('../../.lmas-core/cli/commands/workers/info');
const { levenshteinDistance } = require('../../.lmas-core/cli/commands/workers/search-keyword');
...
describe('Info Formatter - Pretty Output', () => {
  ...
  test('formatInfoPretty includes usage example', () => {
    ...
    expect(output).toContain('lmas task run json-csv-transformer');
  });
...
describe('Not Found Error Formatter', () => {
  ...
  test('formatNotFoundError includes search hint', () => {
    ...
    expect(output).toContain('lmas workers search invalid');
  });
});
