...
describe('SquadMigrator', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should detect missing lmas.type field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);

      expect(analysis.needsMigration).toBe(true);
      expect(analysis.issues.some((i) => i.type === 'MISSING_LMAS_TYPE')).toBe(true);
      expect(analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'lmas.type')).toBe(
        true,
      );
    });

    it('should detect missing lmas.minVersion field (AC 5)', async () => {
      const squadPath = path.join(FIXTURES_PATH, 'legacy-squad');
      const analysis = await migrator.analyze(squadPath);
      ...
      expect(
        analysis.actions.some((a) => a.type === 'ADD_FIELD' && a.path === 'lmas.minVersion'),
      ).toBe(true);
    });
    ...
    it('should return needsMigration: false for up-to-date squad', async () => {
      ...
      const manifest = {
        lmas: { type: 'squad', minVersion: '2.1.0' },
  ...
  describe('migrate()', () => {
    ...
    it('should add missing lmas.type and lmas.minVersion fields (AC 5)', async () => {
      // Copy legacy squad to temp dir
      const srcPath = path.join(FIXTURES_PATH, 'legacy-squad');
      ...
      expect(manifest.lmas?.minVersion).toBe('2.1.0');
    });
