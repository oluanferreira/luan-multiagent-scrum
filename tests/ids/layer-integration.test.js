...
const REGISTRY_PATH = path.resolve(REPO_ROOT, '.lmas-core/data/entity-registry.yaml');
...
describe('Layer Preservation — Registry Healer (AC: 6)', () => {
  test('healer _healIssue only heals known fields, does not strip layer', () => {
    ...
    const healerPath = path.resolve(REPO_ROOT, '.lmas-core/core/ids/registry-healer.js');
  ...
  test('healer functional: healing checksum preserves layer field (C3)', () => {
    ...
    const testEntity = {
      path: '.lmas-core/core/ids/layer-classifier.js',
      layer: 'L1',
      type: 'module',
  ...
  test('healer functional: healing keywords preserves layer field (C3)', () => {
    ...
    const testEntity = {
      path: '.lmas-core/data/entity-registry.yaml',
      layer: 'L3',
      type: 'data',
      keywords: [],
