...
const {
  generateFileHashes,
  generateVersionJson,
} = require('../../packages/installer/src/installer/lmas-core-installer');

describe('LMAS Core Installer - Version Tracking', () => {
  let tempDir;

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'lmas-installer-test-'));
    await fs.ensureDir(path.join(tempDir, '.lmas-core'));
  });
  ...
  describe('generateFileHashes', () => {
    it('should generate hashes for installed files', async () => {
      const lmasCoreDir = path.join(tempDir, '.lmas-core');

      // Create test files
      await fs.writeFile(path.join(lmasCoreDir, 'test1.md'), '# Test File 1');
      await fs.writeFile(path.join(lmasCoreDir, 'test2.md'), '# Test File 2');
      await fs.ensureDir(path.join(lmasCoreDir, 'agents'));
      await fs.writeFile(path.join(lmasCoreDir, 'agents', 'dev.md'), '# Dev Agent');

      const installedFiles = ['test1.md', 'test2.md', 'agents/dev.md'];
      const hashes = await generateFileHashes(lmasCoreDir, installedFiles);
    ...
    it('should skip non-existent files', async () => {
      ...
      await fs.writeFile(path.join(lmasCoreDir, 'exists.md'), '# Exists');
    ...
    it('should skip directories', async () => {
      ...
      await fs.ensureDir(path.join(lmasCoreDir, 'agents'));
      await fs.writeFile(path.join(lmasCoreDir, 'file.md'), '# File');
    ...
    it('should generate consistent hashes for same content', async () => {
      ...
      await fs.writeFile(path.join(lmasCoreDir, 'file1.md'), 'Same content');
      await fs.writeFile(path.join(lmasCoreDir, 'file2.md'), 'Same content');
    ...
    it('should generate different hashes for different content', async () => {
      ...
      await fs.writeFile(path.join(lmasCoreDir, 'file1.md'), 'Content A');
      await fs.writeFile(path.join(lmasCoreDir, 'file2.md'), 'Content B');
  ...
  describe('generateVersionJson', () => {
    it('should create version.json with correct structure', async () => {
      ...
      // Create test files
      await fs.writeFile(path.join(lmasCoreDir, 'test.md'), '# Test');

      const result = await generateVersionJson({
        targetLmasCore: lmasCoreDir,
        version: '1.2.0',
    ...
    it('should write version.json to disk', async () => {
      ...
      await fs.writeFile(path.join(lmasCoreDir, 'agent.md'), '# Agent');
      ...
      const versionJsonPath = path.join(lmasCoreDir, 'version.json');
    ...
    it('should include file hashes in version.json', async () => {
      ...
      await fs.ensureDir(path.join(lmasCoreDir, 'agents'));
      await fs.writeFile(path.join(lmasCoreDir, 'agents', 'dev.md'), '# Dev');
      await fs.writeFile(path.join(lmasCoreDir, 'config.yaml'), 'key: value');
