...
describe('v2.1 Module Structure Tests', () => {
  describe('INS-01: Source Path Configuration', () => {
    it('should point getLmasCorePath to .lmas-core directory', () => {
      const lmasCorePath = resourceLocator.getLmasCorePath();
      expect(lmasCorePath).toContain('.lmas-core');
      expect(lmasCorePath).not.toMatch(/[^.]lmas-core$/);
    });

    it('should have .lmas-core as source for config-loader', () => {
      const lmasCorePath = configLoader.getLmasCorePath();
      expect(lmasCorePath).toContain('.lmas-core');
    });
  });
  ...
  describe('INS-02: Agent Path Resolution', () => {
    ...
    it('should list agents from development/agents/', async () => {
      ...
      for (const agent of agents) {
        expect(agent.file).toContain('.lmas-core/development/agents/');
      }
    });
  });
  ...
  describe('INS-04: Resource Locator v2.1 Paths', () => {
    it('should resolve agent dependencies with v2.1 module mapping', async () => {
      ...
      for (const depPath of deps.all) {
        // Should not have flat structure paths like .lmas-core/tasks/
        expect(depPath).not.toMatch(/\.lmas-core\/(tasks|templates|checklists|workflows|utils|data)\//);
  ...
  describe('INS-05: Manifest Files Location', () => {
    it('should have manifests directory in .lmas-core', async () => {
      const manifestsPath = path.join(resourceLocator.getLmasCorePath(), 'manifests');
      const exists = await fs.pathExists(manifestsPath);
      expect(exists).toBe(true);
    });

    it('should have agents.csv manifest', async () => {
      const agentsCsvPath = path.join(resourceLocator.getLmasCorePath(), 'manifests', 'agents.csv');
      const exists = await fs.pathExists(agentsCsvPath);
      expect(exists).toBe(true);
    });

    it('should have tasks.csv manifest', async () => {
      const tasksCsvPath = path.join(resourceLocator.getLmasCorePath(), 'manifests', 'tasks.csv');
      const exists = await fs.pathExists(tasksCsvPath);
      expect(exists).toBe(true);
    });

    it('should have workers.csv manifest', async () => {
      const workersCsvPath = path.join(resourceLocator.getLmasCorePath(), 'manifests', 'workers.csv');
