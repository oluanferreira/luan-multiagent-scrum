...
describe('PostInstallValidator Security Tests', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `lmas-validator-test-${Date.now()}`);
    targetDir = path.join(testDir, 'target');
    sourceDir = path.join(testDir, 'source');

    await fs.ensureDir(path.join(targetDir, '.lmas-core'));
    await fs.ensureDir(path.join(sourceDir, '.lmas-core'));
  });
  ...
  describe('Symlink Rejection', () => {
    test('should reject symlinks during validation', async () => {
      ...
      const realFile = path.join(targetDir, '.lmas-core', 'real.txt');
      const symlink = path.join(targetDir, '.lmas-core', 'link.txt');
      ...
      await fs.writeFile(
        path.join(targetDir, '.lmas-core', 'install-manifest.yaml'),
        'version: "1.0.0"\nfiles:\n  - path: link.txt',
      );
  ...
  describe('Signature Verification', () => {
    ...
    test('should allow validation without signature in dev mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'test.txt'), 'test');
  ...
  describe('Quick Mode Safety (H2)', () => {
    test('should fail when size is missing in quick mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'test.txt'), 'content');
    ...
    test('should fail on size mismatch in quick mode', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'test.txt'), 'small');
  ...
  describe('Secure Repair (C4)', () => {
    ...
    test('should verify source hash before copying', async () => {
      ...
      const sourceFile = path.join(sourceDir, '.lmas-core', 'test.txt');
...
files:
  - path: test.txt
    ...
    size: 13`;
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'install-manifest.yaml'), manifest);
      await fs.writeFile(path.join(sourceDir, '.lmas-core', 'install-manifest.yaml'), manifest);
  ...
  describe('Hash Error Handling (H3)', () => {
    test('should treat hash errors as failures', async () => {
      ...
      const dirPath = path.join(targetDir, '.lmas-core', 'notafile');
  ...
  describe('DoS Protection (H6)', () => {
    ...
    test('should enforce max manifest size', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'install-manifest.yaml'), bigContent);
    ...
    test('should use byte length not character length for size check (DOS-4)', async () => {
      ...
      await fs.writeFile(path.join(targetDir, '.lmas-core', 'install-manifest.yaml'), emojiContent);
