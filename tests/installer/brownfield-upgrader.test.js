...
describe('brownfield-upgrader', () => {
  ...
  describe('generateUpgradeReport', () => {
    ...
    it('should identify modified files', () => {
      const lmasCoreDir = path.join(targetDir, '.lmas-core');
      fs.ensureDirSync(lmasCoreDir);
      fs.writeFileSync(path.join(lmasCoreDir, 'changed.md'), 'original');
      const originalHash = `sha256:${hashFile(path.join(lmasCoreDir, 'changed.md'))}`;
    ...
    it('should identify user-modified files', () => {
      const lmasCoreDir = path.join(targetDir, '.lmas-core');
      fs.ensureDirSync(lmasCoreDir);
      fs.writeFileSync(path.join(lmasCoreDir, 'user-changed.md'), 'user modified content');
  ...
  describe('applyUpgrade', () => {
    ...
    it('should install new files', async () => {
      ...
      expect(fs.existsSync(path.join(targetDir, '.lmas-core', 'new-file.md'))).toBe(true);
    });
    ...
    it('should perform dry run without modifying files', async () => {
      ...
      expect(result.filesInstalled).toHaveLength(1);
      expect(fs.existsSync(path.join(targetDir, '.lmas-core', 'new-file.md'))).toBe(false);
    });
  });
  ...
  describe('updateInstalledManifest', () => {
    it('should create installed manifest file', () => {
      ...
      fs.ensureDirSync(path.join(targetDir, '.lmas-core'));
      updateInstalledManifest(targetDir, sourceManifest, 'lmas-core@2.1.0');

      const installedPath = path.join(targetDir, '.lmas-core', '.installed-manifest.yaml');
      expect(fs.existsSync(installedPath)).toBe(true);

      const content = yaml.load(fs.readFileSync(installedPath, 'utf8'));
      expect(content.installed_version).toBe('2.1.0');
      expect(content.installed_from).toBe('lmas-core@2.1.0');
    });
  });
