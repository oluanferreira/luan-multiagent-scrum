 ...
 * 4. No {root} placeholders remain (should be replaced with .lmas-core)
 *
...
// Path to .lmas-core directory
const LMAS_CORE_PATH = path.join(__dirname, '..', '..', '.lmas-core');
...
function resolveDependencyPath(type, filename) {
  ...
  if (!modulePath) {
    ...
    return path.join(LMAS_CORE_PATH, 'development', type, filename);
  }
  return path.join(LMAS_CORE_PATH, modulePath, filename);
}
...
describe('v2.1 Path Validation', () => {
  ...
  before(async () => {
    // Load all agents
    const agentDir = path.join(LMAS_CORE_PATH, 'development', 'agents');
    if (await fs.pathExists(agentDir)) {
      ...
      for (const file of agentFiles) {
        ...
        agents.push({
          file: path.relative(LMAS_CORE_PATH, file),
    ...
    // Load all tasks
    const taskDir = path.join(LMAS_CORE_PATH, 'development', 'tasks');
    ...
    // Load all workflows
    const workflowDir = path.join(LMAS_CORE_PATH, 'development', 'workflows');
    ...
    allFiles = await getFilesRecursive(LMAS_CORE_PATH);
  });
        ...
        for (const task of taskDeps) {
          ...
          if (!await fs.pathExists(taskPath)) {
            missingDeps.push({
              ...
              type: 'tasks',
              dependency: task,
              expectedPath: path.relative(LMAS_CORE_PATH, taskPath),
            });
          }
        }
      }
        ...
        for (const checklist of checklistDeps) {
          ...
          if (!await fs.pathExists(checklistPath)) {
            missingDeps.push({
              ...
              expectedPath: path.relative(LMAS_CORE_PATH, checklistPath),
            });
          }
        }
      }
        ...
        for (const template of templateDeps) {
          ...
          if (!await fs.pathExists(templatePath)) {
            missingDeps.push({
              ...
              type: 'templates',
              dependency: template,
              expectedPath: path.relative(LMAS_CORE_PATH, templatePath),
            });
          }
        }
      }
          ...
          for (const dep of deps) {
            ...
            if (!await fs.pathExists(depPath)) {
              missingDeps.push({
                ...
                type,
                dependency: dep,
                expectedPath: path.relative(LMAS_CORE_PATH, depPath),
              });
            }
          }
        }
          ...
          if (step.agent) {
            ...
            if (!await fs.pathExists(agentPath)) {
              invalidRefs.push({
                ...
                expectedPath: path.relative(LMAS_CORE_PATH, agentPath),
              });
            }
          }
  ...
  describe('v2.1 Module Structure Validation', () => {
    it('should have core module directory', async () => {
      const coreDir = path.join(LMAS_CORE_PATH, 'core');
    ...
    it('should have development module directory', async () => {
      const devDir = path.join(LMAS_CORE_PATH, 'development');
    ...
    it('should have product module directory', async () => {
      const productDir = path.join(LMAS_CORE_PATH, 'product');
    ...
    it('should have infrastructure module directory', async () => {
      const infraDir = path.join(LMAS_CORE_PATH, 'infrastructure');
    ...
    it('should have agents in development/agents/', async () => {
      const agentsDir = path.join(LMAS_CORE_PATH, 'development', 'agents');
    ...
    it('should have tasks in development/tasks/', async () => {
      const tasksDir = path.join(LMAS_CORE_PATH, 'development', 'tasks');
    ...
    it('should have templates in product/templates/', async () => {
      const templatesDir = path.join(LMAS_CORE_PATH, 'product', 'templates');
    ...
    it('should have checklists in product/checklists/', async () => {
      const checklistsDir = path.join(LMAS_CORE_PATH, 'product', 'checklists');
...
async function generateValidationReport() {
  ...
  // Check modules exist
  report.modules.core = await fs.pathExists(path.join(LMAS_CORE_PATH, 'core'));
  report.modules.development = await fs.pathExists(path.join(LMAS_CORE_PATH, 'development'));
  report.modules.product = await fs.pathExists(path.join(LMAS_CORE_PATH, 'product'));
  report.modules.infrastructure = await fs.pathExists(path.join(LMAS_CORE_PATH, 'infrastructure'));
