...
// ---------------------------------------------------------------------------
const { SynapseEngine } = require(path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'engine.js'));
const { parseManifest } = require(path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'domain', 'domain-loader.js'));
const { estimateTokens } = require(path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'utils', 'tokens.js'));
const {
  estimateContextPercent,
  calculateBracket,
  getActiveLayers,
  getTokenBudget,
} = require(path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'context', 'context-tracker.js'));
const GitConfigDetector = require(path.join(PROJECT_ROOT, '.lmas-core', 'infrastructure', 'scripts', 'git-config-detector.js'));

let UnifiedActivationPipeline;
try {
  const uapModule = require(path.join(PROJECT_ROOT, '.lmas-core', 'development', 'scripts', 'unified-activation-pipeline.js'));
...
try {
  ProjectStatusLoader = require(path.join(PROJECT_ROOT, '.lmas-core', 'infrastructure', 'scripts', 'project-status-loader.js'));
} catch (err) {
...
This audit measures every stage of the LMAS activation pipeline (UAP + SYNAPSE + Session + Git Detection)
