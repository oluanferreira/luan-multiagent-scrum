...
const { UnifiedActivationPipeline } = require(
  path.join(PROJECT_ROOT, '.lmas-core', 'development', 'scripts', 'unified-activation-pipeline.js'),
);
const { collectTimingMetrics } = require(
  path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'diagnostics', 'collectors', 'timing-collector.js'),
);
const { safeReadJson } = require(
  path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'diagnostics', 'collectors', 'safe-read-json.js'),
);
...
function checkCodeIntel() {
  try {
    const codeIntel = require(path.join(PROJECT_ROOT, '.lmas-core', 'core', 'code-intel', 'index.js'));
