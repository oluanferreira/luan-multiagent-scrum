...
async function runBenchmark(options) {
  const { SynapseEngine } = require(
    path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'engine.js'),
  );
  const { loadSession } = require(
    path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'session', 'session-manager.js'),
  );
  const { parseManifest } = require(
    path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'domain', 'domain-loader.js'),
  );
  ...
  const { formatSynapseRules } = require(
    path.join(PROJECT_ROOT, '.lmas-core', 'core', 'synapse', 'output', 'formatter.js'),
  );
