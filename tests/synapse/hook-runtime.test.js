...
const {
  resolveHookRuntime,
  buildHookOutput,
} = require('../../.lmas-core/core/synapse/runtime/hook-runtime');
...
describe('hook-runtime', () => {
  ...
  it('resolves runtime when required modules and .synapse exist', () => {
    ...
    try {
      ...
      writeFile(
        path.join(cwd, '.lmas-core/core/synapse/session/session-manager.js'),
        "module.exports = { loadSession: () => ({ prompt_count: 7, id: 's-1' }), cleanStaleSessions: () => 0 };",
      );
      writeFile(
        path.join(cwd, '.lmas-core/core/synapse/engine.js'),
        [
          'class SynapseEngine {',
  ...
  it('calls cleanStaleSessions on first prompt (prompt_count === 0)', () => {
    ...
    try {
      ...
      const smPath = path.join(cwd, '.lmas-core', 'core', 'synapse', 'session', 'session-manager.js');
      const engPath = path.join(cwd, '.lmas-core', 'core', 'synapse', 'engine.js');
