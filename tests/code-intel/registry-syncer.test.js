...
const { RegistrySyncer, inferRole, ROLE_MAP } = require('../../.lmas-core/core/code-intel/registry-syncer');
    ...
    RegistryLoader: jest.fn().mockImplementation(() => ({
      load: jest.fn().mockReturnValue({
        ...
        entities: {
          tasks: {
            'dev-develop-story': {
              path: '.lmas-core/development/tasks/dev-develop-story.md',
              type: 'task',
            ...
            'create-next-story': {
              path: '.lmas-core/development/tasks/create-next-story.md',
              type: 'task',
          ...
          scripts: {
            'greeting-builder': {
              path: '.lmas-core/development/scripts/greeting-builder.js',
              type: 'script',
...
function createMockClient(overrides = {}) {
  return {
    findReferences: jest.fn().mockResolvedValue([
      { file: '.lmas-core/development/tasks/create-next-story.md' },
    ]),
...
describe('RegistrySyncer', () => {
  ...
  describe('usedBy population (T2 — AC2)', () => {
    ...
    it('should deduplicate usedBy entries', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.lmas-core/development/tasks/create-next-story.md' },
          { file: '.lmas-core/development/tasks/create-next-story.md' }, // Duplicate
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
      ...
      const entities = {
        tasks: {
          'test-entity': {
            path: '.lmas-core/development/tasks/test.md',
            usedBy: [],
            dependencies: [],
          },
        },
  ...
  describe('codeIntelMetadata schema (T4 — AC4)', () => {
    ...
    it('should set callerCount based on usedBy length', async () => {
      const mockClient = createMockClient({
        findReferences: jest.fn().mockResolvedValue([
          { file: '.lmas-core/development/tasks/create-next-story.md' },
          { file: '.lmas-core/development/scripts/greeting-builder.js' },
        ]),
      });
      const syncer = createSyncer({ client: mockClient });
    ...
    it('should set provider from active provider name', async () => {
      ...
      const entity = {
        id: 'test',
        category: 'tasks',
        data: { path: '.lmas-core/development/tasks/test.md', usedBy: [], dependencies: [] },
      };
  ...
  describe('inferRole', () => {
    it('should infer role from path patterns', () => {
      expect(inferRole('.lmas-core/development/tasks/dev.md')).toBe('task');
      expect(inferRole('.lmas-core/development/agents/dev.md')).toBe('agent');
      expect(inferRole('.lmas-core/development/workflows/sdc.yaml')).toBe('workflow');
      expect(inferRole('.lmas-core/development/scripts/build.js')).toBe('script');
      expect(inferRole('.lmas-core/core/utils/helper.js')).toBe('module');
      expect(inferRole('.lmas-core/data/entity-registry.yaml')).toBe('config');
      expect(inferRole('.lmas-core/product/templates/prd.yaml')).toBe('template');
    });
