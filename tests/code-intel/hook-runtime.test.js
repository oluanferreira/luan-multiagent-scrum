...
const { resolveCodeIntel, formatAsXml, escapeXml, _resetForTesting } = require(
  path.join(__dirname, '..', '..', '.lmas-core', 'core', 'code-intel', 'hook-runtime.js'),
);
...
describe('code-intel hook-runtime', () => {
  ...
  describe('resolveCodeIntel', () => {
    ...
    it('handles absolute paths by normalizing to relative', async () => {
      const absPath = path.join(PROJECT_ROOT, '.lmas-core', 'development', 'tasks', 'create-next-story.md');
  ...
  describe('formatAsXml', () => {
    ...
    it('generates valid XML with entity data', () => {
      const intel = {
        entity: {
          file: '.lmas-core/core/some-module.js',
      ...
      expect(xml).toContain('<path>.lmas-core/core/some-module.js</path>');
