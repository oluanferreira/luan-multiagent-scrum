...
const { maskEmail, openBrowser, promptEmail, recoverLicense, RECOVERY_URL, RECOVERY_MESSAGE } = require('../packages/lmas-pro-cli/src/recover');

// ─── Constants ────────────────────────────────────────────────────────────────

describe('Recovery Constants', () => {
  test('RECOVERY_URL is the correct portal URL', () => {
    expect(RECOVERY_URL).toBe('https://lmas-license-server.vercel.app/reset-password');
  });
...
describe('CLI alias reset-password', () => {
  test('lmas-pro.js switch handles reset-password same as recover', () => {
    // Verify the CLI entry point has reset-password as a case that calls recoverLicense
    const cliSource = require('fs').readFileSync(
      require('path').join(__dirname, '../packages/lmas-pro-cli/bin/lmas-pro.js'),
      'utf-8'
    );
