...
const WorktreeManager = require('../../.lmas-core/infrastructure/scripts/worktree-manager');
...
describe('WorktreeManager', () => {
  ...
  describe('constructor', () => {
    it('should use default options when none provided', () => {
      ...
      expect(mgr.worktreeDir).toBe('.lmas/worktrees');
  ...
  describe('getWorktreePath', () => {
    it('should return correct path for story ID', () => {
      ...
      expect(result).toBe(path.join(projectRoot, '.lmas/worktrees', 'STORY-42'));
    });
  });
  ...
  describe('exists', () => {
    it('should return true when worktree directory exists', async () => {
      ...
      expect(fs.access).toHaveBeenCalledWith(path.join(projectRoot, '.lmas/worktrees', 'STORY-42'));
    });
  ...
  describe('create', () => {
    ...
    it('should create worktree with correct git commands', async () => {
      ...
      expect(fs.mkdir).toHaveBeenCalledWith(path.join(projectRoot, '.lmas/worktrees'), {
        recursive: true,
      });
      expect(execa).toHaveBeenCalledWith(
        ...
        [
          'worktree',
          'add',
          path.join(projectRoot, '.lmas/worktrees', 'STORY-42'),
          '-b',
    ...
    it('should throw error if max worktrees limit reached', async () => {
      ...
      execa.mockResolvedValue({
        stdout: Array(10)
          ...
          .map(
            (_, i) =>
              `worktree /test/project/.lmas/worktrees/STORY-${i}\nbranch refs/heads/auto-claude/STORY-${i}`,
          )
          .join('\n\n'),
        stderr: '',
      });
  ...
  describe('remove', () => {
    ...
    it('should remove worktree and branch', async () => {
      ...
      expect(execa).toHaveBeenCalledWith(
        'git',
        ['worktree', 'remove', path.join(projectRoot, '.lmas/worktrees', 'STORY-42')],
        expect.objectContaining({ cwd: projectRoot }),
      );
      expect(execa).toHaveBeenCalledWith(
        'git',
    ...
    it('should force remove when option is set', async () => {
      ...
      expect(execa).toHaveBeenCalledWith(
        'git',
        ['worktree', 'remove', path.join(projectRoot, '.lmas/worktrees', 'STORY-42'), '--force'],
        expect.objectContaining({ cwd: projectRoot }),
      );
      expect(execa).toHaveBeenCalledWith(
        'git',
...
worktree /test/project/.lmas/worktrees/STORY-42
branch refs/heads/auto-claude/STORY-42

worktree /test/project/.lmas/worktrees/STORY-43
branch refs/heads/auto-claude/STORY-43`;
  ...
  describe('getCount', () => {
    it('should return correct counts', async () => {
      ...
      const porcelainOutput = `worktree /test/project/.lmas/worktrees/STORY-OLD
branch refs/heads/auto-claude/STORY-OLD

worktree /test/project/.lmas/worktrees/STORY-NEW
