...
} = require('../../../packages/lmas-install/src/installer');

const {
  checkAllDependencies,
  checkDockerRunning,
} = require('../../../packages/lmas-install/src/dep-checker');

const {
  checkDocker,
  ensureDocker,
} = require('../../../packages/lmas-install/src/edmcp');
...
describe('Edge Cases - Task 7.7', () => {
  ...
  describe('No Internet Connectivity', () => {
    ...
    it('should handle npm install failure due to network error', async () => {
      ...
      // When/Then
      await expect(async () => {
        await execa('npm', ['install', 'lmas-core']);
  ...
  describe('Insufficient Permissions', () => {
    ...
    it('should handle permission denied during npm install', async () => {
      ...
      // When/Then
      await expect(async () => {
        await execa('npm', ['install', '-g', 'lmas-core']);
      }).rejects.toThrow('EACCES');
    });
