...
const PKG_DIR = path.resolve(__dirname, '../../../packages/lmas-install');
...
describe('Integration - Task 8.3: Local NPX Execution', () => {
  ...
  describe('Package Structure Validation', () => {
    it('should have valid package.json', () => {
      ...
      // Then
      expect(pkgJson.name).toBe('@lmas/lmas-install');
      expect(pkgJson.bin).toBeDefined();
      expect(pkgJson.bin['lmas-install']).toBe('./bin/lmas-install.js');
    ...
    it('should have executable shebang in bin files', () => {
      // Given
      const binFiles = ['bin/lmas-install.js', 'bin/edmcp.js'];
  ...
  describe('CLI Execution Tests', () => {
    it('should execute lmas-install --version via node', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/lmas-install.js');
    ...
    it('should show help for lmas-install', () => {
      // Given
      const binPath = path.join(PKG_DIR, 'bin/lmas-install.js');
      ...
      // Then
      expect(result).toContain('lmas-install');
  ...
  (runNpxIntegration ? describe : describe.skip)('NPX Local Execution Simulation', () => {
    it('should execute via npm exec (simulates npx .)', () => {
      ...
      // When
      const result = execSync('npm exec -- lmas-install --version', {
        cwd: PKG_DIR,
        encoding: 'utf8',
        timeout: 90000,
