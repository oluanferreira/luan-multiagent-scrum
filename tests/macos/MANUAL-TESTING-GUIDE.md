...
This guide provides step-by-step instructions for manually testing LMAS on macOS (Intel and Apple Silicon).
...
```bash
# Backup existing LMAS installation (if any)
if [ -d "$HOME/.lmas" ]; then
    mv "$HOME/.lmas" "$HOME/.lmas.backup.$(date +%Y%m%d-%H%M%S)"
fi

# Verify backup
ls -la "$HOME/.lmas.backup"*
```

### 2. Navigate to Test Directory

```bash
cd path/to/lmas-core/tests/macos
chmod +x *.sh
```
...
```bash
# Check LMAS command availability
which lmas
# Expected: /usr/local/bin/lmas or similar
...
```bash
# Check LMAS installation path
which lmas
# Should use forward slashes: /usr/local/bin/lmas
# NOT backslashes: \usr\local\bin\lmas
```
...
cd ~/.lmas
pwd
# Should show full path: /Users/username/.lmas
```

#### 3. Test Symlinks

```bash
# Check if lmas command is a symlink
ls -la $(which lmas)
...
```bash
# Check line endings of LMAS config
file ~/.lmas/config.json
# Should show: ASCII text (LF line endings)
# NOT: ASCII text, with CRLF line endings

# Check for carriage returns
grep -r $'\r' ~/.lmas/ || echo "No CRLF found - PASS"
```
...
ls -la ~/.lmas/bin/*.sh 2>/dev/null
# Should show: -rwxr-xr-x (executable)

# Try running a script without chmod
~/.lmas/bin/some-script.sh
...
time npx @lmasai/lmas@latest init
...
- [ ] No "unidentified developer" warnings for LMAS
...
```bash
# Create backup
cp -r ~/.lmas ~/.lmas.backup

# Simulate corruption
echo "corrupted" > ~/.lmas/config.json
...
- Permission denied: `chmod 000 ~/.lmas`
...
npx @lmasai/lmas@latest init --verbose
```

#### Command Not Found

**Symptom:** `bash: lmas: command not found`
...
# Check PATH
echo $PATH | grep -o '/[^:]*lmas[^:]*'

# Manually add to PATH if needed
export PATH="$HOME/.lmas/bin:$PATH"
```
...
**Symptom:** `lmas health` shows unhealthy MCPs

**Solution:**

```bash
# Reinstall specific MCP
lmas mcp reinstall <mcp-name>
...
2. **Test Logs:**
   - All files from `/tmp/lmas-test-*.log`
...
**Story:** 1.10b - macOS Testing & Validation
**Reviewed by:** Oracle (QA Guardian) 🛡️
