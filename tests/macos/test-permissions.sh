...
LOG_FILE="/tmp/lmas-test-perms-$(date +%Y%m%d-%H%M%S).log"
...
test_script_permissions() {
    ...
    # Find LMAS scripts
    if [[ -d "$HOME/.lmas/bin" ]]; then
        SCRIPTS=$(find "$HOME/.lmas/bin" -type f -name "*.sh" 2>/dev/null || true)
...
test_directory_permissions() {
    ...
    if [[ -d "$HOME/.lmas" ]]; then
        DIR_PERMS=$(stat -f "%Lp" "$HOME/.lmas" 2>/dev/null || stat -c "%a" "$HOME/.lmas" 2>/dev/null)
...
test_no_sudo_required() {
    log_info "Test 4: Verifying no sudo required for LMAS operations..."

    # Check if current user owns .lmas directory
    if [[ -d "$HOME/.lmas" ]]; then
        OWNER=$(stat -f "%Su" "$HOME/.lmas" 2>/dev/null || stat -c "%U" "$HOME/.lmas" 2>/dev/null)

        if [[ "$OWNER" == "$(whoami)" ]]; then
            pass_test "User owns .lmas directory (no sudo needed)"
        else
            fail_test ".lmas owned by: $OWNER (expected: $(whoami))"
        fi
    fi
}
