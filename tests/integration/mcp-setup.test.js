...
describe('MCP Setup Integration', () => {
  const cliIndexPath = path.join(__dirname, '../../.lmas-core/cli/index.js');
  let tempDir;
  let originalHome;
  ...
  describe('CLI Module Loading', () => {
    it('should load mcp command module without errors', () => {
      expect(() => {
        require('../../.lmas-core/cli/commands/mcp');
      }).not.toThrow();
    });

    it('should export createMcpCommand function', () => {
      const mcpModule = require('../../.lmas-core/cli/commands/mcp');
      expect(typeof mcpModule.createMcpCommand).toBe('function');
    });

    it('should load all mcp subcommand modules', () => {
      expect(() => {
        require('../../.lmas-core/cli/commands/mcp/setup');
        require('../../.lmas-core/cli/commands/mcp/link');
        require('../../.lmas-core/cli/commands/mcp/status');
        require('../../.lmas-core/cli/commands/mcp/add');
      }).not.toThrow();
    });
  });
  ...
  describe('Core Module Loading', () => {
    it('should load os-detector module', () => {
      const osDetector = require('../../.lmas-core/core/mcp/os-detector');

      expect(osDetector.detectOS).toBeDefined();
      expect(osDetector.isWindows).toBeDefined();
      expect(osDetector.getGlobalLmasDir).toBeDefined();
      expect(osDetector.getGlobalMcpDir).toBeDefined();
    });

    it('should load global-config-manager module', () => {
      const configManager = require('../../.lmas-core/core/mcp/global-config-manager');
    ...
    it('should load symlink-manager module', () => {
      const symlinkManager = require('../../.lmas-core/core/mcp/symlink-manager');
    ...
    it('should load config-migrator module', () => {
      const configMigrator = require('../../.lmas-core/core/mcp/config-migrator');
    ...
    it('should load mcp index module with all exports', () => {
      const mcp = require('../../.lmas-core/core/mcp');
  ...
  describe('OS Detection', () => {
    const { detectOS, isWindows, isMacOS, isLinux, getOSInfo } = require('../../.lmas-core/core/mcp/os-detector');
  ...
  describe('Server Templates', () => {
    const { getAvailableTemplates, getServerTemplate, SERVER_TEMPLATES } = require('../../.lmas-core/core/mcp/global-config-manager');
  ...
  describe('Path Generation', () => {
    ...
    it('should generate correct global LMAS directory path', () => {
      const lmasDir = getGlobalLmasDir();

      expect(lmasDir).toContain('.lmas');
      expect(path.isAbsolute(lmasDir)).toBe(true);
    });

    it('should generate correct MCP directory path', () => {
      const mcpDir = getGlobalMcpDir();

      expect(mcpDir).toContain('.lmas');
  ...
  describe('Link Status Constants', () => {
    const { LINK_STATUS } = require('../../.lmas-core/core/mcp/symlink-manager');
  ...
  describe('Migration Options Constants', () => {
    const { MIGRATION_OPTION } = require('../../.lmas-core/core/mcp/config-migrator');
  ...
  describe('Default Config Structure', () => {
    const { DEFAULT_CONFIG } = require('../../.lmas-core/core/mcp/global-config-manager');
  ...
  describe('CLI Help Output', () => {
    const { createMcpCommand } = require('../../.lmas-core/cli/commands/mcp');
...
describe('MCP CLI Command Registration', () => {
  it('should have mcp command in main CLI', () => {
    const { createProgram } = require('../../.lmas-core/cli/index');
