 ...
 * Tests for npx lmas-core@latest execution
 */
...
describeIntegration('npx Execution', () => {
  ...
  describeIntegration('Package Configuration', () => {
    it('should have correct bin entries in package.json', () => {
      ...
      expect(packageJson.bin['lmas']).toBe('./bin/lmas.js');
      expect(packageJson.bin['lmas-core']).toBe('./bin/lmas.js');
    });
    ...
    it('should include required files in package', () => {
      ...
      // Note: .lmas-core/ should be included if it exists
      const hasLmasCore = packageJson.files.some(f => 
        f === '.lmas-core/' || f === 'lmas-core/',
      );
      expect(hasLmasCore).toBe(true);
    });
  ...
  describeIntegration('CLI Execution', () => {
    it('should execute lmas command with --version', (done) => {
    ...
    it('should execute lmas command with --help', (done) => {
      const cliPath = path.join(__dirname, '../../bin/lmas.js');
      ...
      child.on('close', (code) => {
        ...
        expect(output).toContain('lmas-core');
        done();
      });
    }, timeout);

    it('should execute lmas info command', (done) => {
      const cliPath = path.join(__dirname, '../../bin/lmas.js');
  ...
  describeIntegration('Index.js init Export', () => {
    ...
    it('should maintain LMAS class export in source', () => {
      const indexPath = path.join(__dirname, '../../index.js');
      const indexContent = fs.readFileSync(indexPath, 'utf8');

      // Verify LMAS class exists in exports
      expect(indexContent).toContain('LMAS');
      expect(indexContent).toContain('class LMAS');
    });
