...
describe('Pattern Learning Integration', () => {
  ...
  beforeAll(() => {
    learningModule = require('../../../.lmas-core/workflow-intelligence/learning');
  });
  ...
  describe('Capture Hook Integration', () => {
    ...
    beforeAll(() => {
      captureHook = require('../../../.lmas-core/workflow-intelligence/learning/capture-hook');
    });
    ...
    it('should handle disabled state gracefully', async () => {
      // Save original env
      const originalEnv = process.env.LMAS_PATTERN_CAPTURE;

      try {
        process.env.LMAS_PATTERN_CAPTURE = 'false';
        captureHook.reset();
        ...
        if (originalEnv !== undefined) {
          process.env.LMAS_PATTERN_CAPTURE = originalEnv;
        } else {
          delete process.env.LMAS_PATTERN_CAPTURE;
        }
        captureHook.reset();
      }
    });
  ...
  describe('WIS Integration', () => {
    it('should be accessible from main WIS module', () => {
      const wis = require('../../../.lmas-core/workflow-intelligence');
