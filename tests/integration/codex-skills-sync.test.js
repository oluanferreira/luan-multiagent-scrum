...
const {
  syncSkills,
  buildSkillContent,
} = require('../../.lmas-core/infrastructure/scripts/codex-skills-sync/index');
...
describe('Codex Skills Sync', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lmas-codex-skills-'));
    expectedAgentCount = fs.readdirSync(path.join(process.cwd(), '.lmas-core', 'development', 'agents'))
  ...
  it('generates one SKILL.md per LMAS agent in local .codex/skills', () => {
    const localSkillsDir = path.join(tmpRoot, '.codex', 'skills');
    const result = syncSkills({
      sourceDir: path.join(process.cwd(), '.lmas-core', 'development', 'agents'),
      localSkillsDir,
      dryRun: false,
    });

    expect(result.generated).toBe(expectedAgentCount);
    const expected = path.join(localSkillsDir, 'lmas-architect', 'SKILL.md');
    expect(fs.existsSync(expected)).toBe(true);

    const content = fs.readFileSync(expected, 'utf8');
    expect(content).toContain('name: lmas-architect');
    expect(content).toContain('Activation Protocol');
    expect(content).toContain('.lmas-core/development/agents/architect.md');
  ...
  it('supports global installation path when --global mode is enabled', () => {
    ...
    expect(fs.existsSync(path.join(globalSkillsDir, 'lmas-dev', 'SKILL.md'))).toBe(true);
  });
  ...
  it('treats globalOnly as global output and skips local writes', () => {
    ...
    expect(result.generated).toBe(expectedAgentCount);
    expect(result.globalSkillsDir).toBe(globalSkillsDir);
    expect(fs.existsSync(path.join(localSkillsDir, 'lmas-dev', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(globalSkillsDir, 'lmas-dev', 'SKILL.md'))).toBe(true);
  });
