...
const {
  ...
  DEFAULT_COLOR,
  LIFECYCLE_STYLES,
} = require('../../.lmas-core/core/graph-dashboard/formatters/html-formatter');

const MOCK_GRAPH_DATA = {
  nodes: [
    { id: 'dev', label: 'dev', group: 'agents', path: '.lmas-core/agents/dev.md', lifecycle: 'production' },
    { id: 'task-a', label: 'task-a', group: 'tasks', path: '.lmas-core/tasks/task-a.md', lifecycle: 'production' },
    { id: 'tmpl-story', label: 'story-tmpl', group: 'templates', path: '.lmas-core/templates/story-tmpl.yaml', lifecycle: 'experimental' },
    { id: 'script-1', label: 'build.js', group: 'scripts', path: '.lmas-core/scripts/build.js', lifecycle: 'orphan' },
  ],
  edges: [
    { from: 'dev', to: 'task-a' },
...
describe('html-formatter', () => {
  ...
  describe('_buildVisNodes', () => {
    ...
    it('should include path property on nodes', () => {
      const nodes = _buildVisNodes(MOCK_GRAPH_DATA.nodes);
      const devNode = nodes.find((n) => n.id === 'dev');
      expect(devNode.path).toBe('.lmas-core/agents/dev.md');
    });
  ...
  describe('CLI integration (FORMAT_MAP)', () => {
    it('should have html in FORMAT_MAP', () => {
      const { FORMAT_MAP } = require('../../.lmas-core/core/graph-dashboard/cli');
      expect(FORMAT_MAP.html).toBe(formatAsHtml);
    });

    it('should have html in VALID_FORMATS', () => {
      const { VALID_FORMATS } = require('../../.lmas-core/core/graph-dashboard/cli');
      expect(VALID_FORMATS).toContain('html');
    });

    it('should have html in WATCH_FORMAT_MAP', () => {
      const { WATCH_FORMAT_MAP } = require('../../.lmas-core/core/graph-dashboard/cli');
    ...
    it('should parse --format=html correctly', () => {
      const { parseArgs } = require('../../.lmas-core/core/graph-dashboard/cli');
