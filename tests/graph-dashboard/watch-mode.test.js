...
const { parseArgs, handleWatch, WATCH_FORMAT_MAP } = require('../../.lmas-core/core/graph-dashboard/cli');

describe('watch-mode', () => {
  let consoleLogSpy;
  let consoleErrorSpy;
  let watchState;
  const testOutputDir = path.resolve(process.cwd(), '.lmas');
  ...
  describe('handleWatch lifecycle', () => {
    ...
    it('should create .lmas directory if it does not exist', async () => {
      // .lmas should be created by handleWatch
      watchState = await handleWatch({ format: 'dot', interval: 5 });
