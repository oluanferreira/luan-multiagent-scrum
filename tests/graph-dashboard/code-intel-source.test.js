...
jest.mock('../../.lmas-core/core/ids/registry-loader', () => ({
  RegistryLoader: jest.fn().mockImplementation(() => ({
    load: () => mockRegistryData,
  })),
}));

const { CodeIntelSource, _classifyScript, _detectCategory } = require('../../.lmas-core/core/graph-dashboard/data-sources/code-intel-source');
...
describe('CodeIntelSource', () => {
  ...
  describe('getData - registry fallback error', () => {
    it('should return empty graph when RegistryLoader throws', async () => {
      ...
      const { RegistryLoader } = require('../../.lmas-core/core/ids/registry-loader');
  ...
  describe('_classifyScript', () => {
    it('should classify development scripts as scripts/task', () => {
      expect(_classifyScript('.lmas-core/development/scripts/build.js')).toBe('scripts/task');
    });

    it('should classify core scripts as scripts/engine', () => {
      expect(_classifyScript('.lmas-core/core/graph-dashboard/cli.js')).toBe('scripts/engine');
    });

    it('should classify infrastructure scripts as scripts/infra', () => {
      expect(_classifyScript('.lmas-core/infrastructure/ci/deploy.js')).toBe('scripts/infra');
    });
  ...
  describe('_detectCategory', () => {
    it('should detect checklists from path', () => {
      expect(_detectCategory('other', '.lmas-core/development/checklists/pre-push.md')).toBe('checklists');
    });

    it('should detect workflows from path', () => {
      expect(_detectCategory('other', '.lmas-core/development/workflows/deploy.yaml')).toBe('workflows');
    });

    it('should detect utils from path', () => {
      expect(_detectCategory('other', '.lmas-core/utils/helper.js')).toBe('utils');
    });

    it('should detect data from path', () => {
      expect(_detectCategory('other', '.lmas-core/data/entity-registry.yaml')).toBe('data');
    });

    it('should detect tools from path', () => {
      expect(_detectCategory('other', '.lmas-core/development/tools/coderabbit.md')).toBe('tools');
    });

    it('should subcategorize scripts by directory', () => {
      expect(_detectCategory('scripts', '.lmas-core/development/scripts/build.js')).toBe('scripts/task');
      expect(_detectCategory('scripts', '.lmas-core/core/code-intel/index.js')).toBe('scripts/engine');
      expect(_detectCategory('scripts', '.lmas-core/infrastructure/ci/lint.js')).toBe('scripts/infra');
    });

    it('should return base category when no path match', () => {
      expect(_detectCategory('agents', '.lmas-core/development/agents/dev.md')).toBe('agents');
      expect(_detectCategory('tasks', '.lmas-core/development/tasks/build.md')).toBe('tasks');
    });
  ...
  describe('getData - category detection in registry', () => {
    it('should apply _detectCategory to registry entities', async () => {
      mockRegistryData = {
        ...
        entities: {
          scripts: {
            'build-script': { path: '.lmas-core/development/scripts/build.js', type: 'script', dependencies: [] },
            'engine-script': { path: '.lmas-core/core/graph-dashboard/cli.js', type: 'script', dependencies: [] },
          },
          other: {
            'my-checklist': { path: '.lmas-core/development/checklists/pre-push.md', type: 'checklist', dependencies: [] },
          },
        },
      };
  ...
  describe('getData - lifecycle field in registry nodes', () => {
    it('should include lifecycle from registry entity data', async () => {
      mockRegistryData = {
        ...
        entities: {
          agents: {
            'dev-agent': { path: '.lmas-core/agents/dev.md', type: 'agent', lifecycle: 'production', dependencies: [], usedBy: [] },
            'old-agent': { path: '.lmas-core/agents/old.md', type: 'agent', lifecycle: 'deprecated', dependencies: [], usedBy: [] },
          },
          tasks: {
            'orphan-task': { path: '.lmas-core/tasks/orphan.md', type: 'task', lifecycle: 'orphan', dependencies: [], usedBy: [] },
          },
        },
      };
    ...
    it('should default lifecycle to production when missing', async () => {
      mockRegistryData = {
        ...
        entities: {
          agents: {
            'no-lifecycle': { path: '.lmas-core/agents/test.md', type: 'agent', dependencies: [], usedBy: [] },
          },
        },
      };
