...
const EXCLUDE_PATTERNS = [
  ...
  /docs\/standards\/LMAS-LIVRO-DE-OURO/,
  /docs\/standards\/LMAS-FRAMEWORK-MASTER\.md$/,
...
async function generateManifest() {
  ...
  if (!fs.existsSync(lmasCoreDir)) {
    throw new Error(`.lmas-core directory not found at ${lmasCoreDir}`);
  }
  ...
  console.log(`Generating manifest for lmas-core v${version}...`);

  const allFiles = [];

  // Scan folders
  for (const folder of FOLDERS_TO_COPY) {
    const folderPath = path.join(lmasCoreDir, folder);
    if (fs.existsSync(folderPath)) {
      scanDirectory(folderPath, lmasCoreDir, allFiles);
    }
  }

  // Add root files
  for (const file of ROOT_FILES_TO_COPY) {
    const filePath = path.join(lmasCoreDir, file);
  ...
  for (const fullPath of allFiles) {
    try {
      const relativePath = path.relative(lmasCoreDir, fullPath).replace(/\\/g, '/');
...
async function writeManifest(manifest) {
  const manifestPath = path.join(__dirname, '..', '.lmas-core', 'install-manifest.yaml');
  ...
  // Add header comment
  const header = `# LMAS-Core Install Manifest
...
async function main() {
  try {
    ...
    console.log('LMAS-Core Install Manifest Generator');
    console.log('='.repeat(60));
    console.log('');
