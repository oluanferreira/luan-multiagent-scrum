...
function shouldCheckManifest(stagedFiles) {
  ...
  return stagedFiles.some((file) => {
    if (file === '.lmas-core/install-manifest.yaml') return false;
    return file.startsWith('.lmas-core/');
  });
}
...
async function main() {
  ...
  if (!shouldCheckManifest(stagedFiles)) {
    console.log('ℹ️ manifest: no .lmas-core changes staged, skipping check');
    return;
  }
  ...
  execSync('git add .lmas-core/install-manifest.yaml', { stdio: 'inherit' });
