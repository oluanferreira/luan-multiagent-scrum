...
#
# Usage:
#   .\scripts\sign-manifest.ps1 -SecretKey C:\path\to\lmas-core.key
#
# Prerequisites:
...
$ManifestPath = Join-Path $ProjectRoot ".lmas-core\install-manifest.yaml"
$SignaturePath = "$ManifestPath.minisig"

Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host "  LMAS-Core Manifest Signing Tool (Windows)" -ForegroundColor Yellow
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Yellow
Write-Host ""
...
if (-not (Test-Path $ManifestPath)) {
    ...
    Write-Host "  node bin\lmas.js manifest:generate"
    exit 1
}
...
Push-Location (Join-Path $ProjectRoot ".lmas-core")
try {
...
Write-Host "  1. git add .lmas-core\install-manifest.yaml.minisig"
