...
/**
 * Dependency Graph Validator — .lmas-core/package.json completeness
 * Story INS-4.12 (AC1, AC4, AC6)
 *
 * Scans all .js files in .lmas-core/development/scripts/ for require() calls,
 * then verifies each non-builtin, non-relative package is declared in
 * .lmas-core/package.json dependencies.
 *
 * Exit codes: 0 = PASS, 1 = FAIL (missing deps found)
 * Usage: node scripts/validate-lmas-core-deps.js
 */
...
const LMAS_CORE_DIR = path.join(PROJECT_ROOT, '.lmas-core');
const SCRIPTS_DIR = path.join(LMAS_CORE_DIR, 'development', 'scripts');
const PACKAGE_JSON = path.join(LMAS_CORE_DIR, 'package.json');
...
function main() {
  console.log('--- .lmas-core Dependency Validation (INS-4.12) ---\n');

  // Read .lmas-core/package.json
  if (!fs.existsSync(PACKAGE_JSON)) {
    console.error('FAIL: .lmas-core/package.json not found');
    process.exit(1);
  }
  ...
  if (scripts.length === 0) {
    console.log('WARN: No .js files found in .lmas-core/development/scripts/');
    process.exit(0);
  }

  console.log(`Scanning ${scripts.length} scripts in .lmas-core/development/scripts/\n`);
  ...
  console.log(`Declared in .lmas-core/package.json: ${declared.size}`);
  ...
  if (errors.length > 0) {
    ...
    console.error('\nFix: Add missing packages to .lmas-core/package.json dependencies');
    console.error('Or add to ALLOWLIST if they are optional dev-time tools (wrap in try-catch)\n');
    process.exit(1);
  }

  console.log('PASS: All script dependencies are declared in .lmas-core/package.json\n');
  process.exit(0);
}
