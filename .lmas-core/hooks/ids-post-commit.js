 ...
 *
 * Usage: node .lmas-core/hooks/ids-post-commit.js
 */
...
async function main() {
  ...
  if (process.env.LMAS_IDS_FORCE !== '1' && isDocsOnlyCommit(changes)) {
    console.log('[IDS-Hook] Docs-only commit detected, skipping registry update.');
    return;
  }

  try {
    const { RegistryUpdater } = require(path.resolve(REPO_ROOT, '.lmas-core/core/ids/registry-updater.js'));
