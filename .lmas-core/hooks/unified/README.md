...
**Module:** `.lmas-core/hooks/unified`
...
Hook Runners (runners/*.js)
         ↓
Application Logic (lmas-core, lmas-pro)
```
...
- **`precompact-runner.js`** (Story MIS-3)
  ...
  - Open Core architecture (delegates to lmas-pro)
...
pro/memory/session-digest/extractor.js
         ↓
.lmas/session-digests/{session-id}-{timestamp}.yaml
```
...
- If lmas-pro not available: no-op (log and return)
...
```javascript
// .lmas-core/hooks/unified/runners/my-runner.js
...
async function main() {
  ...
  const { onMyEvent } = require('../../.lmas-core/hooks/unified/runners/my-runner');
...
if (!isProAvailable()) {
  console.log('[Hook] lmas-pro not available, skipping');
  return;
}
...
- **Story PRO-5:** lmas-pro Repository Bootstrap (pro-detector pattern)

---

*Unified Hooks System - LMAS Core*
