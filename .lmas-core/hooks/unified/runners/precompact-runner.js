 ...
 * - lmas-core: Hook runner (this file) + pro detection
 * - lmas-pro: Digest extraction logic (memory/session-digest/)
 *
 * @module .lmas-core/hooks/unified/runners/precompact-runner
 * @see Story MIS-3 - Session Digest (PreCompact Hook)
 * @see Story PRO-5 - lmas-pro Repository Bootstrap
 */
 ...
 *
 * Execution flow:
 * 1. Detect lmas-pro availability via pro-detector
...
async function onPreCompact(context) {
  try {
    // 1. Detect lmas-pro availability
    const proAvailable = isProAvailable();

    if (!proAvailable) {
      console.log('[PreCompact] lmas-pro not available, skipping session digest');
    ...
    setImmediate(async () => {
      try {
        ...
        if (!digestModule || typeof digestModule.extractSessionDigest !== 'function') {
          console.error('[PreCompact] Digest extractor not found or invalid in lmas-pro');
          return;
        }
