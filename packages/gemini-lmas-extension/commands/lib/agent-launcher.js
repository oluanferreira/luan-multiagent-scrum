...
const AGENT_INFO = {
  'lmas-master': { icon: '🧠', role: 'Master Orchestrator' },
...
function listAvailableAgents(projectRoot = process.cwd()) {
  const sourceDir = path.join(projectRoot, '.lmas-core', 'development', 'agents');
...
function commandNameForAgent(agentId) {
  if (agentId.startsWith('lmas-')) {
    return `/lmas-${agentId.replace(/^lmas-/, '')}`;
  }
  return `/lmas-${agentId}`;
}

function hasAgent(projectRoot, agentId) {
  const canonical = path.join(projectRoot, '.lmas-core', 'development', 'agents', `${agentId}.md`);
  const gemini = path.join(projectRoot, '.gemini', 'rules', 'LMAS', 'agents', `${agentId}.md`);
  return fs.existsSync(canonical) || fs.existsSync(gemini);
}

function renderGreeting(projectRoot, agentId) {
  const scriptPath = path.join(projectRoot, '.lmas-core', 'development', 'scripts', 'generate-greeting.js');
...
function buildActivationPrompt(agentId) {
  return [
    `Ative o agente ${agentId} usando .lmas-core/development/agents/${agentId}.md`,
    `(fallback: .gemini/rules/LMAS/agents/${agentId}.md),`,
    `renderize o greeting via node .lmas-core/development/scripts/generate-greeting.js ${agentId}`,
...
function runAgentLauncher(agentId, projectRoot = process.cwd()) {
  if (!agentId) {
    console.log('Uso: /lmas-agent <agent-id>');
    return 1;
  }
  ...
  console.log(`${info.icon} LMAS Agent Selected: ${agentId}`);
...
function runAgentMenu(projectRoot = process.cwd()) {
  ...
  console.log('🤖 LMAS Quick Agent Menu (Gemini)');
  console.log('');

  if (agents.length === 0) {
    console.log('No LMAS agents found. Run: npm run sync:ide:gemini');
    return 1;
  }
  ...
  console.log('\nTip: run /lmas-<agent-id> to prepare activation prompt quickly.');
  return 0;
}
