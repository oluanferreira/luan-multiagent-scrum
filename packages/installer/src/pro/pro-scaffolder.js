 ...
 * node_modules/@lmas-fullstack/pro/ into the user's project after
 * license activation.
 *
...
const SCAFFOLD_ITEMS = [
  ...
  },
  {
    ...
    dest: path.join('.lmas-core', 'pro-config.yaml'),
  ...
  },
  {
    ...
    dest: path.join('.lmas-core', 'feature-registry.yaml'),
 ...
 * @param {string} proSourceDir - Path to pro package content (node_modules/@lmas-fullstack/pro)
...
async function scaffoldProContent(targetDir, proSourceDir, options = {}) {
  ...
  if (!await fs.pathExists(proSourceDir)) {
    result.errors.push(
      `Pro package not found at ${proSourceDir}. Run "npm install @lmas-fullstack/pro" first.`
    );
    return result;
  }
...
async function mergeProConfig(targetDir) {
  ...
  const proConfigPath = path.join(targetDir, '.lmas-core', 'pro-config.yaml');
