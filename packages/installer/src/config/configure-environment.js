...
async function configureEnvironment(options = {}) {
  ...
  try {
    ...
    // Step 2: API keys are configured later via .env or lmas-master
    // Skipping prompts during installation for better UX
    const apiKeys = {};

    if (!skipPrompts) {
      console.log('\n💡 API keys can be configured later in .env file or via lmas-master');
    }
    ...
    const coreConfigDir = path.join(targetDir, '.lmas-core');
    ...
    console.log('✅ Created .lmas-core/core-config.yaml');
...
/**
 * Update .gitignore to include critical entries for LMAS projects
 *
...
async function updateGitignore(targetDir) {
  ...
  const criticalEntries = {
    ...
    'LMAS Local': ['.lmas-core/local/', '.claude/settings.local.json', '.lmas/install-log.txt'],
  };
