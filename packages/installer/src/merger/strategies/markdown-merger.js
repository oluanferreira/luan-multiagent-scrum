/**
 * @fileoverview Merge strategy for Markdown files with LMAS-MANAGED sections
...
const { parseMarkdownSections, hasLmasMarkers } = require('../parsers/markdown-section-parser.js');

/**
 * Merge strategy for Markdown files (CLAUDE.md, rules.md).
 * Uses LMAS-MANAGED markers to identify sections that can be updated.
...
class MarkdownMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, options = {}) {
    ...
    // If existing file has no LMAS markers, do legacy migration
    if (!existing.hasLmasMarkers) {
      return this.migrateLegacy(existingContent, template, options);
    }

    const stats = createEmptyStats();
    const changes = [];
    const processedLmasSections = new Set();
    ...
    for (const section of existing.sections) {
      if (section.managed) {
        // LMAS-managed section - update with template content if available
        ...
        if (templateSection) {
          ...
          merged += `<!-- LMAS-MANAGED-START: ${section.id} -->\n`;
          merged += templateSection.lines.join('\n');
          if (!merged.endsWith('\n')) {
            merged += '\n';
          }
          merged += `<!-- LMAS-MANAGED-END: ${section.id} -->\n\n`;
          processedLmasSections.add(section.id);
          changes.push({
            type: 'updated',
    ...
    // Add new LMAS sections that don't exist in current file
    const newSections = template.sections.filter(
      s => s.managed && !processedLmasSections.has(s.id),
    );

    if (newSections.length > 0) {
      merged += '---\n\n';
      merged += '<!-- New LMAS sections added -->\n\n';
  ...
  /**
   * Migrate a legacy file (without LMAS markers) by appending LMAS sections
  ...
  async migrateLegacy(existingContent, template, _options = {}) {
    ...
    // Get all LMAS-managed sections from template
    const lmasSections = template.sections.filter(s => s.managed);
    ...
    // Add separator and LMAS sections
    if (lmasSections.length > 0) {
      merged += '\n---\n\n';
      merged += '<!-- LMAS-MANAGED SECTIONS -->\n';
      merged += '<!-- These sections are managed by LMAS. Edit content between markers carefully. -->\n';
      merged += '<!-- Your custom content above will be preserved during updates. -->\n\n';

      for (const section of lmasSections) {
        merged += `<!-- LMAS-MANAGED-START: ${section.id} -->\n`;
        merged += section.lines.join('\n');
        if (!merged.endsWith('\n')) {
          merged += '\n';
   ...
   */
  getDescription() {
    return 'Merges Markdown files using LMAS-MANAGED section markers';
  }
}
