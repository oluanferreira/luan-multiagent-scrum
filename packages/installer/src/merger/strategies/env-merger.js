...
class EnvMerger extends BaseMerger {
  ...
  async merge(existingContent, newContent, _options = {}) {
    ...
    // Add LMAS section if there are new vars or suggestions
    if (newVars.length > 0 || suggestions.length > 0) {
      const date = new Date().toISOString().split('T')[0];
      merged += `\n# === LMAS Variables (added ${date}) ===\n`;

      // Add new variables
      for (const { key, value, comment } of newVars) {
        if (comment && !comment.includes('LMAS')) {
          // Add original comment if it's not an LMAS header
          merged += `${comment}\n`;
        }
        merged += `${key}=${value}\n`;
      }
      ...
      if (suggestions.length > 0) {
        merged += '\n# LMAS_NOTE: The following variables already exist with different values:\n';
        for (const { key, suggestedValue } of suggestions) {
          merged += `# LMAS_SUGGESTED: ${key}=${suggestedValue}\n`;
        }
      }
    }
