 ...
 * @param {string} options.projectType - 'BROWNFIELD' | 'GREENFIELD' | 'EXISTING_LMAS'
...
async function promptFileExists(filePath, options = {}) {
  ...
  const isBrownfield = projectType === 'BROWNFIELD' || projectType === 'EXISTING_LMAS';
...
function generateTemplateVariables(wizardState) {
  ...
  return {
    ...
    timestamp,
    lmasVersion: '2.1.0', // From package.json in real implementation
  };
}

/**
 * Copy agent files from .lmas-core/development/agents to IDE-specific agent folder
...
async function copyAgentFiles(projectRoot, agentFolder, ideConfig = null) {
  ...
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.lmas-core', 'development', 'agents');
  ...
  for (const file of agentFiles) {
    ...
    if (stat.isFile()) {
      if (isAntiGravity) {
        ...
        try {
          const agentParser = require('../../../../.lmas-core/infrastructure/scripts/ide-sync/agent-parser');
          const copilotTransformer = require('../../../../.lmas-core/infrastructure/scripts/ide-sync/transformers/github-copilot');
...
async function createAntiGravityConfigJson(projectRoot, ideConfig) {
  ...
  const config = {
    ...
    paths: {
      ...
      tasks: '.lmas-core/tasks',
      workflows: '.lmas-core/workflows',
    },
  };
  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // Load template from .lmas-core/product/templates/
        const templatePath = path.join(__dirname, '..', '..', '..', '..', '.lmas-core', 'product', 'templates', ide.template);
        ...
        if (ideKey === 'gemini') {
          ...
          if (settingsFile) {
            createdFiles.push(settingsFile);
            spinner.succeed('Created .gemini/settings.json with LMAS hooks');
          } else {
            spinner.info('Skipped .gemini/settings.json (no hooks to register)');
          }

          spinner.start('Linking Gemini LMAS extension...');
          const extensionResult = await linkGeminiExtension(projectRoot);
          if (extensionResult.status === 'linked') {
            spinner.succeed('Gemini extension "lmas" linked and enabled');
          } else if (extensionResult.status === 'already-linked') {
            spinner.succeed('Gemini extension "lmas" already linked');
          } else {
...
function showSuccessSummary(result) {
  ...
  console.log('  2. The IDE should automatically recognize LMAS configuration');
...
/**
 * Copy .lmas-core/hooks/gemini folder into .gemini/hooks during installation
 * @param {string} projectRoot - Project root directory
 * @returns {Promise<string[]>} List of copied files
 */
async function copyGeminiHooksFolder(projectRoot) {
  const sourceDir = path.join(__dirname, '..', '..', '..', '..', '.lmas-core', 'hooks', 'gemini');
...
/**
 * Create/merge .gemini/settings.json and register LMAS hooks as enabled.
 * @param {string} projectRoot - Project root directory
...
/**
 * Best-effort Gemini extension linking for LMAS project.
...
async function linkGeminiExtension(projectRoot) {
  const extensionDir = path.join(projectRoot, 'packages', 'gemini-lmas-extension');
  ...
  if (output.includes('already installed')) {
    const uninstall = spawnSync('gemini', ['extensions', 'uninstall', 'lmas'], {
      cwd: projectRoot,
      encoding: 'utf8',
      timeout: 30000,
    });
...
async function copyExtraCommandFiles(projectRoot, _sourceRoot) {
  ...
  const DISTRIBUTABLE_ENTRIES = new Set([
    'LMAS',       // Core agent/script commands (agents/ sub-dir excluded below)
    'synapse',    // SYNAPSE context engine commands
    'greet.md',   // Greeting skill
  ]);

  // Within LMAS/, these sub-dirs are excluded (private or handled separately)
  const LMAS_EXCLUDED = new Set([
    'LMAS/agents',   // Already handled by copyAgentFiles()
    'LMAS/stories',  // Project-specific story skills, not distributable
  ]);
  ...
  async function copyRecursive(src, dest, relativePath) {
    ...
    for (const entry of entries) {
      ...
      // Within LMAS/, skip excluded sub-directories
      if (LMAS_EXCLUDED.has(entryRelative) || [...LMAS_EXCLUDED].some(ex => entryRelative.startsWith(ex + '/'))) {
        continue;
      }
