/**
 * LMAS Core Installer Module
 *
 * Story 1.4/1.6: IDE Selection & Environment Configuration
 * Handles copying .lmas-core content (agents, tasks, workflows, templates, etc.)
 * to the target project directory.
 *
 * @module installer/lmas-core-installer
 */
...
/**
 * Get the path to the source .lmas-core directory in the package
 * @returns {string} Absolute path to .lmas-core source
 */
function getLmasCoreSourcePath() {
  // Navigate from packages/installer/src/installer/ to project root/.lmas-core
  return path.join(__dirname, '..', '..', '..', '..', '.lmas-core');
}
...
/**
 * Root files to copy from .lmas-core
 * @constant {string[]}
 */
const ROOT_FILES_TO_COPY = [
  ...
  'constitution.md',    // LMAS fundamental principles
  'user-guide.md',
 ...
 * @param {string} rootPath - Replacement path (e.g., '.lmas-core')
 * @returns {string} Content with {root} replaced
 */
function replaceRootPlaceholder(content, rootPath = '.lmas-core') {
 ...
 *
 * @param {string} targetLmasCore - Path to .lmas-core directory
 * @param {string[]} installedFiles - List of installed files (relative to .lmas-core)
 * @returns {Promise<Object>} Object mapping file paths to their sha256 hashes
 */
async function generateFileHashes(targetLmasCore, installedFiles) {
  const fileHashes = {};

  for (const filePath of installedFiles) {
    const absolutePath = path.join(targetLmasCore, filePath);
 ...
 * @param {string} options.targetLmasCore - Path to .lmas-core directory
...
async function generateVersionJson(options) {
  ...
  const fileHashes = await generateFileHashes(targetLmasCore, installedFiles);
  ...
  const versionJsonPath = path.join(targetLmasCore, 'version.json');
...
async function copyFileWithRootReplacement(sourcePath, destPath, replaceRoot = true) {
  try {
    ...
    if (needsReplacement) {
      ...
      const updatedContent = replaceRootPlaceholder(content, '.lmas-core');
...
/**
 * Install .lmas-core content to target directory
 *
 ...
 *
 * @example
 * const result = await installLmasCore({ targetDir: '/path/to/project' });
 * console.log(result.installedFiles); // List of installed files
 */
async function installLmasCore(options = {}) {
  const {
  ...
  const spinner = ora('Installing LMAS core framework...').start();

  try {
    const sourceDir = getLmasCoreSourcePath();
    const targetLmasCore = path.join(targetDir, '.lmas-core');

    // Check if source exists
    if (!await fs.pathExists(sourceDir)) {
      throw new Error('.lmas-core source directory not found in package');
    }

    // Create target .lmas-core directory
    await fs.ensureDir(targetLmasCore);

    // Copy each folder
    for (const folder of FOLDERS_TO_COPY) {
      const folderSource = path.join(sourceDir, folder);
      const folderDest = path.join(targetLmasCore, folder);
    ...
    for (const file of ROOT_FILES_TO_COPY) {
      ...
      const fileDest = path.join(targetLmasCore, file);
    ...
    await fs.writeFile(
      path.join(targetLmasCore, 'install-manifest.yaml'),
    ...
    const lmasCorePackageJson = path.join(targetLmasCore, 'package.json');
    result.lmasCoreDepsInstalled = false;
    if (await fs.pathExists(lmasCorePackageJson)) {
      spinner.text = 'Installing .lmas-core dependencies (js-yaml, fast-glob, etc.)...';
      try {
        ...
        result.lmasCoreDepsInstalled = true;
        spinner.succeed('Installed .lmas-core dependencies');
        spinner.start('Finishing installation...');
      } catch (depError) {
        spinner.warn(`Could not install .lmas-core dependencies: ${depError.message}`);
    ...
    result.success = true;
    spinner.succeed(`LMAS core installed (${result.installedFiles.length} files)`);

  } catch (error) {
    spinner.fail('LMAS core installation failed');
...
/**
 * Create a basic package.json for LMAS projects
 * @param {Object} options - Options
 * @param {string} [options.targetDir=process.cwd()] - Target directory
...
async function createBasicPackageJson(options = {}) {
  ...
  const packageJson = {
    ...
    version: '0.1.0',
    description: `LMAS-powered ${projectType} project`,
    private: true,
    scripts: {
