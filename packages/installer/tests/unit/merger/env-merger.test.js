...
describe('EnvMerger', () => {
  ...
  describe('merge', () => {
    ...
    it('should preserve existing values over new values', async () => {
      ...
      // The suggested value appears in LMAS_SUGGESTED comment
      expect(result.content).toContain('LMAS_SUGGESTED: API_KEY=placeholder_key');
      expect(result.stats.preserved).toBe(1);
    });

    it('should add LMAS_SUGGESTED comment for differing values', async () => {
      ...
      expect(result.content).toContain('# LMAS_SUGGESTED: PORT=8080');
...
DB_PORT=5432`;
    ...
    it('should add new variables in LMAS Variables section', async () => {
      const existing = 'EXISTING=value';
      const newContent = `LMAS_VAR1=value1
LMAS_VAR2=value2`;

      const result = await merger.merge(existing, newContent);

      // Header includes date: "# === LMAS Variables (added YYYY-MM-DD) ==="
      expect(result.content).toMatch(/# === LMAS Variables \(added \d{4}-\d{2}-\d{2}\) ===/);
      expect(result.content).toContain('LMAS_VAR1=value1');
      expect(result.content).toContain('LMAS_VAR2=value2');
    });
