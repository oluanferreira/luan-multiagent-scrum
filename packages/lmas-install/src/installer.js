/**
 * LMAS Installer - Main installation logic
 *
 * Orchestrates the complete LMAS installation flow:
 * 1. OS detection
...
/**
 * Check if a config-resolver is available (lmas-core installed)
 * @param {string} projectRoot - Project root
 * @returns {Object|null} Config resolver module or null
 */
function tryLoadConfigResolver(projectRoot) {
  const possiblePaths = [
    path.join(projectRoot, '.lmas-core/core/config/config-resolver.js'),
    path.join(projectRoot, 'node_modules/lmas-core/.lmas-core/core/config/config-resolver.js'),
  ];
...
async function createUserConfigDirect(profile, logger, dryRun) {
  const userConfigDir = path.join(os.homedir(), '.lmas');
...
/**
 * Check if this is a brownfield installation (existing LMAS)
 * @param {string} projectRoot - Project root
...
function detectBrownfield(projectRoot) {
  ...
  const legacyConfigPath = path.join(projectRoot, '.lmas-core/core-config.yaml');
  ...
  const frameworkConfigPath = path.join(projectRoot, '.lmas-core/framework-config.yaml');
  if (fs.existsSync(frameworkConfigPath)) {
    result.isBrownfield = true;
    result.hasLayeredConfig = true;
  }

  // Check for .lmas-core directory
  const lmasCoreDir = path.join(projectRoot, '.lmas-core');
...
async function runDoctor(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx lmas-core doctor');
    return;
  }

  const spinner = ora('Running LMAS doctor...').start();

  try {
    const { stdout } = await execa('npx', ['lmas-core', 'doctor'], {
      cwd: projectRoot,
      timeout: 60000,
    });

    spinner.succeed('LMAS doctor completed');

    if (logger.verbose) {
      console.log(chalk.dim(stdout));
    }
  } catch (error) {
    spinner.warn('LMAS doctor had warnings');
    if (logger.verbose) {
...
/**
 * Install lmas-core package
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function installLmasCore(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npm install lmas-core --save-dev');
    return;
  }

  const spinner = ora('Installing lmas-core...').start();

  try {
    await execa('npm', ['install', 'lmas-core', '--save-dev'], {
      cwd: projectRoot,
      timeout: 300000, // 5 minutes
    });

    spinner.succeed('lmas-core installed');
  } catch (error) {
    spinner.fail('Failed to install lmas-core');
    throw error;
  }
}

/**
 * Initialize LMAS in the project
 * @param {string} projectRoot - Project root
 * @param {InstallLogger} logger - Logger instance
 * @param {boolean} dryRun - Whether this is a dry run
 */
async function initializeLmas(projectRoot, logger, dryRun) {
  if (dryRun) {
    logger.action('Run: npx lmas-core install');
    return;
  }

  const spinner = ora('Initializing LMAS...').start();

  try {
    await execa('npx', ['lmas-core', 'install'], {
      cwd: projectRoot,
      timeout: 120000, // 2 minutes
    });

    spinner.succeed('LMAS initialized');
  } catch (error) {
    spinner.fail('Failed to initialize LMAS');
    throw error;
  }
}
...
async function runInstaller(options = {}) {
  ...
  // Introduction
  intro(chalk.bgCyan(' LMAS Installer '));
  ...
  if (!profile) {
    ...
    const profileSelection = await select({
      message: 'Select your LMAS profile:',
      options: [
        {
          value: 'bob',
  ...
  if (brownfield.isBrownfield) {
    logger.info('Existing LMAS installation detected');
    ...
    if (brownfield.hasLegacyConfig && !brownfield.hasLayeredConfig) {
      ...
      if (shouldMigrate && !options.dryRun) {
        ...
        try {
          await execa('npx', ['lmas-core', 'config', 'migrate'], {
            cwd: projectRoot,
            timeout: 60000,
          });
        ...
        logger.action('Run: npx lmas-core config migrate');
      }
    } else {
    ...
    // Install lmas-core
    await installLmasCore(projectRoot, logger, options.dryRun);

    // Initialize LMAS
    await initializeLmas(projectRoot, logger, options.dryRun);
  }
  ...
  if (options.dryRun) {
    ...
    outro(chalk.green(`LMAS installed successfully in ${elapsed}!`));
    console.log('');
    console.log(chalk.dim('Next steps:'));
    console.log(chalk.dim('  1. Run `npx lmas-core info` to see your configuration'));
