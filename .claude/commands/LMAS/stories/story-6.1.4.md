...
- STEP 3: |
    ...
    1. Execute: node .lmas-core/scripts/generate-greeting.js {agent-id}
...
Add to `.lmas-core/scripts/agent-config-loader.js`:
...
1. Add deprecation warning to `.lmas-core/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .lmas-core/
grep -r "from.*config-loader" .lmas-core/
```
...
1. `.lmas-core/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.lmas-core/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.lmas-core/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.lmas-core/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .lmas-core/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .lmas-core/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .lmas-core/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .lmas-core/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .lmas-core/scripts/batch-integrate-greeting-builder.js ] && rm .lmas-core/scripts/batch-integrate-greeting-builder.js
   [ -f .lmas-core/scripts/update-activation-instructions.js ] && rm .lmas-core/scripts/update-activation-instructions.js
   ```
...
Modify `.lmas-core/scripts/greeting-builder.js`:
...
Create `.lmas-core/scripts/generate-greeting.js`:
...
**Pilot Agent:**
- `qa.md` (Oracle - Test Architect & Quality Advisor)
...
- STEP 3: |
    ...
    1. Execute: node .lmas-core/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
2. **Existing Session Test:**
   ...
   node -e "const fs = require('fs'); const path = require('path'); const sessionPath = path.join(process.cwd(), '.lmas', 'session-state.json'); fs.mkdirSync(path.dirname(sessionPath), { recursive: true }); fs.writeFileSync(sessionPath, JSON.stringify({ sessionId: 'test-123', startTime: new Date().toISOString(), lastActivity: new Date().toISOString(), agentSequence: [{ id: 'po', name: 'Keymaker' }], lastCommands: ['create-story'], workflowActive: null }), 'utf8');"
...
3. **Direct Script Test:**
   ...
   node .lmas-core/scripts/generate-greeting.js qa
   
   # Expected: Greeting output without errors
   # Expected: Contains agent name "Oracle"
...
4. **Error Handling Test:**
   ...
   node .lmas-core/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .lmas-core/scripts/generate-greeting.js qa
...
Create script `.lmas/temp-update-remaining-agents.js`:

```javascript
const fs = require('fs');
const path = require('path');

const AGENTS_DIR = path.join(process.cwd(), '.lmas-core', 'agents');
const REMAINING_AGENTS = [
  'dev', 'po', 'sm', 'pm', 'architect', 'analyst',
  'data-engineer', 'devops', 'lmas-master', 'ux-design-expert'
];
...
for agent in dev po sm pm architect analyst data-engineer devops lmas-master ux-design-expert; do
  echo "Testing $agent..."
  node .lmas-core/scripts/generate-greeting.js $agent
  echo "---"
done
```
...
Create `.lmas-core/scripts/command-execution-hook.js`:
 ...
 * 
 * @example
 * updateSessionAfterCommand('qa', 'Oracle', 'analyze-framework');
 */
...
Create `.lmas-core/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.lmas-core/scripts/generate-greeting');

describe('generateGreeting', () => {
  it('should generate greeting for valid agent', async () => {
    const greeting = await generateGreeting('qa');
    expect(greeting).toContain('Oracle');
...
- `.lmas-core/scripts/generate-greeting.js` (Unified greeting generator)
- `.lmas-core/scripts/command-execution-hook.js` (Session update hook)
- `.lmas-core/docs/session-update-pattern.md` (Documentation)
- `tests/unit/generate-greeting.test.js` (Unit tests)
- `tests/integration/greeting-system-integration.test.js` (Integration tests)

### Files Modified

- `.lmas-core/scripts/agent-config-loader.js` (Expanded with `loadAgentDefinition()` and `loadComplete()`)
- `.lmas-core/scripts/greeting-builder.js` (Modified to accept pre-loaded context)
- `.lmas-core/scripts/config-loader.js` (Deprecated warning added)
- `.lmas-core/agents/qa.md` (Pilot - STEP 3 updated first)
- `.lmas-core/agents/*.md` (Remaining 10 agents - STEP 3 updated after QA validation)

### Files Deleted

- `.lmas-core/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.lmas-core/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.lmas-core/scripts/update-activation-instructions.js` (Temporary migration script)
- `.lmas-core/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.lmas-core/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.lmas-core/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.lmas-core/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.lmas-core/scripts/context-detector.js` (Used by greeting-builder.js)
- `.lmas-core/scripts/workflow-navigator.js` (Used by greeting-builder.js)
...
**Output:**
```
✅ Oracle (Guardian) ready. Let's ensure quality!
...
|------|---------|---------|--------|
| 2025-01-17 | 4.1 | Added CodeRabbit Integration section, clarified migration scripts deletion process, verified test directory structure | Keymaker (po) |
| 2025-01-17 | 4.0 | Expanded to unified system integration, integrated all analyses, added script consolidation, added session updates | Keymaker (po) + Aria (architect) |
| 2025-01-16 | 3.0 | Validation improvements, integration tests added | Keymaker (po) |
| 2025-01-15 | 2.0 | Major rewrite to integrate with Story 6.1.2.5 | Oracle (qa) |
