...
To help users contribute to the LMAS open-source project (`lmas-core`) by automating the PR creation process, ensuring contributions follow project standards, pass quality checks, and have proper formatting before submission.

**Target Repository**: `lmas-core` (open-source framework)
...
- **contribution_path**: `string`
  ...
  - **Example**: `"Squads/my-new-pack/"` or `"lmas-core/agents/improved-agent.md"`
...
2. **Validate Repository State**
   - Check if `lmas-core` repository is set as upstream
...
3. **Detect Contribution Type** (if not provided)
   - Scan modified files:
     - `Squads/*` → "Squad"
     - `lmas-core/agents/*` → "agent"
     - `lmas-core/tasks/*` → "task"
     - `lmas-core/tools/*` → "tool"
...
14. **Generate PR Title & Description**
    ...
    - **Description**: Use PR template:
      ...
      **First-time contributor?** Welcome! 🎉 This PR was created using LMAS PR Automation.
      ```
...
- [ ] Fork of lmas-core exists
  - **Validation**: `gh repo view {user}/lmas-core` succeeds
...
```yaml
task: prAutomation()
responsável: Operator (Automator)
...
```bash
lmas activate Otto  # github-devops agent
lmas pr create \
  --type="Squad" \
...
lmas pr create \
  --type="agent" \
  --path="lmas-core/agents/improved-po.md" \
...
lmas pr create \
  --type="bug-fix" \
  --path="lmas-core/tasks/create-next-story.md" \
