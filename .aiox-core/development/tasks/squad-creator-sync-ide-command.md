...
O sistema usa `.lmas-sync.yaml` na raiz do projeto para configuração:
...
| `Squad alias not found` | Squad não está em squad_aliases | Adicione ao .lmas-sync.yaml |
| `File exists`          | Destino já existe               | Use --force ou escolha ação |
| `IDE not active`       | IDE não está em active_ides     | Ative no .lmas-sync.yaml    |
...
const syncConfig = loadYaml('.lmas-sync.yaml');
