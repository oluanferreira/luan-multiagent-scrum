...
- **Tool:** task-runner
  ...
  - **Source:** .lmas-core/core/task-runner.js

- **Tool:** logger
  - **Purpose:** Execution logging and error tracking
  - **Source:** .lmas-core/utils/logger.js
...
*propose-modification lmas-core/agents/weather-agent.md enhance --title "Add caching support" --description "Implement response caching to reduce API calls" --priority medium

# Propose critical refactoring with impact analysis
*propose-modification lmas-core/scripts/core-utility.js refactor --title "Optimize performance" --priority critical --impact-analysis --assignees "alice,bob"

# Create draft proposal for workflow deprecation
*propose-modification lmas-core/workflows/legacy-workflow.yaml deprecate --draft --title "Deprecate legacy workflow" --link-issues "123,456"
```
...
class ProposeModificationTask {
  ...
  async execute(params) {
    try {
      console.log(chalk.blue('📝 LMAS Modification Proposal'));
  ...
  async createProposal(component, config) {
    const proposal = {
      ...
      metadata: {
        createdBy: process.env.USER || 'lmas-developer',
  ...
  async storeProposal(proposal, result) {
    const proposalsDir = path.join(this.rootPath, '.lmas', 'proposals');
...
[Describe validation approach]`
  ...
  generateProposalUrl(proposalId) {
    ...
    return `http://lmas-framework.local/proposals/${proposalId}`;
  }
}
