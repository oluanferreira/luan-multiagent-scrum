...
**Created:** 2025-12-02
**Updated:** 2025-12-02
**Agent:** @devops (Operator)

---

## Purpose

Complete environment bootstrap for new LMAS projects. Verifies and installs all required CLIs, authenticates services, initializes Git/GitHub repository, and validates the development environment before starting the greenfield workflow.
...
- campo: environment_report
  tipo: object
  destino: File system (.lmas/environment-report.yaml)
  persistido: true
...
post-conditions:
  ...
  - [ ] Environment report generated
    ...
    blocker: false
    validação: |
      Check .lmas/environment-report.yaml exists
...
acceptance-criteria:
  ...
  - [ ] Project structure follows LMAS conventions
    tipo: acceptance-criterion
    blocker: false
    validação: |
      Check docs/, .lmas/, and package.json exist
...
- **Tool:** cli-checker
  ...
  - **Source:** .lmas-core/infrastructure/scripts/cli-checker.js

- **Tool:** github-cli
  - **Purpose:** Repository creation and authentication
  - **Source:** .lmas-core/infrastructure/tools/cli/github-cli.yaml
...
- Cache detection results in .lmas/environment-report.yaml
...
# LMAS generated files
.lmas/project-status.yaml
.lmas/environment-report.yaml
...
---
*Generated by LMAS Environment Bootstrap*
...
- Initialize Synkra LMAS project structure
...
**Action:** Create LMAS-compliant project structure
...
# Create .lmas/config.yaml
@"
# LMAS Project Configuration
version: 2.1.0
project:
  name: $PROJECT_NAME
  type: greenfield
...
"@ | Out-File -FilePath ".lmas/config.yaml" -Encoding utf8
...
@"
{
  "name": "$PROJECT_NAME",
  "version": "0.1.0",
  "description": "Created with Synkra LMAS",
  "scripts": {
...
**Action:** Ask user for their profile preference and persist to `~/.lmas/user-config.yaml`
...
```bash
# Create ~/.lmas/ with secure permissions
mkdir -p ~/.lmas
chmod 700 ~/.lmas

# Write user-config.yaml with selected profile
cat > ~/.lmas/user-config.yaml << EOF
# LMAS User Preferences (global, cross-project)
...
```javascript
const { setUserConfigValue, ensureUserConfigDir } = require('.lmas-core/core/config/config-resolver');
...
- `~/.lmas/` directory must have permissions 700
- `~/.lmas/user-config.yaml` must be valid YAML after write
...
$report | ConvertTo-Json -Depth 5 | Out-File -FilePath ".lmas/environment-report.json" -Encoding utf8

Write-Host "✅ Environment report saved to .lmas/environment-report.json"
```
...
```
╔═══════════════════════════════════════════════════════════════════════════╗
║              ✅ LMAS ENVIRONMENT BOOTSTRAP COMPLETE                        ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
║                                                                            ║
║  Full workflow: .lmas-core/development/workflows/greenfield-fullstack.yaml ║
║                                                                            ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  Quick Reference                                                           ║
╠═══════════════════════════════════════════════════════════════════════════╣
║  • View environment report: cat .lmas/environment-report.json              ║
║  • Check GitHub repo: gh repo view --web                                   ║
║  • LMAS help: @lmas-master *help                                           ║
...
— Operator, environment configured with confidence 🚀
```
...
- [LMAS Greenfield Workflow](.lmas-core/development/workflows/greenfield-fullstack.yaml)
