...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.lmas-core/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.lmas-core/development/agents/)
...
steps:
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .lmas-core/install-manifest.yaml
    when: Any file in .lmas-core/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .lmas-core/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .lmas-core/development/agents/ or ide-sync/
```
