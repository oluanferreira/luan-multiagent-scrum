# Task: Update LMAS Framework

> **Version:** 4.0.0
> **Created:** 2026-01-29
> **Updated:** 2026-01-31
> **Type:** SYNC (git-native framework synchronization)
> **Agent:** @devops (Operator) or @lmas (Morpheus)
> **Execution:** Simple bash script (~15 lines)

## Purpose

Git-native sync of LMAS framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .lmas-core/scripts/update-lmas.sh

# Review changes shown by the script, then:
git add .lmas-core && git commit -m "chore: sync LMAS framework"   # Apply changes
# OR
git checkout -- .lmas-core/                                         # Cancel changes
```
...
| Path | Reason |
|------|--------|
| `.lmas-core/squads/` | Custom copywriters, data, ralph |
| `.lmas-core/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.lmas-core/context/` | Compiled contexts |
...
```yaml
task: updateLMASFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .lmas-core/scripts/update-lmas.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .lmas-core/scripts/update-lmas.sh
  3. Review changes displayed
  4. git add .lmas-core && git commit -m "chore: sync LMAS framework"  # to apply
  5. git checkout -- .lmas-core/                                        # to cancel
...
ls -la .lmas-core/squads/  # if exists
