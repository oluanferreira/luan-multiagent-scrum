...
### Instalando LMAS-FullStack

O LMAS-FullStack oferece instalação simplificada através de um instalador interativo completo.
...
npx @lmas/fullstack install
```

**O que acontece durante a instalação**:

```
🚀 LMAS-FullStack Installation Wizard

✓ Detecting installation state...
  Current state: Clean installation

📦 Select Squads to Install:
  ◉ hybrid-ops (Luan Ferreira methodology)
  ◯ expansion-creator (Create new squads)
  ◯ lmas-infrastructure-devops (DevOps utilities)
...
Se você está contribuindo para o LMAS-FullStack em si:
...
```
your-project/
├── .lmas-core/                    # ✅ Framework core instalado
...
└── Squads/               # ✅ Se squads selecionados
    ├── hybrid-ops/                # Metodologia Luan Ferreira
    └── expansion-creator/         # Criador de squads
```

### Upgrade de Instalação Existente

Se você já tem LMAS instalado e quer fazer upgrade:

```bash
# Upgrade automático (RC.9+)
npx lmas-core@latest install --force-upgrade

# OU interativo (todos os RCs)
npx lmas-core@latest install
# → Menu aparece:
#   1. Keep current version (v1.0.0-rc.8)
#   2. Upgrade LMAS core (v1.0.0-rc.8 → v1.0.0-rc.10)
...
npm view lmas-core@latest version
# Output: 1.0.0-rc.10

# Ver opções de instalação e comandos disponíveis
npx lmas-core@latest --help
...
# Ajuda do instalador
npx lmas-core@latest install --help
```
...
O futuro comando `lmas init` permitirá escolher explicitamente entre dois modos:

#### Framework Development Mode
- Para desenvolvedores contribuindo ao LMAS-FullStack
- `.lmas-core/` é código fonte (commitado)
- Mudanças afetam o framework
- Quality gates testam integridade do framework

#### Project Development Mode
- Para desenvolvedores usando LMAS em seus projetos
- `.lmas-core/` é dependência (gitignored)
...
# Cria .lmas-installation-config.yaml
...
npx lmas-core@1.0.0-rc.10 install --force-upgrade
```
...
2. **Recursos especiais**:
   - Reconhecimento automático de comandos LMAS
...
O LMAS-FullStack impõe integridade de repositório através de um agente centralizado de DevOps.
...
O LMAS-FullStack oferece três modos de desenvolvimento para acomodar diferentes níveis de habilidade e restrições de tempo.
...
O **lmas-master** é o agente de orquestração principal:
...
O **lmas-orchestrator** gerencia fluxos de trabalho:
...
O **lmas-developer** é o meta-agente para o próprio LMAS:
...
[lmas-developer guiará você na criação de um novo agente]
```
...
- **lmas-infrastructure-devops** - Utilities de DevOps e infraestrutura
...
Use o **lmas-developer** para criar agentes customizados:
...
Adicione templates próprios em `lmas-core/templates/`:
...
- [ ] IDE configurado com regras LMAS
...
- [Arquitetura do LMAS](../docs/architecture.md)
