...
The Core module contains the foundational runtime components that all other LMAS modules depend on. It provides configuration management, session handling, elicitation workflows, and essential utilities.
...
```javascript
// CommonJS
const core = require('./.lmas-core/core');

// ES Modules
import { ConfigCache, ElicitationEngine } from './.lmas-core/core/index.esm.js';
```
...
│   ├── lmas-kb.md          # LMAS knowledge base
...
```javascript
const { globalConfigCache } = require('./.lmas-core/core');
...
```javascript
const { loadAgentConfig } = require('./.lmas-core/core');
...
```javascript
const { loadConfigSections } = require('./.lmas-core/core');
...
```javascript
const { ContextDetector } = require('./.lmas-core/core');
...
```javascript
const { SessionContextLoader } = require('./.lmas-core/core');
...
```javascript
const { ElicitationEngine } = require('./.lmas-core/core');
...
```javascript
const { ElicitationSessionManager } = require('./.lmas-core/core');
...
```javascript
const { YAMLValidator, validateYAML } = require('./.lmas-core/core');
...
```javascript
const { PersonalizedOutputFormatter } = require('./.lmas-core/core');
