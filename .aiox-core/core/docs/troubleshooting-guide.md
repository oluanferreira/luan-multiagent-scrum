...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /lmas-core/agents/data-analyst.md
```
...
3. If overwriting is intended:
   ...
   rm lmas-core/agents/data-analyst.md
   *create-agent
   ```
...
1. Create session directory:
   ```bash
   mkdir -p lmas-core/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 lmas-core/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls lmas-core/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la lmas-core/agents/file1.md
   
   # Remove manually if needed
   rm lmas-core/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls lmas-core/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat lmas-core/logs/transactions/latest.json
   ```
...
# Specific modules
DEBUG=lmas:template,lmas:elicitation *create-agent
...
```bash
# Application logs
tail -f lmas-core/logs/lmas-developer.log

# Transaction logs
ls -la lmas-core/logs/transactions/

# Error logs
grep ERROR lmas-core/logs/*.log
```
...
- Template syntax: `lmas-core/docs/template-syntax.md`
- Creation guide: `lmas-core/docs/component-creation-guide.md`
- API reference: `lmas-core/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf lmas-backup.tar.gz lmas-core/
   ```
