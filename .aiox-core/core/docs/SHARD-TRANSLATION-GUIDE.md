...
Synkra LMAS now automatically translates Portuguese documentation into English filenames when sharding documents. This ensures consistency across all projects and compatibility with hardcoded English filenames in configuration files.
...
**See full dictionary:** `.lmas-core/tasks/shard-doc.md` (Section 3)
...
**Configuration:** `.lmas-core/core-config.yaml`
...
```bash
# Check for issues
node .lmas-core/scripts/validate-filenames.js

# Auto-fix issues (renames files)
node .lmas-core/scripts/validate-filenames.js --fix
```
...
Edit `.lmas-core/tasks/shard-doc.md`, Section 3:
...
**Solution:**
1. Check you're using the updated `.lmas-core/tasks/shard-doc.md`
2. Run validator to confirm: `node .lmas-core/scripts/validate-filenames.js`
...
**Manual fix:**
Edit `.lmas-core/core-config.yaml` to add more fallbacks.
...
```javascript
// .lmas-core/scripts/validate-filenames.js
...
A: Edit `.lmas-core/tasks/shard-doc.md`, Section 3, and add to the YAML dictionary.
...
2. Run the validator: `node .lmas-core/scripts/validate-filenames.js`
3. Review `.lmas-core/tasks/shard-doc.md` for translation rules
