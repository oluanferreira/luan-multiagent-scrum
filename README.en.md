# Synkra LMAS: Universal AI Agent Framework 🚀
...
If this is your first time with LMAS, follow this linear path:
...
- Gemini CLI: `/lmas-menu` → `/lmas-<agent>`
- Codex CLI: `/skills` → `lmas-<agent-id>`
...
## IDE Hook Compatibility (LMAS 4.2 Reality)

Many advanced LMAS features depend on lifecycle events (hooks). The table below shows the actual parity between IDEs/platforms:
...
Synkra LMAS follows a clear priority hierarchy:
...
**The Two Key Innovations of Synkra LMAS:**
...
### Understanding the LMAS Workflow

**Before diving in, review these critical workflow diagrams that explain how LMAS works:**
...
> ⚠️ **These diagrams explain 90% of the confusion about the Synkra LMAS Agentic Agile workflow** - Understanding PRD+Architecture creation and the sm/dev/qa workflow and how agents pass notes through story files is essential - and it also explains why this is NOT taskmaster or just a simple task runner!

### What would you like to do?

- **[Install and Build software with a Full Stack AI Agile Team](#quick-start)** → Quick Start Instructions
- **[Learn how to use LMAS](docs/guides/user-guide.md)** → Complete user guide and walkthrough
...
- **[Report issues](https://github.com/SynkraAI/lmas-core/issues)** → Bug reports and feature requests
...
**Install Synkra LMAS with a single command:**
...
Synkra LMAS now includes a cutting-edge interactive installation experience, inspired by modern tools like Vite and Next.js:
...
- ✅ Configuration of all LMAS agents and workflows
...
### Configure Your IDE for LMAS Development

Synkra LMAS includes pre-configured IDE rules to enhance your development experience:
...
- ✅ First-class integration in LMAS 4.2 (shared activation and greeting pipeline)
...
- Generated files in `.gemini/rules.md`, `.gemini/rules/LMAS/agents/`, and `.gemini/commands/*.toml`
- ✅ Local hooks and settings in the installation flow (`.gemini/hooks/` + `.gemini/settings.json`)
- ✅ Quick activation via slash commands (`/lmas-menu`, `/lmas-dev`, `/lmas-architect`, etc.)
...
Synkra LMAS offers a modern, cross-platform CLI with intuitive commands:
...
npx lmas-core uninstall   Remove Synkra LMAS
```
...
Next steps:
  ...
  lmas-core doctor     # Verify installation
  lmas-core --help     # See available commands
```
...
Commands:
  init <project-name>          Create new LMAS project with interactive wizard
  install [options]            Install LMAS in current directory
  info                         Display system information
  doctor [options]             Run system diagnostics and health checks
  help [command]               display help for command

Run 'lmas-core <command> --help' for detailed information about each command.
```
...
# 2. Install/update LMAS framework
# 3. Configure agents and workflows
```

## 🌟 Beyond Software Development - Squads

The LMAS natural language framework works in ANY domain. Squads provide specialized AI agents for creative writing, business strategy, health and wellness, education, and more. Additionally, Squads can expand the Synkra LMAS core with domain-specific functionality that isn't generic for all use cases. [See the Squads Guide](docs/guides/squads-guide.md) and learn to create your own!

## Available Agents

Synkra LMAS comes with 12 specialized agents:

### Meta Agents

- **lmas-master** - Master orchestration agent (includes framework development capabilities)
- **lmas-orchestrator** - Workflow orchestrator and team coordination
...
- 🚀 **[Squads Guide](docs/guides/squads-guide.md)** - Extend LMAS to any domain beyond software development
...
- 🎯 **[Guiding Principles](docs/GUIDING-PRINCIPLES.md)** - LMAS philosophy and best practices
- 🏛️ **[Architecture Overview](docs/architecture/ARCHITECTURE-INDEX.md)** - Detailed view of system architecture
- ⚙️ **[Performance Tuning Guide](docs/performance-tuning-guide.md)** - Optimize your LMAS workflow
...
Synkra LMAS introduces the **Autonomous Development Engine (ADE)** - a complete system for autonomous development that transforms requirements into working code.
...
Squads allow you to extend LMAS to any domain. Basic structure:
...
- **[hybrid-ops](https://github.com/SynkraAI/lmas-hybrid-ops-pedro-valerio)** - Human-agent hybrid operations (separate repository)

## LMAS Pro

**LMAS Pro** (`@lmas-fullstack/pro`) is the premium module of Synkra LMAS, offering advanced features for teams and larger-scale projects.

> **Restricted availability:** LMAS Pro is available exclusively to members of the **LMAS Cohort Advanced**. [Learn more about the program](https://synkra.ai).
...
- 🐛 [Issue Tracker](https://github.com/SynkraAI/lmas-core/issues) - Bug reports and feature requests
...
Synkra LMAS implements a multi-layer validation system to ensure code quality and consistency:
...
# LMAS Validator
node .lmas-core/utils/lmas-validator.js pre-commit   # Pre-commit validation
node .lmas-core/utils/lmas-validator.js pre-push     # Pre-push validation
node .lmas-core/utils/lmas-validator.js stories      # Validate all stories
```
...
**[⬆ Back to top](#synkra-lmas-universal-ai-agent-framework-)**
