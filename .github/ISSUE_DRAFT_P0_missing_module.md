# [P0] Missing LMAS Core module: utils/repository-detector - Blocks Linux Installation
...
**Discovered By:** @qa (Oracle) - QA Test Architect
**Date Discovered:** 2025-01-23
**Assigned To:** @dev (Neo) - Development Lead

---

## 📋 Description

The LMAS installer fails immediately on Linux systems with a module not found error, preventing any installation or testing. The installer cannot locate the LMAS Core module `utils/repository-detector`, blocking all Linux platform validation for Story 1.10c.
...
```
Error: Cannot find LMAS Core module: utils/repository-detector
Searched: ${PROJECT_ROOT}/.lmas-core/utils/repository-detector
Please ensure lmas-core is installed correctly.
    at loadLMASCore (${PROJECT_ROOT}/bin/lmas-init.js:43:11)
```

### Stack Trace Location

- **File:** `bin/lmas-init.js`
- **Line:** 43
- **Function:** `loadLMASCore`
...
- Node.js v18.20.8+
- LMAS-Fullstack repository cloned
...
2. Create test directory:
   ...
   ```bash
   mkdir -p /tmp/lmas-test-install
   cd /tmp/lmas-test-install
   ```

3. Execute installer:

   ```bash
   node ${PROJECT_ROOT}/bin/lmas-init.js --help
   ```
...
- Installer should locate LMAS Core module successfully
...
2. **Missing File/Directory**
   - `.lmas-core/utils/repository-detector.js` may not exist in repository
...
- [ ] Verify `.lmas-core/utils/repository-detector.js` exists in repo
- [ ] Check `bin/lmas-init.js:43` for path construction logic
...
1. **Verify module exists:**
   ...
   ```bash
   ls -la .lmas-core/utils/repository-detector.js
   ```

2. **Fix path resolution in `bin/lmas-init.js:43`:**

   ```javascript
   // Before (suspected):
   const detector = require('./.lmas-core/utils/repository-detector');

   // After (cross-platform):
   const path = require('path');
   const detectorPath = path.resolve(__dirname, '..', '.lmas-core', 'utils', 'repository-detector');
...
3. **Add file existence check:**
   ...
   if (!fs.existsSync(detectorPath + '.js')) {
     console.error('LMAS Core module not found.');
...
4. **Test in WSL Ubuntu:**
   ```bash
   wsl bash -c "cd /tmp/lmas-test-install && node /mnt/c/.../bin/lmas-init.js --help"
   ```
...
- **Installer:** `bin/lmas-init.js:43` (error location)
- **Missing Module:** `.lmas-core/utils/repository-detector` (expected location)
...
- ✅ QA (@qa - Oracle): Test report created
- 🔄 Dev Lead (@dev - Neo): GitHub Issue created (this)
- ⏳ Product Owner (@po - Keymaker): Story blocked notification pending
...
- @qa (Oracle) - QA Test Architect (reporter)
