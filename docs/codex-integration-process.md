# Integracao LMAS com Codex CLI (Estado Atual)

Este documento descreve o estado operacional atual da integracao LMAS + Codex CLI no LMAS `4.2.11`.
O foco aqui nao e historico: e operacao pratica, compatibilidade real e como manter sem regressao.

## Resumo Executivo

O Codex hoje e alvo de primeira classe no LMAS:
...
Em termos praticos: Codex esta no mesmo trilho arquitetural das integracoes principais do LMAS, sem caminho paralelo legado.

## Status de Compatibilidade (LMAS 4.2)

| O que você quer fazer | Funciona no Codex? | Como fazer |
| --- | --- | --- |
| Ativar agentes LMAS | Works | `/skills` depois escolha `lmas-<agent-id>` |
| Sincronizar e validar arquivos LMAS | Works | `npm run sync:ide:codex` e `npm run validate:codex-sync` |
...
- Agentes canonicos: `.lmas-core/development/agents/*.md`
- Regras de projeto Codex: `AGENTS.md`

### Pipeline de ativacao

- Runtime: `.lmas-core/development/scripts/activation-runtime.js`
- Entrada de greeting: `.lmas-core/development/scripts/generate-greeting.js`
...
- Skills locais: `.codex/skills/lmas-*/SKILL.md`
...
4. No Codex, usar `/skills` e escolher `lmas-<agent-id>` como ativacao padrao.
...
- Integracao estrutural: `.lmas-core/infrastructure/scripts/validate-codex-integration.js`
- Skills: `.lmas-core/infrastructure/scripts/codex-skills-sync/validate.js`
- Paths/contratos: `.lmas-core/infrastructure/scripts/validate-paths.js`
...
- `.codex/skills/lmas-*/SKILL.md` existente
- validadores sem erro

## Recursos Recentes do Codex (LMAS 4.0)
...
No LMAS, o caminho recomendado continua: `AGENTS.md` + `/skills` + MCP + scripts de sync/validacao.
...
- menor automacao de eventos de ciclo de sessao (`SessionStart/SessionEnd`) no padrao LMAS
...
- garantir skill apontando para agente canonico em `.lmas-core/development/agents/`
