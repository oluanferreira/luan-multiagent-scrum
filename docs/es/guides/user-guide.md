...
# Guia de Usuario LMAS

> **ES** | [EN](../../guides/user-guide.md)

---

Guia completa para usar Synkra LMAS - el Sistema Orquestado por IA para Desarrollo Full Stack.
...
Antes de usar LMAS, asegurate de tener:
...
LMAS proporciona estructura orquestada mientras permite flexibilidad en la comunicacion. Esto significa:
...
| Desarrollo IA Tradicional       | LMAS                                       |
| ------------------------------- | ------------------------------------------ |
...
LMAS incluye 11 agentes especializados, cada uno con un rol y personalidad distintos:

| Agente    | ID               | Arquetipo    | Responsabilidad            |
| --------- | ---------------- | ------------ | -------------------------- |
| **Neo**   | `@dev`           | Constructor  | Implementacion de codigo   |
| **Oracle** | `@qa`            | Guardian     | Aseguramiento de calidad   |
...
| **Dozer**  | `@data-engineer` | Arquitecto   | Ingenieria de datos        |
| **Felix** | `@devops`        | Optimizador  | CI/CD y operaciones        |
| **Uma**   | `@ux-expert`     | Creador      | Experiencia de usuario     |
| **Keymaker**   | `@lmas-master`   | Orquestador  | Orquestacion del framework |

### Activacion de Agentes

```bash
# Activar un agente usando sintaxis @
@dev                # Activar Neo (Desarrollador)
@qa                 # Activar Oracle (QA)
@architect          # Activar Aria (Arquitecto)
@lmas-master        # Activar Keymaker (Orquestador)
...
Las Tasks son el punto de entrada principal en LMAS. Todo es una task.
...
| `brownfield-integration` | Agregar LMAS a existente       | dev, architect       |
...
Los Squads son equipos modulares de agentes IA que extienden la funcionalidad de LMAS.
...
# Descargar un squad
lmas squads download etl-squad
...
```
my-project/
├── .lmas-core/                # Configuracion del framework
...
La configuracion principal esta en `.lmas-core/core/config/`:
...
LMAS soporta multiples IDEs. La configuracion se sincroniza a traves de:
...
cat .lmas-core/development/tasks/{task-name}.md
...
LMAS implementa quality gates de 3 capas:
...
_Guia de Usuario Synkra LMAS v4.0_
