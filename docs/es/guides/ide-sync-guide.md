...
Sincroniza agentes, tasks, workflows y checklists de LMAS a traves de multiples configuraciones de IDE.

## Vision General

La task `*command` automatiza la sincronizacion de componentes LMAS a todos los directorios de IDE configurados (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminando operaciones de copia manuales.
...
```bash
cp .lmas-core/infrastructure/templates/lmas-sync.yaml.template .lmas-sync.yaml
```

### 2. Configurar IDEs

Editar `.lmas-sync.yaml` para habilitar tus IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Leer configuracion .lmas-sync.yaml               │
...
```
tu-proyecto/
├── .lmas-sync.yaml           # Configuracion de sincronizacion
...
**Solucion**: Agregar el alias a `.lmas-sync.yaml`:
...
- [Arquitectura LMAS](../core-architecture.md)
