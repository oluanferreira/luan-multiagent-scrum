...
| **Hallazgos de Oracle**      | Problemas encontrados por el agente @qa               |
...
```
.lmas/data/quality-metrics.json
```
...
```bash
# Sincronizar desde .lmas/data al dashboard
npm run sync-metrics
...
Esto copia `.lmas/data/quality-metrics.json` a `tools/quality-dashboard/public/.lmas/data/`.
...
ls -la .lmas/data/quality-metrics.json

# Si falta, sembrar con datos de ejemplo
npx lmas metrics seed
```
