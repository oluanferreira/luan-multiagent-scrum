...
> Controla cuanta autonomia tienen los agentes LMAS sobre tu sistema.

---

## Vision General

Los Modos de Permisos te permiten controlar el nivel de autonomia que tienen los agentes LMAS. Ya sea que estes explorando un nuevo codebase o ejecutando builds completamente autonomos, hay un modo para tu flujo de trabajo.
...
El modo se persiste en `.lmas/config.yaml`:
...
```yaml
# .github/workflows/lmas.yml
- name: Run LMAS
  run: |
    echo "permissions:\n  mode: auto" > .lmas/config.yaml
    lmas run build
```
...
Verificar que `.lmas/config.yaml` existe y es escribible:

```bash
ls -la .lmas/config.yaml
```
