...
# Guía del Sistema de Quality Gates de LMAS

> 🌐 [EN](../../guides/quality-gates.md) | [PT](../../pt/guides/quality-gates.md) | **ES**

---

> Guía completa del sistema de quality gates de 3 capas para Synkra LMAS.
...
El Sistema de Quality Gates de LMAS proporciona aseguramiento de calidad automatizado a través de tres capas progresivas de validación. Cada capa detecta diferentes tipos de problemas en la etapa apropiada del desarrollo.
...
```yaml
# .lmas-core/core/quality-gates/quality-gate-config.yaml
layer1:
  enabled: true
...
lmas qa run --layer=1 --check=lint
lmas qa run --layer=1 --check=test
lmas qa run --layer=1 --check=typecheck

# Ejecutar con salida detallada
lmas qa run --layer=1 --verbose
```
...
| **Oracle (@qa)** | Revisión de QA automatizada | CRÍTICO              |
...
layer2:
  ...
  quinn:
    enabled: true
    autoReview: true
    agentPath: '.claude/commands/LMAS/agents/qa.md'
    severity:
      block: ['CRITICAL']
      warn: ['HIGH', 'MEDIUM']
```
...
lmas qa run --layer=2 --tool=coderabbit

# Ejecutar revisión de Oracle (@qa)
lmas qa run --layer=2 --tool=quinn
```
...
```javascript
// Invocación programática de Oracle
...
lmas qa request-review --pr=123

# Aprobar la revisión
lmas qa signoff --pr=123 --reviewer="@architect"

# Verificar estado de aprobación
lmas qa signoff-status --pr=123
```
...
lmas qa run --verbose          # Salida detallada
lmas qa run --fail-fast        # Detener en primera falla
lmas qa run --continue-on-fail # Continuar a pesar de fallas
```
...
lmas qa status --layer=1

# Obtener estado para PR
lmas qa status --pr=123
```
...
# Exportar a archivo
lmas qa report --output=qa-report.json
lmas qa report --format=markdown --output=qa-report.md
```
...
lmas qa configure --layer1.coverage.minimum=90
lmas qa configure --layer2.coderabbit.enabled=false
lmas qa configure --layer3.requireSignoff=true
```
...
jobs:
  ...
  layer3:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: lmas qa request-review --pr=${{ github.event.pull_request.number }}
```
...
| Oracle bloqueó         | Revisar feedback de @qa, actualizar código         |
...
_Guía del Sistema de Quality Gates de Synkra LMAS v4_
