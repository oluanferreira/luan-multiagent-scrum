...
El Sistema de Saludo Contextual es una mejora de UX que hace que los saludos de los agentes LMAS sean inteligentes y adaptativos, mostrando información y comandos relevantes basados en el contexto de la sesión.

## Lo Que Se Ha Implementado

### Componentes Core (Story 6.1.2.5)

1. **ContextDetector** (`.lmas-core/core/session/context-detector.js`)
...
2. **GitConfigDetector** (`.lmas-core/infrastructure/scripts/git-config-detector.js`)
   - Detecta configuración de git del proyecto
   - Caché con TTL de 5 minutos
   - Protección de timeout de 1000ms

3. **GreetingBuilder** (`.lmas-core/development/scripts/greeting-builder.js`)
...
4. **WorkflowNavigator** (`.lmas-core/development/scripts/workflow-navigator.js`)
...
5. **Workflow Patterns** (`.lmas-core/data/workflow-patterns.yaml`)
...
**Ejemplo:**
```
💻 Neo (Builder) ready. Let's build something solid!
...
```bash
node .lmas-core/development/scripts/test-greeting-system.js
```
...
### Scripts Core
- `.lmas-core/core/session/context-detector.js` - Detección de tipo de sesión
- `.lmas-core/infrastructure/scripts/git-config-detector.js` - Detección de git config
- `.lmas-core/development/scripts/greeting-builder.js` - Construcción del saludo
- `.lmas-core/development/scripts/workflow-navigator.js` - Navegación de workflow
- `.lmas-core/development/scripts/agent-exit-hooks.js` - Hooks de salida (para persistencia)

### Archivos de Datos
- `.lmas-core/data/workflow-patterns.yaml` - Definiciones de workflows
...
### Configuración
- `.lmas-core/core-config.yaml` - Configuración global (secciones git + agentIdentity)

### Agentes (Actualizados)
- `.lmas-core/agents/dev.md` - Metadatos de visibilidad de comandos
- `.lmas-core/agents/po.md` - Metadatos de visibilidad de comandos
- `.lmas-core/agents/*.md` - 9 agentes restantes (actualización pendiente)
...
R: Usa el script de prueba: `node .lmas-core/development/scripts/test-greeting-system.js`
