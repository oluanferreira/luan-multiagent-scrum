# Referencia de API LMAS

> [EN](../../guides/api-reference.md) | [PT](../../pt/guides/api-reference.md) | **ES**

---

Referencia completa de la API para Synkra LMAS - el Sistema Orquestado por IA para Desarrollo Full Stack.
...
LMAS proporciona una API unificada para interactuar con agentes de IA especializados a través de dos mecanismos principales:
...
| ---------------- | ------ | ------------ | ---------------------------------------------- |
| `@dev`           | Neo    | Builder      | Implementación de código, depuración, pruebas  |
| `@qa`            | Oracle  | Guardian     | Aseguramiento de calidad, revisión de código   |
...
| `@po`            | Keymaker    | Champion     | Gestión de backlog, criterios de aceptación    |
...
| `@data-engineer` | Dozer   | Architect    | Esquema de BD, migraciones, consultas          |
| `@devops`        | Operator   | Optimizer    | CI/CD, despliegue, operaciones git             |
| `@ux-expert`     | Uma    | Creator      | Diseño UI/UX, wireframes                       |
| `@lmas-master`   | Morpheus  | Orchestrator | Orquestación del framework, meta-operaciones   |

### Comportamiento de Activación

Cuando se activa un agente:

1. Se carga el archivo de definición del agente desde `.lmas-core/development/agents/{id}.md`
...
# Salida:
# 💻 Neo (Builder) listo. ¡Construyamos algo genial!
#
# **Comandos Rápidos:**
...
*develop story-1.2.3
    │
    ▼
.lmas-core/development/tasks/dev-develop-story.md
    │
    ▼
...
### @lmas-master (Orquestador)

**Desarrollo del Framework:**

| Comando                | Argumentos      | Descripción                                    |
| ---------------------- | --------------- | ---------------------------------------------- |
| `*create`              | `{type} {name}` | Crear componente LMAS (agent/task/workflow)    |
...
| `*kb`   | -                          | Alternar modo KB (conocimiento LMAS Method)   |
...
```bash
# Iniciar workflow
@lmas-master *workflow greenfield-fullstack
...
| -------------------- | ---------------------------------- | -------------------------------------- |
| `AGENT_NOT_FOUND`    | Definición de agente faltante      | Verificar `.lmas-core/development/agents/` |
...
/lmas-master  → Activa agente @lmas-master
```
...
- [Guía de Usuario](./user-guide.md) - Comenzando con LMAS
...
- [Guía de Squads](./squads-guide.md) - Extendiendo LMAS con squads

---

_Synkra LMAS API Reference v4.2.11_
