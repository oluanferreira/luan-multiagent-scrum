...
La funcionalidad de Estado del Proyecto muestra automáticamente tu contexto de trabajo actual cuando cualquier agente LMAS se activa. Esto incluye:
...
3. Crea el archivo de caché `.lmas/project-status.yaml`
...
1. Editar `.lmas-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Ubicación: `.lmas-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .lmas/project-status.yaml  # Ubicación del archivo de caché
...
1. **Verifica caché** - Busca `.lmas/project-status.yaml`
...
- **Ubicación del Caché:** `.lmas/project-status.yaml`
...
Los 11 agentes LMAS muestran estado del proyecto:
...
3. ¿Existe `.lmas-core/infrastructure/scripts/project-status-loader.js`?
...
R: Sí, todos los agentes usan el mismo archivo de caché (`.lmas/project-status.yaml`).
...
- **Config:** `.lmas-core/core-config.yaml` (sección projectStatus)
- **Script:** `.lmas-core/infrastructure/scripts/project-status-loader.js`
- **Init Task:** `.lmas-core/tasks/init-project-status.md`
- **Tests:** `.lmas-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
