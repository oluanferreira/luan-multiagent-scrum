...
Guia completa para crear, validar, publicar y gestionar Squads en LMAS.

> **LMAS Squads:** Equipos de agentes de IA trabajando contigo
...
Los Squads son equipos modulares de agentes de IA que extienden la funcionalidad de LMAS. Cada squad es un paquete autocontenido que incluye:
...
│  Nivel 2: LMAS-SQUADS  → github.com/oluanferreira (Publico/Gratis)│
...
| -------------------------------------------------------------------------- | ------- | ------------------------------------- |
| [etl-squad](https://github.com/oluanferreira/luan-multiagent-scrum/tree/main/etl)         | 2.0.0   | Recoleccion y transformacion de datos |
| [creator-squad](https://github.com/oluanferreira/luan-multiagent-scrum/tree/main/creator) | 1.0.0   | Utilidades de generacion de contenido |
...
- Node.js 18+
- Proyecto LMAS inicializado (`.lmas-core/` existe)
...
Las tareas deben seguir [TASK-FORMAT-SPECIFICATION-V1](../../../.lmas-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Modo       | Comportamiento                                     |
| ---------- | -------------------------------------------------- |
| `extend`   | Agregar reglas del squad a las reglas base de LMAS |
...
```javascript
const { SquadAnalyzer } = require('./.lmas-core/development/scripts/squad/squad-analyzer');
const { SquadExtender } = require('./.lmas-core/development/scripts/squad/squad-extender');
...
```javascript
const { SquadValidator } = require('./.lmas-core/development/scripts/squad');
...
### Nivel 2: Repositorio lmas-squads (Publico)
...
Esto crea un PR a [LMAS/lmas-squads](https://github.com/oluanferreira/luan-multiagent-scrum).

### Nivel 3: Marketplace de LMAS
...
3. **Agregar Campos** - `lmas.type`, `lmas.minVersion`
...
```javascript
const { SquadLoader } = require('./.lmas-core/development/scripts/squad');
...
```javascript
const { SquadLoader, SquadLoaderError } = require('./.lmas-core/development/scripts/squad');
...
# - lmas.type: debe ser "squad"
# - lmas.minVersion: debe ser semver valido
```
...
### Cual es la diferencia entre un Squad y formatos legados de squad en LMAS?

**Squads** son el formato estandar en LMAS 2.1+ con:
...
3. Re-publicar: `*publish-squad` o `*sync-squad-lmas`
...
- **Nivel 3**: Sincronizar con bandera `--private`: `*sync-squad-lmas my-squad --private`

### Cual es la version minima de LMAS para Squads?

Los Squads requieren LMAS 2.1.0+. Configurar en el manifiesto:
...
- [TASK-FORMAT-SPECIFICATION-V1](../../../.lmas-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md)
...
- [Discusiones de GitHub](https://github.com/oluanferreira/luan-multiagent-scrum/discussions)
- [Rastreador de Issues](https://github.com/oluanferreira/luan-multiagent-scrum/issues)

---

_LMAS Squads: Equipos de agentes de IA trabajando contigo_
