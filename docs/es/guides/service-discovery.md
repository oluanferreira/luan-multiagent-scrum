...
# Guía de Descubrimiento de Servicios de LMAS

> 🌐 [EN](../../guides/service-discovery.md) | [PT](../../pt/guides/service-discovery.md) | **ES**

---

> Cómo descubrir, consultar y usar workers en el framework LMAS.
...
El sistema de Descubrimiento de Servicios permite encontrar y usar workers (tareas, plantillas, scripts, workflows) en todo el framework LMAS. El **Registro de Servicios** es el catálogo central que contiene metadatos sobre todos los workers disponibles.
...
```javascript
const { getRegistry, loadRegistry } = require('./.lmas-core/core/registry/registry-loader');
...
//   name: 'Create Story',
//   category: 'task',
//   path: '.lmas-core/development/tasks/po-create-story.md',
...
```bash
# Buscar por texto
lmas discover "create story"

# Buscar por categoría
lmas discover --category task

# Buscar por etiqueta
lmas discover --tag testing

# Buscar para agente
lmas discover --agent dev

# Combinar filtros
lmas discover --category task --tag development --agent dev
```
...
Encontrados 5 workers que coinciden con "create story":
  ...
  [task] po-create-story
         Ruta: .lmas-core/development/tasks/po-create-story.md
         Etiquetas: task, creation, story, product
         Agentes: po

  [task] dev-create-brownfield-story
         Ruta: .lmas-core/development/tasks/dev-create-brownfield-story.md
...
lmas info create-story

# Obtener info del worker con ruta completa
lmas info --path .lmas-core/development/tasks/po-create-story.md
```
...
lmas list tasks --page 1 --limit 20
```
...
**Ubicación:** `.lmas-core/development/tasks/`
...
**Ubicación:** `.lmas-core/product/templates/`
...
**Ubicación:** `.lmas-core/infrastructure/scripts/`
...
**Ubicación:** `.lmas-core/development/workflows/`
...
**Ubicación:** `.lmas-core/product/checklists/`
...
node .lmas-core/core/registry/build-registry.js
```

El constructor escanea:

- `.lmas-core/development/tasks/**/*.md`
- `.lmas-core/product/templates/**/*`
- `.lmas-core/infrastructure/scripts/**/*.js`
- `.lmas-core/product/checklists/**/*.md`
- `.lmas-core/development/workflows/**/*.yaml`
- `.lmas-core/core/data/**/*`
...
```javascript
const { getRegistry } = require('./.lmas-core/core/registry/registry-loader');
const { TaskExecutor } = require('./.lmas-core/development/scripts/task-executor');
...
ls .lmas-core/core/registry/service-registry.json

# Reconstruir registro
node .lmas-core/core/registry/build-registry.js

# Validar registro
node .lmas-core/core/registry/validate-registry.js
```
...
_Guía de Descubrimiento de Servicios de Synkra LMAS v4_
