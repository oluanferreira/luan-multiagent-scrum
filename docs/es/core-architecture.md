...
# Método LMAS: Arquitectura Central
...
Todo el ecosistema del Método LMAS está diseñado alrededor del directorio `lmas-core` instalado, que actúa como el cerebro de la operación. El directorio `tools` proporciona los medios para procesar y empaquetar este cerebro para diferentes entornos.
...
El directorio `lmas-core` contiene todas las definiciones y recursos que dan a los agentes sus capacidades.

### 3.1. Agentes (`lmas-core/agents/`)

- **Propósito**: Estos son los bloques de construcción fundamentales del sistema. Cada archivo markdown (por ejemplo, `lmas-master.md`, `pm.md`, `dev.md`) define la persona, capacidades y dependencias de un único agente de IA.
...
- **Ejemplo**: El agente `lmas-master` lista sus dependencias, lo que indica a la herramienta de construcción qué archivos incluir en un paquete web e informa al agente de sus propias capacidades.

### 3.2. Equipos de Agentes (`lmas-core/agent-teams/`)
...
### 3.3. Flujos de Trabajo (`lmas-core/workflows/`)

- **Propósito**: Los flujos de trabajo son archivos YAML (por ejemplo, `greenfield-fullstack.yaml`) que definen una secuencia prescrita de pasos e interacciones de agentes para un tipo de proyecto específico. Actúan como una guía estratégica para el usuario y el agente `lmas-orchestrator`.
...
LMAS incluye una capa de personalización a través del archivo `technical-preferences.md` en `lmas-core/data/`. Este archivo sirve como un perfil técnico persistente que influye en el comportamiento de los agentes en todos los proyectos.
...
- **Para IDEs**: Los usuarios interactúan con los agentes directamente a través de sus archivos markdown en `lmas-core/agents/`. La integración del IDE (para Cursor, Claude Code, etc.) sabe cómo llamar a estos agentes.
...
## 5. Flujos de Trabajo de LMAS

### 5.1. El Flujo de Trabajo de Planificación

Antes de que comience el desarrollo, LMAS sigue un flujo de trabajo de planificación estructurado que establece la base para la ejecución exitosa del proyecto:
...
**Orquestación del Flujo de Trabajo**: El agente `lmas-orchestrator` usa estas definiciones de flujo de trabajo para guiar a los usuarios a través del proceso completo, asegurando transiciones apropiadas entre las fases de planificación (UI web) y desarrollo (IDE).

### 5.2. El Ciclo Central de Desarrollo

Una vez que las fases iniciales de planificación y arquitectura están completas, el proyecto pasa a un flujo de trabajo de desarrollo cíclico, como se detalla en `lmas-kb.md`. Esto asegura un proceso de implementación constante, secuencial y con control de calidad.
