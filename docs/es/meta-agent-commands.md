...
Guia de referencia completa para todos los comandos del meta-agente de Synkra LMAS.
...
LMAS> create-agent helper
LMAS> activate helper
LMAS> helper translate "Hello" --to spanish
LMAS> exit
```
...
```bash
LMAS_AI_PROVIDER=openai          # Proveedor de IA
LMAS_AI_MODEL=gpt-4             # Modelo de IA
LMAS_LOG_LEVEL=debug            # Nivel de log
LMAS_TELEMETRY=disabled         # Configuracion de telemetria
LMAS_TIMEOUT=30000             # Timeout de comando (ms)
LMAS_MEMORY_CACHE=true         # Cache de memoria
```
