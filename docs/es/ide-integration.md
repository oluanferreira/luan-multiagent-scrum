...
Guía para integrar LMAS con IDEs compatibles y plataformas de desarrollo con IA.
...
LMAS es compatible con 6 plataformas de desarrollo potenciadas por IA. Elige la que mejor se adapte a tu flujo de trabajo.
...
| IDE | Paridad de Hooks vs Claude | Qué se degrada sin hooks completos | Mitigación en LMAS |
| --- | --- | --- | --- |
| Claude Code | Completa | Ninguno (comportamiento de referencia) | Hooks nativos + pipeline completo de LMAS |
...
**Nivel de Recomendación:** Mejor integración con LMAS
...
1. LMAS crea automáticamente el directorio `.claude/` al inicializar
...
4. Usa `/skills` y selecciona `lmas-<agent-id>`
...
1. LMAS crea el directorio `.cursor/` al inicializar
...
2. LMAS crea `.github/copilot-instructions.md`
...
1. LMAS crea el directorio `.antigravity/`
...
LMAS mantiene una única fuente de verdad para las definiciones de agentes y las sincroniza con todos los IDEs configurados:

```
┌─────────────────────────────────────────────────────┐
│                    LMAS Core                         │
│  .lmas-core/development/agents/  (Fuente de Verdad) │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
LMAS puede configurarse para sincronizar automáticamente cuando hay cambios en los agentes:
...
ls .claude/commands/LMAS/agents/  # Para Claude Code
```
...
├── Sí --> Claude Code (Mejor integración con LMAS)
└── No
    └── ¿Prefieres VS Code?
...
_Guía de Integración con IDEs de Synkra LMAS v4.0_
