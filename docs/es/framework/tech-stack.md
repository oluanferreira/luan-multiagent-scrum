...
**Aviso de Migración:** Este documento migrará al repositorio `SynkraAI/lmas-core` en Q2 2026 (ver Decision 005)
...
LMAS está construido sobre JavaScript/TypeScript moderno con runtime Node.js, optimizado para desarrollo CLI multiplataforma con UX interactiva y capacidades de orquestación de agentes.
...
LMAS se integra con servidores Model Context Protocol (MCP):
...
```yaml
Baseline: 30MB (Node.js + LMAS core)
...
```bash
# LMAS Configuration
LMAS_DEBUG=true                    # Enable debug logging
LMAS_CONFIG_PATH=/custom/path      # Custom config location
LMAS_YOLO_MODE=true               # Force yolo mode
...
| 1.1     | 2025-12-14 | Actualizado aviso de migración a SynkraAI/lmas-core, semantic-release a v25.0.2 [Story 6.10] | Neo (dev)        |

---

_Este es un estándar oficial del framework LMAS. Todas las elecciones tecnológicas deben alinearse con este stack._
