...
### ¿Por qué usar LMAS con Cursor?

Cursor es uno de los **IDEs de IA más populares** con excelente integración de LMAS:
...
Verificar que se crearon los archivos de LMAS:
...
# Reinicializar LMAS
npx @anthropic/lmas init --ide cursor --force
```
...
R: Ejecutar `npm run sync:ide` después de actualizaciones de LMAS.
...
2. Instalar LMAS:
   ```bash
   npx @anthropic/lmas init --ide cursor
   ```
...
2. Inicializar LMAS para Claude Code:
...
- [Guía de Integración de LMAS](../README.md)

---

_Synkra LMAS - Guía de Plataforma Cursor v1.0_
