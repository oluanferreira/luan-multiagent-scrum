...
### ¿Por qué usar LMAS con Gemini CLI?
...
## Activación
Incluye "Como el agente LMAS dev" en tu prompt.
...
### Activando Agentes de LMAS

Incluye contexto de agentes en los prompts:

```bash
# Agente de desarrollador
gemini "Como el agente LMAS dev, implementa la característica de login"

# Agente de QA
gemini "Como el agente LMAS qa, revisa este código por seguridad"

# Agente de arquitecto
gemini "Como el agente LMAS architect, diseña el esquema de base de datos"
```
...
cat src/auth.ts | gemini "Como LMAS qa, revisa este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como LMAS qa, revisa este archivo" >> reviews.md
done
```
...
1. Instalar LMAS para Gemini CLI:
   ```bash
...
1. Los agentes de LMAS en `.lmas-core/` son portátiles
