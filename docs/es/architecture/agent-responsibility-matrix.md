...
Este documento define límites claros de responsabilidad para todos los agentes LMAS, con enfoque particular en:
...
1. **Hook Git Pre-Push** (Aplicación Primaria)
   ...
   - Verifica: Variable de entorno `$LMAS_ACTIVE_AGENT`
...
- [ ] **Crear Hook Git Pre-Push**
  - Ubicación: `.git/hooks/pre-push`
  - Contenido: Verificar `$LMAS_ACTIVE_AGENT`, bloquear si != "github-devops"
...
- [ ] **Actualizar Scripts de Activación de Agentes**
  - Agregar configuración de variable de entorno: `LMAS_ACTIVE_AGENT={agent_id}`
  - Configurar `LMAS_GIT_PUSH_ALLOWED` apropiadamente
