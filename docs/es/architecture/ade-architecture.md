...
El **Motor de Desarrollo Autónomo (ADE)** es la infraestructura de LMAS para flujos de trabajo de desarrollo autónomo. Permite que los agentes de IA trabajen de forma independiente a través de pipelines inteligentes, bucles autorreparables y aprendizaje persistente.
...
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────────────────────────────────────────────────────────┐  │ │
│  │  │                     Estado en Tiempo de Ejecución .lmas/        │  │ │
│  │  └─────────────────────────────────────────────────────────────────┘  │ │
│  │         │                │                │                │          │ │
│  │         ▼                ▼                ▼                ▼          │ │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐   │ │
...
Todo el estado de ADE se persiste en `.lmas/` para:
...
Archivo de Estado: .lmas/project-status.yaml
```
...
| ----------------- | ------------------------------- | --------------------------------- |
| Patrones de Código | Patrones reutilizables de la base de código | .lmas/patterns/code-patterns.json |
| Gotchas           | Trampas conocidas y soluciones  | .lmas/patterns/gotchas.json       |
| Insights de Sesión | Descubrimientos durante sesiones | .lmas/sessions/                   |
| Mapa de Base de Código | Análisis de estructura del proyecto | .lmas/codebase-map.json           |
...
ADE se integra con agentes LMAS a través de:
...
Todos los componentes de ADE actualizan `.lmas/status.json`:
...
Ubicado en `.lmas-core/core-config.yaml`:
...
```
.lmas-core/workflow-intelligence/
├── engine/
...
_Este es un estándar oficial del framework LMAS que documenta el Motor de Desarrollo Autónomo._
