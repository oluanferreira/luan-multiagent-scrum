...
Este documento define los modos de ejecución para el Sistema de Verificación de Salud de LMAS (HCS), basado en investigación de mejores prácticas de la industria de Kubernetes, VS Code, Terraform, npm/yarn, y patrones CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Al instalar/bootstrap**    | `npx lmas install` | 60-120s  | Esperado                 | Validación de setup         | ✅ **Requerido**  |
...
jobs:
  health-check:
    ...
    steps:
      ...
      - run: npx lmas health-check --mode=full --report
      - uses: actions/upload-artifact@v4
        with:
          name: health-report
          path: .lmas/reports/health-check-*.md
```
...
**Veredicto:** Requerido. Parte de `npx lmas install` y `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Post to Slack (on failure)
        ...
        with:
          payload: |
            {
              "text": "⚠️ LMAS Health Check Falló",
              "blocks": [
                {
                  "type": "section",
                  "text": {
...
jobs:
  post-merge-health:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: npx lmas health-check --mode=quick
```
...
{
  "lmas.healthCheck.enabled": true,
  "lmas.healthCheck.interval": 300,
  "lmas.healthCheck.mode": "quick",
  "lmas.healthCheck.showNotifications": true
}
```
