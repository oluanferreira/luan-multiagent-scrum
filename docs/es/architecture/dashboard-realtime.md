# 🔴 LMAS Dashboard - Arquitectura de Observabilidad en Tiempo Real
...
Este documento describe la arquitectura para **observabilidad en tiempo real** del LMAS Dashboard, permitiendo que los usuarios monitoreen comandos ejecutados en la CLI con máximo detalle visual.
...
│                                   ▼                                      │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │               .lmas/dashboard/events.jsonl (append-only)         │    │
...
```
.lmas-core/core/events/dashboard-emitter.ts
```

### Interfaz

```typescript
// .lmas-core/core/events/types.ts
...
const EVENTS_DIR = '.lmas/dashboard';
...
```typescript
// .lmas-core/integrations/claude-code/hooks.ts
...
const LMAS_DIR = process.env.LMAS_PROJECT_ROOT || process.cwd();
const STATUS_FILE = join(LMAS_DIR, '.lmas/dashboard/status.json');
const EVENTS_FILE = join(LMAS_DIR, '.lmas/dashboard/events.jsonl');
...
│                                                                              │
│  .lmas/dashboard/events.jsonl (SOLO ALTO NIVEL)                             │
│  ─────────────────────────────────────────────────                          │
...
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │                         StatusBar                            │    │   │
│  │  │  ● Conectado │ Sesión activa (5 min)  │ @architect (Architect)  │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │                                                                       │   │
│  └───────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
