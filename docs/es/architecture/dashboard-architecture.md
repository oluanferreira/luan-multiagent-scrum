# 🏛️ LMAS Dashboard - Arquitectura Completa
...
El LMAS Dashboard es una aplicación Next.js que proporciona una interfaz visual para monitorear y gestionar el sistema LMAS. Se comunica con el CLI/LMAS a través de archivos de estado en el sistema de archivos y Server-Sent Events (SSE).

### Principios Arquitectónicos

1. **CLI-First**: Dashboard es complementario al CLI, no sustituto
2. **File-Based Communication**: Estado a través de `.lmas/dashboard/status.json`
...
│  │  │  useLuan - MultiAgentes ScrumStatus │ │                │ │  (SSE + Polling)       ││   │
│  │  └────────────────┘ └────────────────┘ └────────────────────────┘│   │
│  └──────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│  ┌──────────────────────────────────────────────────────────────────┐   │
...
│   │   │   ├── status/route.ts       # Polling de estado LMAS
...
│   │   ├── use-lmas-status.ts        # Estado con SWR
...
| ---------------- | ----------------------------- | -------------------------------- |
| `story-store`    | `lmas-stories`            | `storyOrder` (orden de las columnas) |
| `ui-store`       | `lmas-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `lmas-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `lmas-dashboard-settings` | Todo el objeto `settings`         |
...
```typescript
// Retorna estado actual de LMAS
// Lee de: .lmas/dashboard/status.json

interface Luan - MultiAgentes ScrumStatus {
  version: string;
  updatedAt: string;
  connected: boolean;
...
// Eventos:
//   - status:update     → Luan - MultiAgentes ScrumStatus
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agente activado                         │
       │  ─────────────────────────────────────────▶  │
       │     Escribe .lmas/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
#### useLuan - MultiAgentes ScrumStatus

```typescript
// Polling basado en SWR del estado
const { status, isLoading, isConnected, statusError, mutate } = useLuan - MultiAgentes ScrumStatus({
...
_Documentación generada por @architect (Architect) - LMAS Core v2.0_
