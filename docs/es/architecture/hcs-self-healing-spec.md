...
**Autor:** @architect (Architect) vía @dev (Neo)
...
Este documento especifica las capacidades de auto-reparación del Sistema de Verificación de Salud de LMAS (HCS). La auto-reparación permite que el sistema corrija automáticamente ciertos problemas mientras mantiene la seguridad y el control del usuario.
...
| ---------------------- | ----------------------------------------------------- | ------------ |
| `recreate_config`      | Recrear `.lmas/config.yaml` faltante desde plantilla  | Sí           |
| `clear_cache`          | Limpiar archivos de caché obsoletos en `.lmas/cache/` | Sí           |
...
- id: PC-001
  description: '.lmas/config.yaml faltante'
  severity: CRITICAL
  tier: 1
...
✅ Auto-corregidos 3 problemas:
   • Recreado .lmas/config.yaml (respaldo: .lmas/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Agregado .lmas/cache/ a .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      - 'Abrir .lmas-core/development/tasks/deploy.md'
      - 'Encontrar línea: agent: legacy-dev'
      - 'Reemplazar con: agent: devops'
      - 'Verificar con: npx lmas task validate deploy'
...
Pasos para resolver:
  1. Abrir .lmas-core/development/tasks/deploy.md
  2. Encontrar línea: agent: legacy-dev
  3. Reemplazar con: agent: devops
  4. Verificar con: npx lmas task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config de usuario (fuera de .lmas/)
...
safeToAutoFix:
  paths:
    - '.lmas/**' # Archivos del workspace LMAS
    - '.lmas-core/**/*.yaml' # YAML del framework (con cuidado)
...
```json
{
  ...
  "rollbackCommand": "npx lmas health-check --rollback health-check-2025-12-30T10-30-00"
}
```
...
async function createBackup(action) {
  ...
  const backupDir = path.join('.lmas', 'backups', backupId);
...
npx lmas health-check --rollback health-check-2025-12-30T10-30-00

# Listar respaldos disponibles
npx lmas health-check --list-backups

# Revertir último respaldo
npx lmas health-check --rollback-last
```
...
class RecreateConfigHealer extends BaseHealer {
  ...
  async canHeal(issue) {
    return issue.id === 'PC-001' && !(await fs.pathExists('.lmas/config.yaml'));
  }

  async heal(issue, context) {
    const template = await fs.readFile('.lmas-core/templates/config-template.yaml');
    await fs.writeFile('.lmas/config.yaml', template);
    return { success: true, message: 'Config recreado desde plantilla' };
  }

  async verify(issue) {
    return await fs.pathExists('.lmas/config.yaml');
  }
}
```
...
```javascript
// .lmas/logs/self-healing.log
const healingLog = {
  append: async (entry) => {
    const logPath = '.lmas/logs/self-healing.log';
    const logEntry = {
