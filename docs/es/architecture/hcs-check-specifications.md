...
**Propósito:** Verificar que los archivos del framework LMAS están correctamente configurados y son consistentes.

### Verificaciones

| ID     | Nombre                      | Severidad | Nivel | Modo  | Timeout | Descripción                                         |
| ------ | --------------------------- | --------- | ----- | ----- | ------- | --------------------------------------------------- |
| PC-001 | Config existe               | CRITICAL  | 1     | quick | 100ms   | `.lmas/config.yaml` existe y es YAML válido         |
...
| PC-006 | Sin archivos huérfanos      | LOW       | 3     | full  | 5s      | Todos archivos en `.lmas-core/` están referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    const configPath = path.join(context.projectRoot, '.lmas', 'config.yaml');

    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.lmas/config.yaml no encontrado',
  ...
  async heal(context) {
    const templatePath = '.lmas-core/infrastructure/templates/core-config/config-template.yaml';
    const configPath = path.join(context.projectRoot, '.lmas', 'config.yaml');
...
const requiredSections = [
  'Project Overview', // o 'LMAS-FULLSTACK Development Rules'
...
```yaml
# .lmas-core/health-check/checks/ide-checks.yaml
checks:
  - id: IDE-VSCODE
    ...
    validation:
      type: json-schema
      schema: '.lmas-core/schemas/vscode-settings.json'
    autoFix:
      tier: 2
      action: 'create-from-template'
      template: '.lmas-core/infrastructure/templates/ide/vscode-settings.json'
...
Los usuarios pueden definir verificaciones específicas del proyecto en `.lmas/custom-checks.yaml`:

```yaml
# .lmas/custom-checks.yaml
version: 1.0
...
checks:
  ...
  - id: CUSTOM-001
    ...
    autoFix:
      action: create-from-template
      template: '.lmas-core/infrastructure/templates/project-docs/readme-template.md'
  ...
  - id: CUSTOM-005
    ...
    type: json-schema
    path: 'package.json'
    schema: '.lmas-core/schemas/package-json.json'
    severity: CRITICAL
    tier: 3
    mode: quick
```
