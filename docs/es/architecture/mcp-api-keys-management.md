...
Este documento describe las mejores prácticas para gestionar claves de API utilizadas por servidores MCP (Model Context Protocol) en LMAS. La gestión adecuada de claves de API es crítica para la seguridad e integridad operacional.

---

## Arquitectura MCP en LMAS

LMAS utiliza Docker MCP Toolkit como la infraestructura principal de MCP:
...
## Gobernanza MCP en LMAS

**IMPORTANTE:** Toda la gestión de infraestructura MCP es manejada EXCLUSIVAMENTE por el **Agente DevOps (@devops / Operator)**.
