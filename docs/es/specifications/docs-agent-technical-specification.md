...
**@docs (Ajax)** es un agente de IA especializado en crear **documentacion educativa** para LMAS. A diferencia de la documentacion tecnica (JSDoc/Typedoc), Ajax se enfoca en **tutoriales, guias de incorporacion, explicadores visuales y materiales de capacitacion para socios**.
...
- Aprobacion unanime de mesa redonda (Brad Frost, Marty Cagan, Paul Graham, Luan Ferreira)
...
whenToUse: |
  ...
  - Tutoriales explicando como usar funciones de LMAS
...
Entrada:
  ...
  - campo: agent_definition_path
    ...
    obligatorio: false
    example: "squads/lmas/agents/dev/agent.yaml"
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Usa estilo consistente (paleta de colores LMAS)"
      blocker: false
...
whenToUse: |
  ...
  - Nuevo socio se une al ecosistema LMAS
...
Checklist:
  ...
  acceptance-criteria:
    ...
    - check: "Enlaces a documentacion LMAS relevante"
      blocker: false
...
**Agente:** @[agent-name]
**Nombre:** [Nombre Completo (ej., Ajax, Neo)]
...
[Nombre del Agente] es el agente [rol] de LMAS, especializado en [experiencia principal].
...
**Notas de Plantilla:**
- Usar paleta de colores LMAS (Azul primario, Purpura secundario, Rosa terciario)
...
```bash
#!/bin/bash
# Hook de Auto-Actualizacion LMAS @docs
...
jobs:
  check-docs:
    ...
    steps:
      ...
      - name: Instalar LMAS CLI
        run: npm install -g @lmas/core
  ...
  qa-docs:
    ...
    steps:
      ...
      - name: Ejecutar QA de Documentacion
        id: qa
        run: |
          lmas agent invoke @docs --task qa-documentation \
            --paths "docs/**/*.md" \
...
- [ ] Crear `squads/lmas/agents/docs/agent.yaml`
...
- [ ] **Reconocimiento de Industria:** Docs de LMAS citados como "mejor en espacio de agentes IA"
...
Con la adicion de @docs (Ajax), LMAS ahora tiene **13 agentes nombrados**:

| # | Agente | Nombre | Rol | Arquetipo | Color |
|---|--------|--------|-----|-----------|-------|
| 1 | @dev | Neo | Constructor | Acuario | Cian |
| 2 | @qa | Oracle | Guardian | Virgo | Verde |
| 3 | @po | Keymaker | Equilibrador | Libra | Amarillo |
...
| 9 | @data-engineer | Dozer | Sabio | Geminis | Amarillo |
| 10 | @devops | Operator | Automatizador | Tauro | Verde |
| 11 | **@docs** | **Ajax** | **Estratega de Contenido** | **Aries** | **Azul** |
| 12 | ~~@security~~ | ~~Apex~~ | ~~Conductor~~ | ~~Leo~~ | ~~Rojo~~ (CANCELADO) |
| 13 | @lmas-master | Morpheus | Comandante | Aries | Cian |
...
**Especificacion Aprobada Por:** Architect (Architect)
