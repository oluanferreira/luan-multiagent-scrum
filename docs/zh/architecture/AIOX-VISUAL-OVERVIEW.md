...
Luan Ferreira 的 **Task-First** 方法论颠覆了传统范式:

| 传统范式 | Task-First (LMAS) |
|-----------------------|-------------------|
| 代理是中心 | **任务** 是中心 |
| "我使用哪个代理？" | "我需要执行什么任务？" |
...
flowchart LR
    ...
    subgraph TASKFIRST["Task-First (LMAS)"]
        direction TB
        TK1["已验证任务"] --> DEC1{"执行器?"}
        DEC1 --> E1["代理"]
        DEC1 --> E2["工作器"]
...
```mermaid
flowchart TB
    subgraph EXECUTORS["LMAS 的 4 种执行器"]
        direction TB
...
flowchart TB
    subgraph WS["公司动态工作空间"]
        ...
        subgraph FRAMEWORK[".lmas-core/ — 框架 (不可变)"]
            AGENTS_CORE["11 个核心代理<br/>dev, qa, architect, pm,<br/>po, sm, analyst, devops,<br/>data-engineer, ux, lmas-master"]
...
LMAS 的差异化在于小队 **不会取代** 人类 — 它们 **一起** 工作:
...
flowchart TB
    subgraph COLLAB["小队 + 人类协作"]
        ...
        subgraph PLANNING["阶段 1: 规划"]
            H_PM["人类 PM"] <-->|"完善需求"| AI_PM["@pm (Trinity)<br/>代理"]
            H_PM <-->|"验证架构"| AI_ARCH["@architect (Architect)<br/>代理"]
            AI_PM --> PRD["完整 PRD"]
            AI_ARCH --> ARCH_DOC["架构<br/>文档"]
        end

        subgraph EXECUTION["阶段 2: 开发"]
            AI_SM["@sm (Niobe)<br/>代理"] -->|"创建故事"| STORIES["超详细<br/>故事"]
            STORIES --> AI_DEV["@dev (Neo)<br/>代理"]
            AI_DEV -->|"实现"| CODE["代码"]
            CODE --> AI_QA["@qa (Oracle)<br/>代理"]
            H_DEV["人类开发者"] <-->|"结对编程"| AI_DEV
            H_DEV <-->|"代码审查"| AI_QA
        end

        subgraph REVIEW["阶段 3: 质量门禁"]
            AI_QA -->|"通过"| AI_DEVOPS["@devops (Operator)<br/>代理"]
            AI_QA -->|"失败"| AI_DEV
...
flowchart TD
    ...
    ANALYST --> PM["@pm (Trinity)<br/>创建 PRD"]
    PM --> ARCH["@architect (Architect)<br/>架构设计"]

    ARCH --> SM["@sm (Niobe)<br/>创建故事"]
    SM --> PO{"@po (Keymaker)<br/>验证故事?<br/>10点检查表"}

    PO -->|"通过 >= 7/10"| DEV["@dev (Neo)<br/>实现"]
    PO -->|"不通过 < 7/10"| SM

    DEV --> CODERABBIT{"CodeRabbit<br/>自愈<br/>最多 2 次迭代"}
    CODERABBIT -->|"严重"| DEV
    CODERABBIT -->|"通过"| QA{"@qa (Oracle)<br/>质量门禁<br/>7 项检查"}

    QA -->|"通过"| DEVOPS["@devops (Operator)<br/>推送 + PR"]
    QA -->|"顾虑"| DEVOPS
    QA -->|"失败"| DEV
...
flowchart LR
    ...
    style LMAS_WAY fill:#27ae60,color:#fff
```

### 宪法原则

LMAS 在具有不可协商原则的 **正式宪法** 下运行:
...
| 文档 | 内容 |
|-----------|----------|
| [EXECUTOR-DECISION-TREE.md](../../.lmas-core/docs/standards/EXECUTOR-DECISION-TREE.md) | 带示例和成本效益分析的详细决策树 |
| [SYNAPSE-FLOWCHARTS.md](SYNAPSE/SYNAPSE-FLOWCHARTS.md) | SYNAPSE 上下文引擎的 12 个流程图 |
| [Constitution](../../.lmas-core/constitution.md) | 框架的不可协商原则 |
| [User Guide](../../.lmas-core/user-guide.md) | LMAS 使用完整指南 |
