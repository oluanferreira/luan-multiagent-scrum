...
LMAS v4 采用**多仓库策略**以实现模块化开发、社区贡献和核心框架、扩展(squads)和专有组件之间的清晰分离。
...
主仓库包含所有项目依赖的基础 LMAS 框架。

### 内容

| 目录                       | 描述                                    |
| -------------------------- | --------------------------------------- |
| `.lmas-core/core/`         | 框架基础(配置、注册表、质量门)          |
| `.lmas-core/development/`  | 代理、任务、工作流定义                  |
| `.lmas-core/product/`      | 模板、检查表、PM 数据                   |
| `.lmas-core/infrastructure/` | 脚本、工具、集成                      |
| `docs/`                    | 框架文档                                |
...
| 包                   | 注册表 | 许可证     | 仓库        |
| -------------------- | ------ | ---------- | ----------- |
| `@lmas/core`         | npm    | MIT        | lmas-core   |
| `@lmas/squad-etl`    | npm    | MIT        | lmas-squads |
| `@lmas/squad-creator`| npm    | MIT        | lmas-squads |
| `@lmas/squad-mmos`   | npm    | MIT        | lmas-squads |
| `@lmas/mcp-presets`  | npm    | Apache 2.0 | mcp-ecosystem |
