...
**迁移通知:** 本文档将在 2026 Q2 迁移到 `SynkraAI/lmas-core` 仓库 (参见决策 005)
...
LMAS 构建在现代 JavaScript/TypeScript 和 Node.js 运行时上，优化用于开发具有交互式 UX 和代理编排功能的跨平台 CLI。
...
LMAS 与 Model Context Protocol (MCP) 服务器集成:
...
```bash
# LMAS 配置
LMAS_DEBUG=true                    # 启用调试日志
LMAS_CONFIG_PATH=/custom/path      # 自定义配置位置
LMAS_YOLO_MODE=true               # 强制 yolo 模式
...
| 1.1  | 2025-12-14 | 更新对 SynkraAI/lmas-core 的迁移通知、semantic-release 到 v25.0.2 [故事 6.10] | Neo (dev)        |

---

_这是 LMAS 框架的官方模式。所有技术选择应与本栈保持一致。_
