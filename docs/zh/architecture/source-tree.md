...
**迁移通知**: 此文档将在 Q2 2026 迁移到 `LMAS/lmas-core` 存储库（参见决策 005）
...
- [概述](#概述)
- [当前结构 (lmas-core Brownfield)](#当前结构-lmas-core-brownfield)
...
## 当前结构 (lmas-core Brownfield)

```
lmas-core/                             # 根目录 (brownfield 项目)
├── .lmas-core/                        # 框架核心 (可移植)
│   ├── core/                          # 框架必要组件 (v4)
│   │   ├── config/                    # 配置系统
│   │   ├── data/                      # 核心知识库
...
│
├── bin/                               # CLI 可执行文件
│   ├── @lmas/lmas-core.js              # 主 CLI 入口点
│   └── lmas-minimal.js                # 最小 CLI
│
├── tools/                             # 构建和实用工具
│   ├── cli.js                         # CLI 构建器
...
Squad 是添加专用功能到 LMAS 的模块化扩展。不同于已弃用的 Squad，Squad 遵循标准化模板结构。
...
### 存储库 2: LMAS/squads (MIT)

```
squads/
├── verified/                          # LMAS 审核的 Squad
│   ├── github-devops/
│   ├── db-sage/
│   └── coderabbit-workflow/
│
...
### 存储库 3: LMAS/mcp-ecosystem (Apache 2.0)
...
### 存储库 4: LMAS/certified-partners (私有)
...
```yaml
# 我在创建新代理:
位置: .lmas-core/agents/{agent-name}.md
示例: .lmas-core/agents/security-expert.md

# 我在创建新任务:
位置: .lmas-core/tasks/{task-name}.md
示例: .lmas-core/tasks/deploy-to-production.md

# 我在创建新工作流:
位置: .lmas-core/workflows/{workflow-name}.yaml
示例: .lmas-core/workflows/continuous-deployment.yaml

# 我在创建新模板:
位置: .lmas-core/product/templates/{template-name}-tmpl.{yaml|md}
示例: .lmas-core/product/templates/deployment-plan-tmpl.yaml

# 我在编写故事:
位置: docs/stories/{epic-context}/{story-file}.md
示例: docs/stories/lmas migration/story-6.1.2.6.md
...
# 我在创建实用脚本:
位置: .lmas-core/utils/{utility-name}.js
示例: .lmas-core/utils/performance-monitor.js
...
| 1.1  | 2025-12-14 | 更新 org 为 LMAS，用 Squad 系统替换 Squads [Story 6.10]    | Neo (dev)       |

---

_这是 LMAS 框架的官方模式。所有文件放置应遵循此结构。_
