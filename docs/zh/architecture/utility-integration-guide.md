...
**目的:** 为在 LMAS 框架中集成工具脚本定义模式

---

## 什么是工具集成?

**定义:** 工具集成是使孤立工具脚本**可发现、文档化和可用**的过程，位于 LMAS 框架内。
...
**修改的文件:**
- `.lmas-core/agents/{agent}.yaml` (添加到 dependencies.utils)
- `.lmas-core/core-config.yaml` (如必要则注册)
- `.lmas-core/utils/README.md` (记录工具)
...
**修改的文件:**
- `.lmas-core/tasks/{task}.md` (添加 execution.utils)
- `.lmas-core/agents/{agent}.yaml` (添加任务到 executes 列表)
- `.lmas-core/core-config.yaml` (如必要则注册)
- `.lmas-core/utils/README.md` (记录工具)
...
**修改的文件:**
- `.lmas-core/core-config.yaml` (在框架中注册)
- `.lmas-core/utils/README.md` (记录为框架工具)
- 框架文档 (如适用)
...
**目的:** 分析、改进、验证代码
**模式:** 代理辅助工具 (代理 dev、qa)
**示例:** lmas-validator、code-quality-improver、coverage-analyzer
