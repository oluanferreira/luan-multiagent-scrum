...
**创建:** 2025-12-30
**故事:** HCS-1 调查
**作者:** @architect (Architect) via @dev (Neo)

---

## 执行摘要

本文档指定了 LMAS 健康检查系统 (HCS) 的自动恢复能力。自动恢复允许系统自动纠正某些问题,同时保持安全性和用户控制。
...
| 操作                | 描述                                      | 备份 |
| ------------------- | ----------------------------------------- | ---- |
| `recreate_config`   | 从模板重新创建缺失的 `.lmas/config.yaml` | 是   |
| `clear_cache`       | 清除 `.lmas/cache/` 中的过期缓存文件     | 是   |
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # 用户配置(out of .lmas/)
    - '.env*' # 环境文件
    - '**/secrets/**' # 密钥
    - '**/credentials*' # 凭证
    - '.git/**' # Git 内部
