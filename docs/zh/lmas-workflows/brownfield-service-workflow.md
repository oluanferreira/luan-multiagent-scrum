...
graph LR
    ...
    subgraph "管理代理"
        PO["@po<br/>Keymaker<br/>产品所有者"]
        SM["@sm<br/>River<br/>敏捷主管"]
    end

    subgraph "执行代理"
        Dev["@dev<br/>Neo<br/>全栈开发人员"]
        QA["@qa<br/>Oracle<br/>测试架构师"]
    end
...
| @po | Keymaker | 产品所有者 | 工件验证、文档分片、回顾 |
| @sm | River | 敏捷主管 | 故事创建 |
| @analyst | Atlas | 业务分析师 | 故事草稿的可选审查 |
| @dev | Neo | 全栈开发人员 | 故事实现、修复反馈 |
| @qa | Oracle | 测试架构师 | 实现审查、质量关卡 |
...
| 问题 | 首次联系 | 升级到 |
|------|--------|--------|
| PRD不完整 | @pm (Trinity) | @po (Keymaker) |
| 架构不一致 | @architect (Architect) | @pm (Trinity) |
| 故事模糊 | @sm (Niobe) | @po (Keymaker) |
| 实现有错误 | @dev (Neo) | @qa (Oracle) |
| 质量关卡失败 | @qa (Oracle) | @po (Keymaker) |
| 集成中断 | @architect (Architect) | @devops (Operator) |
...
| 文件 | 描述 | 路径 |
|------|------|------|
| 工作流定义 | 工作流的YAML定义 | `.lmas-core/development/workflows/brownfield-service.yaml` |
| PRD模板 | 棕地PRD的模板 | `.lmas-core/development/templates/brownfield-prd-tmpl.yaml` |
| 架构模板 | 架构的模板 | `.lmas-core/development/templates/brownfield-architecture-tmpl.yaml` |
| 故事模板 | 故事的模板 | `.lmas-core/development/templates/story-tmpl.yaml` |
| PO主清单 | 验证清单 | `.lmas-core/development/checklists/po-master-checklist.md` |
| 故事草稿清单 | 故事清单 | `.lmas-core/development/checklists/story-draft-checklist.md` |
| 故事DoD清单 | 完成定义 | `.lmas-core/development/checklists/story-dod-checklist.md` |
...
- [LMAS知识库](../../../.lmas-core/data/lmas-kb.md) - 框架知识库
- [技术偏好](../../../.lmas-core/development/data/technical-preferences.md) - 项目技术偏好
- [IDE开发工作流](../../../.lmas-core/data/lmas-kb.md#IDE-Development-Workflow) - IDE开发流程
...
*从workflow `brownfield-service.yaml` 生成 - LMAS框架 v2.2*
