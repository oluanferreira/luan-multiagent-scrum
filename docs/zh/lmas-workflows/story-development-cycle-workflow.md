...
**故事开发循环**是 LMAS 中央工作流，用于故事开发。它自动化从创建到交付的完整流程，整合质量网关，遵循序列: **创建 -> 验证 -> 实现 -> QA 审查**。
...
Scrum Master (Niobe) 使用 PRD 共享或项目文档作为来源识别和创建下一个故事。
...
产品所有者 (Keymaker) 使用严格的 10 点检查清单验证已创建的故事，确保故事已准备好实现。
...
Dev Agent (Neo) 实现已验证的故事，遵循验收标准和定义的任务。包括 CodeRabbit 自我修复循环以确保代码质量。
...
QA Agent (Oracle) 执行最终审查和质量网关，验证代码、测试和验收标准的遵循。
...
1. **core-config.yaml** - 必需的 LMAS 配置文件
   - `devStoryLocation` - 故事位置
   - `prd.*` - PRD 配置
   - `architecture.*` - 架构配置
   - `qa.qaLocation` - QA 工件位置

2. **故事模板** - `.lmas-core/development/templates/` 中的 `story-tmpl.yaml`
...
| 故事模板 | YAML | `.lmas-core/development/templates/` | 标准模板 |
...
| 文件 | 路径 |
|------|------|
| 工作流定义 | `.lmas-core/development/workflows/story-development-cycle.yaml` |
| SM 代理 | `.lmas-core/development/agents/sm.md` |
| PO 代理 | `.lmas-core/development/agents/po.md` |
| 开发代理 | `.lmas-core/development/agents/dev.md` |
| QA 代理 | `.lmas-core/development/agents/qa.md` |
| 创建故事任务 | `.lmas-core/development/tasks/create-next-story.md` |
| 验证故事任务 | `.lmas-core/development/tasks/validate-next-story.md` |
| 开发故事任务 | `.lmas-core/development/tasks/dev-develop-story.md` |
| QA 网关任务 | `.lmas-core/development/tasks/qa-gate.md` |
| 故事模板 | `.lmas-core/development/templates/story-tmpl.yaml` |
