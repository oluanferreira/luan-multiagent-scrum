...
欢迎使用 Synkra LMAS。本指南针对 10 分钟内实现首次价值进行了优化。
...
- Gemini CLI：`/lmas-menu` 然后 `/lmas-<agent>`
- Codex CLI：`/skills` 然后 `lmas-<agent-id>`
...
LMAS 配置位于 `.lmas-core/core/config/`。安装程序处理初始设置。要验证您的安装：
...
LMAS 代理通过您的 IDE 激活。激活后，代理响应以 `*` 为前缀的命令：
...
| 代理 | 名称 | 专注领域 |
| --- | --- | --- |
| `@dev` | Neo | 代码实现、bug 修复、重构 |
| `@qa` | Oracle | 测试、质量门控、代码审查 |
| `@architect` | Architect | 系统设计、技术决策 |
| `@pm` | Bob | PRD、战略、路线图 |
| `@po` | Keymaker | 待办事项、故事验证、优先级排序 |
| `@sm` | Niobe | 故事创建、冲刺计划 |
| `@analyst` | Link | 研究、竞争分析 |
| `@data-engineer` | Dozer | 数据库设计、迁移 |
| `@ux-design-expert` | Switch | UI/UX 设计、无障碍访问 |
| `@devops` | Operator | Git 操作、CI/CD、部署 |
...
并非所有 IDE 都同等支持 LMAS 功能。请参阅 [`docs/ide-integration.md`](./ide-integration.md) 中的完整比较。
...
| Gemini CLI | 可用 | `/lmas-menu` 然后 `/lmas-<agent>` |
| Codex CLI | 有限 | `/skills` 然后 `lmas-<agent-id>` |
...
当您首次在现有项目中激活 LMAS 代理时：

1. **检测**：LMAS 检测到代码但没有 LMAS 文档
...
- **非破坏性**：LMAS 创建文件，从不覆盖现有文件
- **回滚**：`git checkout HEAD~1 -- .` 恢复 LMAS 之前的状态
- **配置保留**：您的 `.eslintrc`、`tsconfig.json` 等保持不变

### 资源

- **[棕地项目工作指南](.lmas-core/working-in-the-brownfield.md)** - 完整的棕地项目文档
- **[兼容性检查清单](.lmas-core/development/checklists/brownfield-compatibility-checklist.md)** - 迁移前/后检查
- **[风险报告模板](.lmas-core/product/templates/brownfield-risk-report-tmpl.yaml)** - 分阶段风险评估
...
所有 LMAS 开发都遵循 `docs/stories/` 中的故事。每个故事包含：
- 带复选框的验收标准
- 映射到特定 AC 的任务
- CodeRabbit 集成用于自动审查
- 质量门控分配
...
小队将 LMAS 扩展到软件开发之外的任何领域。请参阅[小队指南](./guides/squads-guide.md)。
...
- **[小队指南](./guides/squads-guide.md)** - 将 LMAS 扩展到任何领域
- **[故障排除](./troubleshooting.md)** - 常见问题和解决方案

---

_Synkra LMAS 入门指南 v4.2.11_
