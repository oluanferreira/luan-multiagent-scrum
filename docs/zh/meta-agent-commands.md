...
```bash
LMAS_AI_PROVIDER=openai          # AI 提供商
LMAS_AI_MODEL=gpt-4             # AI 模型
LMAS_LOG_LEVEL=debug            # 日志级别
LMAS_TELEMETRY=disabled         # 遥测设置
LMAS_TIMEOUT=30000             # 命令超时（毫秒）
LMAS_MEMORY_CACHE=true         # 内存缓存
```
