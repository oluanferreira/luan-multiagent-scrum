...
LMAS 使用**模块化架构**，具有清晰的关注点分离：

1. **框架核心** (`.lmas-core/`) - 按领域组织的可移植框架组件
...
│
├── data/                              # 共享数据
│   ├── lmas-kb.md                     # LMAS 知识库（@lmas-master，延迟加载）
...
### 共享数据文件 (.lmas-core/data/)

| 文件 | 所有者 | 填充规则 | 更新触发器 | 使用者 |
|------|--------|----------|------------|--------|
| `lmas-kb.md` | @lmas-master | 框架重大变更时更新 | 手动编辑 | @lmas-master (延迟) |
...
Squads 是为 LMAS 添加专门功能的模块化扩展。与已弃用的 Squads 不同，Squads 遵循标准化的模板结构。
...
ADE 通过 `.lmas-core/core-config.yaml` 配置：
...
_这是官方 LMAS 框架标准。所有文件放置必须遵循此结构。_
