...
Synkra LMAS 完整 API 参考 - AI 编排全栈开发系统。
...
LMAS 通过两个主要机制提供统一 API 来与专业 AI 代理交互：
...
| 代理 ID          | 名称   | 原型    | 主要责任                           |
| ---------------- | ------ | ------- | ---------------------------------- |
| `@dev`           | Neo    | Builder | 代码实现、调试、测试               |
| `@qa`            | Oracle  | Guardian| 质量保证、代码审查、测试           |
| `@architect`     | Aria   | Visionary| 系统架构、API 设计                |
| `@pm`            | Morgan | Strategist| 产品需求、史诗、战略            |
| `@po`            | Keymaker    | Champion| 待办事项管理、验收标准             |
| `@sm`            | River  | Facilitator| 冲刺规划、故事创建             |
| `@analyst`       | Atlas  | Explorer| 市场研究、竞争分析                 |
| `@data-engineer` | Dozer   | Architect| 数据库架构、迁移、查询           |
| `@devops`        | Operator   | Optimizer| CI/CD、部署、git 操作             |
| `@ux-expert`     | Uma    | Creator | UI/UX 设计、线框图                 |
| `@lmas-master`   | Morpheus  | Orchestrator| 框架编排、元操作             |

### 激活行为

激活代理时：

1. 从 `.lmas-core/development/agents/{id}.md` 加载代理定义文件
2. 采用角色（语气、词汇、问候）
3. 基于会话类型显示上下文问候
4. 代理停止并等待用户输入
...
| 类别                 | 描述                         | 解决方案                           |
| -------------------- | ---------------------------- | ---------------------------------- |
| `AGENT_NOT_FOUND`    | 缺少代理定义                 | 检查 `.lmas-core/development/agents/` |
