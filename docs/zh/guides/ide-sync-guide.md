...
在多个IDE配置中同步LMAS代理、任务、工作流和检查清单。

## 概述

`*command` 任务自动化LMAS组件到所有配置的IDE目录（`.claude/`、`.cursor/`、`.gemini/` 等）的同步，省去手动复制操作。
