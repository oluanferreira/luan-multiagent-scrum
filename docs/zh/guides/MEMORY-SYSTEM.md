...
LMAS 的记忆系统在**两个独立的层**上运行，它们共存但**彼此不通信**：

| 层 | 管理者 | 范围 |
|--------|---------------|--------|
| **第一层: Claude Code 原生** | Claude Code CLI | 自动记忆, CLAUDE.md, 会话转录 |
| **第二层: LMAS 框架** | `.lmas-core/` 中的 JS 脚本 | Gotchas, 会话状态, 上下文快照, Timeline |
...
flowchart TB
    subgraph CLAUDE_NATIVE["第一层: Claude Code 原生"]
        ...
        CLAUDE_MD_P["CLAUDE.md 项目<br/>lmas-core/.claude/CLAUDE.md"]
    ...
    subgraph LMAS_FRAMEWORK["第二层: LMAS 框架"]
        direction TB
        GOTCHAS["Gotchas 记忆<br/>.lmas-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["上下文快照<br/>.lmas-core/core/memory/context-snapshot.js"]
        FILE_EVO["文件演化追踪器<br/>.lmas-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.lmas-core/core/memory/timeline-manager.js"]
        CTX_LOADER["会话上下文加载器<br/>.lmas-core/core/session/context-loader.js"]
        CTX_DETECT["上下文检测器<br/>.lmas-core/core/session/context-detector.js"]
        SESS_STATE["会话状态<br/>.lmas-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.lmas-core/hooks/gemini/"]
        HOOKS_UNI["统一 Hooks<br/>.lmas-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_LMAS["LMAS 存储 (.lmas/)"]
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["记忆层 (.lmas-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["会话层 (.lmas-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["编排 (.lmas-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["激活管道 (.lmas-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks 系统 (.lmas-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["自动加载 — 固定顺序"]
        ...
        LOAD_PROJECT["3. lmas-core/.claude/CLAUDE.md<br/>项目级指令"]
        LOAD_RULES["4. lmas-core/.claude/rules/*.md<br/>5 个详细规则文件"]
...
flowchart LR
    ...
    NOTE["注意: 只有定义在<br/>.claude/agents/ 中的代理<br/>使用此记忆。<br/>LMAS 代理 (.lmas-core/<br/>development/agents/)<br/>不使用。"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("持久化到:<br/>.lmas/gotchas.json<br/>.lmas/gotchas.md<br/>.lmas/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("持久化到:<br/>.lmas/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("持久化到:<br/>.lmas/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — 统一门面"]
        ...
        T4[("持久化到:<br/>.lmas/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD2{"有对话<br/>历史?"}
        CD3["分析最近命令"]
        CD4["读取 .lmas/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("持久化到:<br/>.lmas/session-state.json<br/>TTL: 1 小时")]
...
sequenceDiagram
    ...
    Note over CC: 没有 LMAS 记忆<br/>自动加载<br/>到 system prompt。<br/>仅会话上下文<br/>出现在 greeting 中。
```
...
flowchart TD
    ...
    subgraph SAVES["保存的内容"]
        S1[".lmas/session-state.json<br/>当前代理, 命令, 工作流"]
        S2[".lmas/error-tracking.json<br/>错误发生"]
        S3[".lmas/gotchas.json<br/>新自动捕获的 gotcha"]
        S4[".lmas/gotchas.json<br/>添加的手动 gotcha"]
        S5["docs/stories/.session-state.yaml<br/>更新的阶段"]
        S6["docs/stories/.session-state.yaml<br/>Story 标记完成"]
        S7[".lmas/snapshots/{id}.json<br/>捕获的上下文"]
        S8[".lmas/file-evolution/<br/>记录的演化"]
    end
...
sequenceDiagram
    ...
    CL->>FS: 读取 .lmas/session-state.json
    FS-->>CL: 当前状态 (或空)
    ...
    CL->>FS: 写入 .lmas/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["自动捕获流程"]
        ...
        TRACK --> READ_ET["读取 .lmas/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["保存 .lmas/gotchas.json"]
        SAVE_JSON --> SAVE_MD["生成 .lmas/gotchas.md"]
...
flowchart TD
    subgraph CAPTURE["捕获 — context-snapshot.js"]
        ...
        C4 --> C5["保存 .lmas/snapshots/{id}.json"]
        C5 --> C6{"autoCleanup?"}
        C6 -->|"是"| C7["移除快照:<br/>- > 50 个存在<br/>- > 7 天"]
    end

    subgraph RESTORE["恢复 — 恢复"]
        R1(["snapshot.restore(id)"]) --> R2["读取 .lmas/snapshots/{id}.json"]
        R2 --> R3["返回完整上下文"]
        R3 --> R4["代理可以从<br/>上次位置恢复"]
    end

    subgraph LIST["列出 — 查询"]
        L1(["snapshot.list()"]) --> L2["列出 .lmas/snapshots/ 中所有"]
        L2 --> L3["按时间戳排序"]
        L3 --> L4["返回元数据数组"]
    end
...
| 文件 | 模块 | Story/Epic | 功能 |
|---------|--------|------------|--------|
| `.lmas-core/core/memory/gotchas-memory.js` | 记忆 | Epic 9, Story 9.4 | 自动捕获重复错误, 手动 gotchas, 注入任务 |
| `.lmas-core/core/memory/context-snapshot.js` | 记忆 | Story 12.6 | 捕获和恢复开发上下文 |
| `.lmas-core/core/memory/file-evolution-tracker.js` | 记忆 | Gap impl | 追踪文件演化, 检测漂移 |
| `.lmas-core/core/memory/timeline-manager.js` | 记忆 | Gap impl | 跨会话统一 timeline 门面 |
| `.lmas-core/core/session/context-loader.js` | 会话 | Story 2.2, 6.1.2.5 | 代理间连续性, 上下文交接 |
| `.lmas-core/core/session/context-detector.js` | 会话 | Story 2.2 | 混合会话类型检测 (new/existing/workflow) |
| `.lmas-core/core/orchestration/session-state.js` | 编排 | Story 11.5 | Epic/story 持久状态, 崩溃恢复 |
| `.lmas-core/core/orchestration/context-manager.js` | 编排 | Legacy | 阶段间工作流状态 (迁移到 session-state) |
| `.lmas-core/core/elicitation/session-manager.js` | Elicitation | — | Elicitation 会话保存/加载 |

### 激活脚本 (消费记忆)

| 文件 | 功能 |
|---------|--------|
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | 主编排器 — 在 Tier 3 加载会话 |
| `.lmas-core/development/scripts/greeting-builder.js` | 构建带会话/记忆上下文的 greeting |

### Hooks (会话持久化)

| 文件 | CLI | 功能 |
|---------|-----|--------|
| `.lmas-core/hooks/gemini/session-start.js` | Gemini | 会话开始时加载 LMAS 上下文 |
| `.lmas-core/hooks/gemini/session-end.js` | Gemini | 将会话摘要持久化到 `.lmas/sessions/` |
| `.lmas-core/hooks/gemini/before-agent.js` | Gemini | 代理前预处理 |
| `.lmas-core/hooks/gemini/before-tool.js` | 两者 | 工具前预处理 |
| `.lmas-core/hooks/gemini/after-tool.js` | 两者 | 工具后后处理 |
| `.lmas-core/hooks/unified/hook-interface.js` | 两者 | UnifiedHook 基类 + EVENT_MAPPING |
| `.lmas-core/hooks/unified/hook-registry.js` | 两者 | hooks 中央注册表 |
| `.lmas-core/hooks/unified/index.js` | 两者 | 统一系统入口点 |
...
| `lmas-core/.claude/CLAUDE.md` | 项目指令 (始终加载) |
| `lmas-core/.claude/rules/*.md` | 5 个规则文件 (始终加载) |
...
LMAS 模块 (gotchas, snapshots, timeline) 在会话结束时**不会自动 flush**。如果进程突然终止，内存中的数据可能丢失。
...
Claude 自动记忆 (`MEMORY.md`) 和 LMAS 记忆 (`.lmas/`) 从不同步。LMAS 捕获的 Gotchas 不会出现在 MEMORY.md 中，反之亦然。
...
| Gotchas 记忆脚本 | `.lmas-core/core/memory/gotchas-memory.js` |
| 上下文快照脚本 | `.lmas-core/core/memory/context-snapshot.js` |
| 文件演化追踪器 | `.lmas-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.lmas-core/core/memory/timeline-manager.js` |
| 会话上下文加载器 | `.lmas-core/core/session/context-loader.js` |
| 上下文检测器 | `.lmas-core/core/session/context-detector.js` |
| 会话状态 | `.lmas-core/core/orchestration/session-state.js` |
| 统一 Hook 接口 | `.lmas-core/hooks/unified/hook-interface.js` |
| Hook 注册表 | `.lmas-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.lmas-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.lmas-core/hooks/gemini/session-end.js` |
| Core 配置 | `.lmas-core/core-config.yaml` |
