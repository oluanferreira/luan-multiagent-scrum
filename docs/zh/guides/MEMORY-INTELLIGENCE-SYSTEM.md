...
> **架构:** MIS 遵循 LMAS 的 **Open Core** 模型。所有记忆智能都位于私有仓库 `lmas-pro`（子模块 `pro/`）中。`lmas-core`（开源）仅提供扩展点，并且在没有智能记忆的情况下 100% 正常运行 — 与现在完全一样。
...
当前记忆系统在**两个断开的层**（Claude Code 原生 + LMAS 框架）上运行，存在关键缺口：
...
| 层 | 功能 | 仓库 | Story |
|--------|--------|-------------|-------|
| **清理** | 移除死代码，准备基础 | **lmas-core** | MIS-2 |
| **捕获** | 通过 hooks 捕获会话知识 | **lmas-pro** | MIS-3 |
| **存储** | 使用带 YAML frontmatter 的 Markdown 存储 | **lmas-pro** | MIS-3, MIS-4 |
| **检索** | 按相关性渐进式检索 | **lmas-pro** + **lmas-core** 扩展点 | MIS-4, MIS-6 |
| **演化** | 自动学习和演化规则 | **lmas-pro** | MIS-5, MIS-7 |

### 关键原则 (vs 当前状态)

| 方面 | 当前状态 | 使用 MIS (lmas-pro 激活) | 没有 MIS (仅 lmas-core) |
|---------|-------------|--------------------------|--------------------------|
...
> **lmas-core 在没有 MIS 的情况下 100% 正常运行** — 与现在的运行方式完全相同。当 `lmas-pro` 存在时（子模块 `pro/`），MIS 通过扩展点自动连接。无需手动配置。

### Open Core 模型

```mermaid
flowchart LR
    subgraph CORE["lmas-core (开源, MIT)"]
        direction TB
    ...
    subgraph PRO["lmas-pro (私有, 子模块 pro/)"]
        direction TB
...
lmas-core 已有模块 `bin/utils/pro-detector.js` (Story PRO-5):
...
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | 修改 | 在 Tier 2 添加 `if (isProAvailable())` 用于记忆加载 |
| `.lmas-core/development/scripts/greeting-builder.js` | 修改 | 如果 pro 可用，在 greeting 中添加记忆统计 |
| `.lmas-core/core/memory/gotchas-memory.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
| `.lmas-core/core/session/context-loader.js` | 保留 | 路径修复 (MIS-2)，独立运行 |
...
Claude Code 的 hooks 在 `.claude/settings.json` (lmas-core) 中配置。指向的脚本位于 `pro/`:
...
```javascript
// .lmas-core/hooks/pro-hook-runner.js (在 core 中, ~20 行)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["lmas-core (开源)"]
        direction TB
        EXT["管道中的<br/>扩展点"]
    ...
    subgraph PRO_BOUNDARY["lmas-pro (私有 — pro/)"]
...
> **仓库:** `lmas-pro` (pro/memory/)
> **core 中的扩展点:** `.lmas-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (lmas-core — 已提交)
{
  "hooks": {
    "PreCompact": [
      {
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["lmas-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("持久化到<br/>.lmas/memories/")]
    end
...
> **仓库:** `lmas-pro` (pro/memory/) 管理存储
> **存储位置:** `.lmas/memories/` (gitignored, 由 pro 创建)
...
```
.lmas/memories/                          # 由 pro/memory/ 创建和管理
  shared/                                # 跨代理记忆
    durable/                             # 永久知识
...
> **仓库:** `lmas-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **仓库:** `lmas-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
> **仓库:** `lmas-pro` (pro/memory/agent-memory-api.js)
...
每个代理访问：
1. 自己的私有记忆 (`.lmas/memories/{agent}/`)
2. 共享记忆 (`.lmas/memories/shared/`)
3. **绝不** 其他代理的私有记忆
...
> **100% 在 lmas-pro 中** — core 中没有自学习组件
...
**没有 pro:** 仅 `[CORE]` 和 `[原生]` 存在。`[PRO]` (.lmas/memories/) 永远不会创建。
...
| `.lmas-core/hooks/pro-hook-runner.js` | **新建** | MIS-3 | 包装器: 检查 pro, 委托 hook (~20 行) |
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | **修改** | MIS-6 | 在 Tier 2 添加 `if (isProAvailable())` |
| `.lmas-core/development/scripts/greeting-builder.js` | **修改** | MIS-6 | 如果 pro 可用，在 greeting 中显示记忆统计 |
| `.lmas-core/core/memory/gotchas-memory.js` | **路径修复** | MIS-2 | 修正 `.lmas/error-tracking.json` |
| `.lmas-core/core/session/context-loader.js` | **路径修复** | MIS-2 | 修正 `.lmas/session-state.json` |
...
### 删除的文件 (MIS-2, lmas-core)

| 文件 | 行数 | 原因 |
|---------|--------|-------|
| `.lmas-core/core/memory/timeline-manager.js` | 746 | 0 个消费者 (孤立) |
| `.lmas-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 个消费者 (孤立) |
| `.lmas-core/core/memory/context-snapshot.js` | 648 | 0 个消费者 (孤立) |
| `.lmas-core/elicitation/session-manager.js` | — | 死代码 |
| `.lmas/compound-analysis/` | ~14K | 0 个读取者 (孤立数据) |

---

## 差异对比: 当前状态 vs MIS

| 方面 | 当前 lmas-core | lmas-core + lmas-pro (MIS) | 没有 pro 的 lmas-core |
|---------|---------------|---------------------------|------------------|
...
| Story | 标题 | 仓库 | 依赖 | 工时 |
|-------|--------|-------------|------------|-------|
| MIS-1 | 调研与架构设计 | docs (lmas-core) | — | 12h (完成) |
| MIS-2 | 死代码清理与路径修复 | **lmas-core** | MIS-1 | 4h |
| MIS-3 | 会话摘要 (PreCompact Hook) | **lmas-core** (hook runner) + **lmas-pro** (digest) | MIS-1 | 14h |
| MIS-4 | 渐进式记忆检索 | **lmas-pro** | MIS-3 | 16h |
| MIS-5 | 自学习引擎 | **lmas-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | 管道集成与代理记忆 API | **lmas-core** (扩展点) + **lmas-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md 与规则自动演化 | **lmas-pro** | MIS-5 | 8h |
| **总计** | | | | **~78h** |
...
| Epic PRO 架构 | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-lmas-pro-architecture/EPIC-PRO-INDEX.md) |
...
*LMAS 记忆智能系统 — 架构愿景 v4.0.4 (Core/Pro 分离)*
*Epic MIS 完成后的目标状态 (7 个 stories, ~78 小时)*
*lmas-core: 扩展点 + 死代码清理*
*lmas-pro: 所有记忆智能 (捕获, 存储, 检索, 演化)*
