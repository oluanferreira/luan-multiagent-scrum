...
上下文问候系统是一项 UX 改进，使 LMAS 代理的问候变得智能和自适应，根据会话上下文显示相关信息和命令。
...
5. **工作流模式** (`.lmas-core/data/workflow-patterns.yaml`)
   - 定义了 10 个常见工作流
   - 带下一步建议的状态转换
   - 根据实际项目使用验证的模式
...
### 核心脚本
- `.lmas-core/core/session/context-detector.js` - 会话类型检测
- `.lmas-core/infrastructure/scripts/git-config-detector.js` - Git 配置检测
- `.lmas-core/development/scripts/greeting-builder.js` - 问候构建
- `.lmas-core/development/scripts/workflow-navigator.js` - 工作流导航
- `.lmas-core/development/scripts/agent-exit-hooks.js` - 退出钩子 (用于持久化)

### 数据文件
- `.lmas-core/data/workflow-patterns.yaml` - 工作流定义
...
### 配置
- `.lmas-core/core-config.yaml` - 全局配置 (git + agentIdentity 部分)

### 代理 (已更新)
- `.lmas-core/agents/dev.md` - ✅ 命令可见性元数据
- `.lmas-core/agents/po.md` - ✅ 命令可见性元数据
- `.lmas-core/agents/*.md` - ⏳ 剩余 9 个代理 (待更新)
...
**Q: 如何现在测试?**
A: 使用测试脚本: `node .lmas-core/development/scripts/test-greeting-system.js`
