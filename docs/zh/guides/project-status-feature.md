...
项目状态功能在任何LMAS Agent激活时自动显示你当前的工作上下文。这包括：
...
3. 创建 `.lmas/project-status.yaml` 缓存文件
...
1. 编辑 `.lmas-core/core-config.yaml`：
   ```yaml
   projectStatus:
     enabled: true
...
projectStatus:
  ...
  statusFile: .lmas/project-status.yaml  # 缓存文件位置
...
1. **检查缓存** - 查找 `.lmas/project-status.yaml`
2. **验证TTL** - 缓存 < 60秒？
...
3. `.lmas-core/infrastructure/scripts/project-status-loader.js` 存在吗？
4. Agent激活输出中有错误吗？
...
**Q: 缓存在agent之间共享吗?**
A: 是的，所有agent使用相同的缓存文件 (`.lmas/project-status.yaml`)。
...
- **配置:** `.lmas-core/core-config.yaml` (projectStatus部分)
- **脚本:** `.lmas-core/infrastructure/scripts/project-status-loader.js`
- **Init Task:** `.lmas-core/tasks/init-project-status.md`
- **测试:** `.lmas-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
