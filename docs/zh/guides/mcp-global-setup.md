...
MCP全局系统允许你配置一次MCP服务器并在所有LMAS项目之间共享。这消除了在每个项目中配置相同服务器的需要。
...
- `~/.lmas/mcp/servers/` - 个别服务器配置
- `~/.lmas/mcp/cache/` - 响应缓存
- `~/.lmas/credentials/` - 安全凭据存储
...
位置: ~/.lmas/mcp/global-config.json
状态:   ✓ 已配置

服务器: 0个已配置
缓存:   为空

运行 'lmas mcp add <server>' 来添加服务器。
```
...
凭据存储在 `~/.lmas/credentials/` 中，包含 `.gitignore` 以防止意外提交。
...
| 目录存在 | 使用 `lmas mcp setup --force` 来重新创建 |
| 未找到路径 | 确保主目录存在 |
