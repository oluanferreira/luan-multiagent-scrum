...
**@qa 代理 (Oracle)** 是 LMAS 的**测试架构师和质量顾问**。其角色是为开发团队提供全面的质量分析、质量门决策和可行的建议。
...
| 文件 | 命令 | 目的 |
|---------|---------|-----------|
| `.lmas-core/development/tasks/qa-gate.md` | `*gate {story}` | 创建质量门决策文件 |
| `.lmas-core/development/tasks/qa-review-story.md` | `*review {story}` | 完整故事审查和门决策 |
| `.lmas-core/development/tasks/qa-test-design.md` | `*test-design {story}` | 创建全面的测试场景 |
| `.lmas-core/development/tasks/qa-risk-profile.md` | `*risk-profile {story}` | 生成风险评估矩阵 |
| `.lmas-core/development/tasks/qa-nfr-assess.md` | `*nfr-assess {story}` | 验证非功能需求 |
| `.lmas-core/development/tasks/qa-trace-requirements.md` | `*trace {story}` | 将需求映射到测试(给定-当-然) |
| `.lmas-core/development/tasks/qa-generate-tests.md` | `*generate-tests` | 自动生成测试套件 |
| `.lmas-core/development/tasks/qa-run-tests.md` | `*run-tests` | 执行测试套件和质量门 |
| `.lmas-core/development/tasks/qa-backlog-add-followup.md` | `*backlog-add` | 添加后续项到待办事项 |
| `.lmas-core/development/tasks/qa-create-fix-request.md` | `*create-fix-request {story}` | 为 @dev 生成修复请求文档 |

### 代理定义文件

| 文件 | 目的 |
|---------|-----------|
| `.lmas-core/development/agents/qa.md` | QA 代理的完整定义 |
| `.claude/commands/LMAS/agents/qa.md` | Claude Code 命令用于激活 @qa |

### 工作流文件

| 文件 | 目的 |
|---------|-----------|
| `.lmas-core/development/workflows/qa-loop.yaml` | QA 循环协调器(审查 -> 修复 -> 重新审查) |
