# 数据工程师代理系统 (@data-engineer) - LMAS

> **版本:** 1.0.0
> **创建于:** 2026-02-04
> **所有者:** @data-engineer (Dozer - The Sage)
> **状态:** 官方文档
...
**@数据工程师(Dozer)**是LMAS的数据库架构师和运营工程师，负责架构设计、迁移、安全性(RLS)、性能优化和DBA操作。该代理充当**Sage(智者)**，建模业务领域，使用快照和回滚实现安全迁移，并确保数据完整性和安全性。
...
| 文件 | 命令 | 目的 |
|------|------|------|
| `.lmas-core/development/tasks/db-domain-modeling.md` | `*model-domain` | 交互式域建模会话 |
| `.lmas-core/development/tasks/setup-database.md` | `*setup-database [type]` | 数据库项目设置(Supabase、PostgreSQL、MongoDB、MySQL、SQLite) |
| `.lmas-core/development/tasks/db-bootstrap.md` | `*bootstrap` | 创建标准Supabase项目结构 |
| `.lmas-core/development/tasks/db-env-check.md` | `*env-check` | 验证数据库环境变量 |
| `.lmas-core/development/tasks/db-apply-migration.md` | `*apply-migration {path}` | 应用迁移(含快照和锁定建议) |
| `.lmas-core/development/tasks/db-dry-run.md` | `*dry-run {path}` | 测试迁移无提交 |
| `.lmas-core/development/tasks/db-seed.md` | `*seed {path}` | 应用种子数据(幂等) |
| `.lmas-core/development/tasks/db-snapshot.md` | `*snapshot {label}` | 创建架构快照 |
| `.lmas-core/development/tasks/db-rollback.md` | `*rollback {snapshot_or_file}` | 恢复快照或执行回滚 |
| `.lmas-core/development/tasks/db-smoke-test.md` | `*smoke-test {version}` | 全面的数据库测试 |
| `.lmas-core/development/tasks/security-audit.md` | `*security-audit {scope}` | 安全审计(rls、架构、完整) |
| `.lmas-core/development/tasks/analyze-performance.md` | `*analyze-performance {type}` | 性能分析(查询、热路径、交互) |
| `.lmas-core/development/tasks/db-policy-apply.md` | `*policy-apply {table} {mode}` | 安装RLS策略(KISS或详细) |
| `.lmas-core/development/tasks/test-as-user.md` | `*test-as-user {user_id}` | 模拟用户以测试RLS |
| `.lmas-core/development/tasks/db-verify-order.md` | `*verify-order {path}` | 验证DDL顺序以处理依赖 |
| `.lmas-core/development/tasks/db-load-csv.md` | `*load-csv {table} {file}` | 安全的CSV加载程序(staging->merge) |
| `.lmas-core/development/tasks/db-run-sql.md` | `*run-sql {file_or_inline}` | 执行原始SQL(含事务) |
| `.lmas-core/development/tasks/create-deep-research-prompt.md` | `*research {topic}` | 生成深度研究提示 |
| `.lmas-core/development/tasks/execute-checklist.md` | `*execute-checklist {checklist}` | 执行DBA检查清单 |
| `.lmas-core/development/tasks/create-doc.md` | `*doc-out` | 完整文档输出 |

### 代理定义文件

| 文件 | 目的 |
|------|------|
| `.lmas-core/development/agents/data-engineer.md` | @data-engineer代理核心定义(角色、命令、工作流) |
| `.claude/commands/LMAS/agents/data-engineer.md` | Claude Code命令激活@data-engineer |
...
| 文件 | 目的 |
|------|------|
| `.lmas-core/development/workflows/brownfield-discovery.yaml` | Brownfield发现工作流(第2和第5阶段) |
...
flowchart TB
    ...
    subgraph UPSTREAM["上游 - 提供需求"]
        ARCH["@architect(Architect)<br/>系统架构"]
    end
