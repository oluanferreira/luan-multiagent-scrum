...
- **FR9:** O sistema deve funcionar como CLI command (`lmas graph`) com sub-comandos: `--deps`, `--blast <file>`, `--stats`, `--watch`
...
O CLI Graph Dashboard será adicionado ao monorepo existente `lmas-core`, como um novo módulo em `.lmas-core/core/graph-dashboard/` com entrypoint em `bin/lmas-graph.js`.
...
### Epic 1: MVP — ASCII Text Output (`lmas graph`)
...
**Goal:** Estabelecer o CLI command `lmas graph` com output ASCII do dependency graph e entity stats. Entrega valor imediato com zero dependências TUI pesadas — apenas `asciichart` para line charts simples. Este é o fundamento sobre o qual os epics seguintes construirão.

### Story 1.1: CLI Entrypoint e Dependency Tree ASCII

**As a** developer using LMAS,
**I want** to run `lmas graph --deps` in the terminal,
**so that** I can see the dependency tree of code-intel entities as ASCII text.

#### Acceptance Criteria

1. Comando `npx lmas-core graph` existe e é executável
...
5. Output é válido para pipe (`lmas graph --deps | grep helper`)
6. Testes unitários cobrem: tree rendering, fallback data, empty graph

### Story 1.2: Entity Stats e Cache Metrics ASCII

**As a** developer using LMAS,
**I want** to run `lmas graph --stats` to see entity statistics and cache metrics,
...
1. Sem flags adicionais, `lmas graph` mostra summary view (dependency tree + stats + provider status)
...
1. `lmas graph --interactive` abre dashboard blessed-contrib fullscreen
...
**Goal:** Adicionar modo watch para updates automáticos, blast radius interativo para qualquer arquivo, e comandos de agente (`*graph`) para integração no workflow dos agentes LMAS.
...
1. `lmas graph --blast <file>` mostra blast radius de um arquivo específico
...
> @architect — Crie a arquitetura técnica detalhada para o CLI Graph Dashboard usando este PRD como input. Foco em: (1) estrutura de módulos dentro de `.lmas-core/core/graph-dashboard/`, (2) data source adapters para code-intel e entity-registry, (3) widget composition pattern com blessed-contrib, (4) CLI command routing via `bin/lmas-graph.js`, (5) fallback strategy quando Code Graph MCP está offline. Reference: `docs/research/2026-02-21-cli-graph-dashboard/03-recommendations.md` para stack recomendado.
