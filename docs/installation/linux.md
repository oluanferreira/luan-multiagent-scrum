...
node bin/lmas-init.js ~/projects/my-project
```
...
2. Commands are installed to `.claude/commands/LMAS/`
...
2. Configure LMAS rules in `.continue/`
...
# LMAS configuration
export LMAS_HOME="$HOME/.lmas-core"
export PATH="$LMAS_HOME/bin:$PATH"
...
# Config files: ~/.config/lmas/
# Cache: ~/.cache/lmas/
# State: ~/.local/state/lmas/
```
...
git pull
npm install
node bin/lmas-init.js ~/projects/my-project --update
```

The updater will:

- Detect your existing installation
- Back up any customizations to `.lmas-backup/`
