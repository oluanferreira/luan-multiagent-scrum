...
Guide for integrating LMAS with supported IDEs and AI development platforms.
...
## Compatibility Contract (LMAS 4.2.11)

The IDE matrix is enforced by a versioned contract:

- Contract file: `.lmas-core/infrastructure/contracts/compatibility/lmas-4.2.11.yaml`
...
LMAS supports multiple AI-powered development platforms. Choose the one that best fits your workflow.

### Quick Status Matrix (LMAS 4.2.11)
...
| Gemini CLI | Works | `/lmas-menu` then `/lmas-<agent>` | Works (minor differences in event handling) | -- |
| Codex CLI | Limited | `/skills` then `lmas-<agent-id>` | Limited (some checks need manual sync) | Run `npm run sync:ide:codex` and follow `/skills` flow |
...
Legend:
- `Works`: fully recommended for new users in LMAS 4.2.11.
...
**Recommendation Level:** Best LMAS integration

```yaml
config_file: .claude/CLAUDE.md
agent_folder: .claude/commands/LMAS/agents
...
1. LMAS automatically creates `.claude/` directory on init
...
# Verify setup
ls -la .claude/commands/LMAS/agents/
```
...
special_features:
  ...
  - /skills activators (lmas-<agent-id>)
...
4. Use `/skills` and choose `lmas-architect`, `lmas-dev`, etc.
...
1. LMAS creates `.cursor/` directory on init
...
2. LMAS creates `.github/copilot-instructions.md`
...
1. LMAS creates `.antigravity/` directory
...
agent_folder: .gemini/rules/LMAS/agents
...
2. LMAS creates:
   - `.gemini/rules.md`
   - `.gemini/rules/LMAS/agents/*.md`
   - `.gemini/commands/*.toml` (`/lmas-menu`, `/lmas-<agent>`)
   - `.gemini/hooks/*.js`
...
4. Quick agent activation (recommended):
   - `/lmas-menu` to list shortcuts
   - `/lmas-dev`, `/lmas-architect`, `/lmas-qa`, etc.
   - `/lmas-agent <agent-id>` for generic launcher
...
LMAS maintains a single source of truth for agent definitions and synchronizes them to all configured IDEs:

```
┌─────────────────────────────────────────────────────┐
│                    LMAS Core                         │
│  .lmas-core/development/agents/  (Source of Truth)  │
│                        │                             │
│            ┌───────────┼───────────┐                │
│            ▼           ▼           ▼                │
│  .claude/     .codex/      .cursor/                  │
...
LMAS can be configured to automatically sync on agent changes:

```yaml
# .lmas-core/core/config/sync.yaml
auto_sync:
  enabled: true
  watch_paths:
...
ls .claude/commands/LMAS/agents/       # Claude Code
ls .gemini/rules/LMAS/agents/          # Gemini CLI
```
...
├── Yes --> Claude Code (Best LMAS integration)
└── No
...
# Verify migration
diff -r ./rules-backup/ .claude/commands/LMAS/agents/
```
