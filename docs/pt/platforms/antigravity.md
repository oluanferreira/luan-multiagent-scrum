...
### Por que usar LMAS com AntiGravity?
...
```markdown
# Regras Synkra LMAS para AntiGravity
...
1. Seus agentes em `.lmas-core/` são portáveis
2. Inicialize AntiGravity:
   ```bash
   npx @anthropic/lmas init --ide antigravity
   ```
3. Arquivos de workflow são gerados automaticamente

### Do AntiGravity para Outras IDEs

1. Agentes LMAS permanecem em `.lmas-core/`
...
*Synkra LMAS - Guia da Plataforma AntiGravity v1.0*
