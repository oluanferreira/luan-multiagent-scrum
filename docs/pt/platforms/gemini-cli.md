...
### Por que usar LMAS com Gemini CLI?
...
```markdown
# Regras Synkra LMAS para Gemini CLI
...
## Ativação
Inclua "Como agente LMAS dev" no seu prompt.
...
```bash
# Agente desenvolvedor
gemini "Como agente LMAS dev, implemente a feature de login"

# Agente QA
gemini "Como agente LMAS qa, revise este código para segurança"

# Agente arquiteto
gemini "Como agente LMAS architect, projete o schema do banco de dados"
```
...
cat src/auth.ts | gemini "Como LMAS qa, revise este código"
...
for file in src/*.ts; do
  gemini --file "$file" "Como LMAS qa, revise este arquivo" >> reviews.md
done
```
...
**Frase de Ativação:** "Como agente LMAS dev"
...
1. Instale LMAS para Gemini CLI:
   ```bash
   npx @anthropic/lmas init --ide gemini-cli
   ```
2. Agentes transformam para formato baseado em prompt

### Do Gemini CLI para Outras IDEs

1. Agentes LMAS em `.lmas-core/` são portáveis
...
*Synkra LMAS - Guia da Plataforma Gemini CLI v1.0*
