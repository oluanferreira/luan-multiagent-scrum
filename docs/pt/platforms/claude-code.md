# Guia LMAS para Claude Code

> 🌐 [EN](../../platforms/claude-code.md) | **PT** | [ES](../../es/platforms/claude-code.md)

---

> **CLI Oficial da Anthropic** - Plataforma Recomendada para LMAS
...
### Por que usar LMAS com Claude Code?

O Claude Code oferece a **melhor integração** com o LMAS devido a:
...
# Verificar se os arquivos LMAS foram criados
ls -la .claude/
```
...
- Entendimento do framework LMAS
...
export LMAS_DEBUG="true"                        # Habilitar log de debug
```
...
```mermaid
flowchart LR
    Source["LMAS Core<br/>.lmas-core/development/agents"] --> Parser["Parser de Agente"]
    Parser --> Transform["Transformador Claude"]
    Transform --> Output[".claude/commands/LMAS/agents"]
```
...
1. Alterações locais são salvas em `.claude/commands/LMAS/agents/.backup/`
...
# Verificar logs do LMAS
cat .lmas-core/logs/sync.log
```
...
R: Em `.claude/commands/LMAS/agents/` após a sincronização.

**P: Com que frequência devo sincronizar agentes?**
R: Após atualizar o LMAS ou quando agentes são modificados no core.
...
2. Inicialize o LMAS com Claude Code:
   ```bash
   npx @anthropic/lmas init --ide claude-code
   ```
...
1. Seus agentes LMAS estão armazenados em `.lmas-core/development/agents/`
2. Inicialize o LMAS para a IDE de destino
...
*Synkra LMAS - Guia da Plataforma Claude Code v1.0*
