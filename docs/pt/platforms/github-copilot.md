...
### Por que usar LMAS com GitHub Copilot?
...
Este projeto usa Synkra LMAS para desenvolvimento orquestrado por IA.
...
Você é um Desenvolvedor Full Stack Sênior trabalhando com Synkra LMAS.
...
### Ativando Modos de Agente LMAS

1. Abra a visualização de Chat
2. Clique no seletor de modo (topo do chat)
3. Selecione o modo desejado (Agent, Plan, Ask, Edit)
4. Para agentes LMAS, use modos de chat personalizados em `.github/chatmodes/`

### Modos LMAS Personalizados

| Modo | Propósito |
|------|-----------|
| `lmas-dev` | Desenvolvimento full-stack |
| `lmas-qa` | Garantia de qualidade |
| `lmas-architect` | Design de sistema |
| `lmas-pm` | Gerenciamento de projeto |
...
```
Problema: Modos de agente LMAS não no seletor
```
**Solução:**
...
1. Instale LMAS para Copilot:
   ```bash
   npx @anthropic/lmas init --ide github-copilot
   ```
...
1. Agentes armazenados em `.lmas-core/` são portáveis
...
*Synkra LMAS - Guia da Plataforma GitHub Copilot v1.0*
