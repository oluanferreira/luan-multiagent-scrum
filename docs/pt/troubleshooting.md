...
# Guia de Solução de Problemas do Synkra LMAS

> 🌐 [EN](../troubleshooting.md) | **PT** | [ES](../es/troubleshooting.md)

---

Este guia abrangente ajuda você a diagnosticar e resolver problemas comuns com o Synkra LMAS.
...
# Saída detalhada
npx lmas-core doctor --verbose

# Verificar componente específico
npx lmas-core doctor --component memory-layer
```
...
npm exec lmas-core init my-project
```
...
npx lmas-core doctor --component config
...
```javascript
// Editar .lmas/memory-config.json
{
...
```
Error: "lmas-core" cannot be opened
```
...
xattr -d com.apple.quarantine /usr/local/bin/lmas-core
```
...
```bash
# Ver logs recentes
tail -f .lmas/logs/lmas.log

# Buscar por erros
grep -i error .lmas/logs/*.log
...
npx lmas-core doctor --report diagnostic.json

# Incluir informações do sistema
npx lmas-core info --detailed >> diagnostic.json
...
```bash
# Analisar perfil
*debug analyze-profile .lmas/profiles/latest.cpuprofile
```
...
1. **Execute diagnósticos:**
   ```bash
   npx lmas-core doctor --verbose > diagnostic.log
   ```
...
2. **Colete informações:**
   ...
   - Versão do LMAS: `npx lmas-core version`

3. **Verifique issues existentes:**
   - [GitHub Issues](https://github.com/lmas-core/lmas-core/issues)
   - [Discussions](https://github.com/lmas-core/lmas-core/discussions)
...
- **Stack Overflow**: Marque perguntas com `lmas-core`
...
1. Executar `npx lmas-core init test`
...
# Reset completo
rm -rf .lmas node_modules package-lock.json
npm cache clean --force
