...
```bash
LMAS_AI_PROVIDER=openai          # Provedor de IA
LMAS_AI_MODEL=gpt-4             # Modelo de IA
LMAS_LOG_LEVEL=debug            # Nível de log
LMAS_TELEMETRY=disabled         # Configuração de telemetria
LMAS_TIMEOUT=30000             # Timeout de comando (ms)
LMAS_MEMORY_CACHE=true         # Cache de memória
```
