...
Guia para integrar o LMAS com IDEs e plataformas de desenvolvimento com IA suportadas.
...
O LMAS suporta 6 plataformas de desenvolvimento com IA. Escolha a que melhor se adapta ao seu fluxo de trabalho.
...
| IDE | Paridade de Hooks vs Claude | O que degrada sem hooks completos | Mitigação no LMAS |
| --- | --- | --- | --- |
| Claude Code | Completa | Nenhum (comportamento de referência) | Hooks nativos + pipeline completo do LMAS |
...
**Nível de Recomendação:** Melhor integração com LMAS
...
1. LMAS cria automaticamente o diretório `.claude/` durante a inicialização
...
4. Use `/skills` e selecione `lmas-<agent-id>`
...
1. LMAS cria o diretório `.cursor/` durante a inicialização
...
2. LMAS cria `.github/copilot-instructions.md`
...
1. LMAS cria o diretório `.antigravity/`
...
O LMAS mantém uma única fonte de verdade para definições de agentes e as sincroniza com todas as IDEs configuradas:
...
O LMAS pode ser configurado para sincronizar automaticamente quando houver mudanças nos agentes:
...
├── Sim --> Claude Code (Melhor integração com LMAS)
└── Não
...
_Guia de Integração com IDEs do Synkra LMAS v4.0_
