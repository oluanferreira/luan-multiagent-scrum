...
LMAS usa agentes especializados para diferentes tarefas. Na sua IDE com IA ou Claude Code CLI, digite:
...
| ------------------- | -------------------- | ---------------------------------- |
| `@dev` (Neo)        | Desenvolvimento      | Implementação de código, debugging |
| `@qa` (Oracle)       | Qualidade            | Testes e validação                 |
| `@architect` (Architect) | Arquitetura          | Design de sistema e documentação   |
| `@pm` (Sage)        | Product Manager      | Requisitos e planejamento          |
| `@devops` (Operator)    | DevOps               | Git push, criação de PR, CI/CD     |
| `@po` (Maven)       | Product Owner        | Criação de stories e backlog       |
| `@sm` (Niobe)       | Scrum Master         | Gerenciamento de sprint            |
...
O agente desenvolvedor (Neo) será ativado com uma saudação mostrando:
...
Stories direcionam o desenvolvimento no LMAS. Ative o Product Owner e crie uma:
...
```bash
# Instalação e setup
npx lmas-core@latest           # Executar wizard
npx lmas-core@latest doctor    # Executar diagnósticos
npx lmas-core@latest info      # Mostrar info do sistema

# Desenvolvimento
npm run lint                           # Verificar estilo de código
npm run typecheck                      # Verificar tipos TypeScript
npm test                               # Executar testes unitários
npm run validate:structure             # Validar estrutura LMAS
```
...
3. **Aprenda sobre agentes:** [Definições de Agentes](../../../.lmas-core/development/agents/)
...
3. Verifique se o arquivo do agente existe: `ls .lmas-core/development/agents/`
...
### Estrutura LMAS não encontrada

```bash
# Reinstalar LMAS no projeto atual
...
- **Documentação:** [Repositório GitHub](https://github.com/oluanferreira/luan-multiagent-scrum)
