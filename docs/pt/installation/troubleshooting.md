...
# Guia de Solução de Problemas do LMAS
...
### Problema 1: "npx lmas-core is not recognized"
...
```
npm ERR! code E404
npm ERR! 404 Not Found - GET https://registry.npmjs.org/lmas-core
```
...
npm ERR! network request to https://registry.npmjs.org/lmas-core failed
```
...
```
Error: ENOTEMPTY: directory not empty, rmdir '.lmas-core'
```
...
cp .lmas-core.backup/custom-files/* .lmas-core/
```
...
2. Verifique se os arquivos foram criados:
   ...
   ```bash
   # Claude Code
   ls .claude/commands/LMAS/agents/
...
```
Error: Agent 'dev' not found in .lmas-core/agents/
```
...
npx yaml-lint .lmas-core/agents/dev.md
...
mv .lmas-core/agents/dev.md .lmas-core/agents/dev.md.backup
npx lmas-core install --full
```
...
```bash
# Verifique status da instalação LMAS
npx lmas-core status
...
```bash
# Verifique estrutura .lmas-core
find .lmas-core -type f | wc -l
# Esperado: 200+ arquivos

# Verifique YAML corrompido
for f in .lmas-core/**/*.yaml; do npx yaml-lint "$f"; done
...
1. Execute `npx lmas-core status` e anote a saída
2. Consulte este guia de solução de problemas
3. Pesquise [Issues existentes no GitHub](https://github.com/oluanferreira/luan-multiagent-scrum/issues)
...
- **Issues no GitHub**: [lmas-core/issues](https://github.com/oluanferreira/luan-multiagent-scrum/issues)
