...
# Guia do Sistema de Quality Gates LMAS
...
O Sistema de Quality Gates do LMAS fornece garantia de qualidade automatizada através de três camadas progressivas de validação. Cada camada captura diferentes tipos de problemas no estágio apropriado do desenvolvimento.
...
| **Oracle (@qa)** | Revisão automatizada de QA | CRITICAL               |
...
```javascript
// Invocação programática do Oracle
...
# Executar com opções
lmas qa run --verbose          # Saída detalhada
lmas qa run --fail-fast        # Parar na primeira falha
lmas qa run --continue-on-fail # Continuar apesar de falhas
```
...
| Oracle bloqueado       | Revise feedback do @qa, atualize o código            |
