...
O Sistema de Greeting Contextual é uma melhoria de UX que torna os greetings dos agentes LMAS inteligentes e adaptativos, mostrando informações e comandos relevantes baseados no contexto da sessão.
...
### Scripts Core
- `.lmas-core/core/session/context-detector.js` - Detecção de tipo de sessão
- `.lmas-core/infrastructure/scripts/git-config-detector.js` - Detecção de git config
- `.lmas-core/development/scripts/greeting-builder.js` - Montagem do greeting
- `.lmas-core/development/scripts/workflow-navigator.js` - Navegação de workflow
- `.lmas-core/development/scripts/agent-exit-hooks.js` - Hooks de saída (para persistência)

### Arquivos de Dados
- `.lmas-core/data/workflow-patterns.yaml` - Definições de workflows
...
### Configuração
- `.lmas-core/core-config.yaml` - Configuração global (seções git + agentIdentity)

### Agentes (Atualizados)
- `.lmas-core/agents/dev.md` - Metadata de visibilidade de comandos
- `.lmas-core/agents/po.md` - Metadata de visibilidade de comandos
- `.lmas-core/agents/*.md` - 9 agentes restantes (atualização pendente)
...
R: Use o script de teste: `node .lmas-core/development/scripts/test-greeting-system.js`
