...
O LMAS segue uma estratégia abrangente de testes que garante qualidade de código em todas as camadas do framework. Nossa filosofia de testes é baseada em:
 ...
 *
 * Garante que agentes de versões anteriores do LMAS continuem funcionando.
 */
...
O LMAS suporta Windows, macOS e Linux. Testes cross-platform garantem comportamento consistente.
...
│   └── .lmas-core/       # Cobertura por módulo
...
O Sistema de Quality Gate do LMAS (veja [Guia de Quality Gates](./quality-gates.md)) integra testes em múltiplas camadas:
