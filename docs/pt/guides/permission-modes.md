...
> Controle o nível de autonomia que os agentes LMAS têm sobre seu sistema.

---

## Visão Geral

Os Modos de Permissão permitem controlar o nível de autonomia dos agentes LMAS. Seja explorando um novo codebase ou executando builds totalmente autônomos, há um modo para seu workflow.
...
O modo é persistido em `.lmas/config.yaml`:
...
Verifique se `.lmas/config.yaml` existe e é gravável:
