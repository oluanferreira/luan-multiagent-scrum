...
Antes de usar o LMAS, certifique-se de ter:
...
O LMAS fornece estrutura orquestrada enquanto permite flexibilidade na comunicação. Isso significa:
...
| Desenvolvimento Tradicional com IA | LMAS                                        |
| ---------------------------------- | ------------------------------------------- |
...
O LMAS inclui 11 agentes especializados, cada um com papel e personalidade distintos:

| Agente    | ID               | Arquétipo    | Responsabilidade          |
| --------- | ---------------- | ------------ | ------------------------- |
| **Neo**   | `@dev`           | Construtor   | Implementação de código   |
| **Oracle** | `@qa`            | Guardião     | Garantia de qualidade     |
...
| **Dozer**  | `@data-engineer` | Arquiteto    | Engenharia de dados       |
| **Felix** | `@devops`        | Otimizador   | CI/CD e operações         |
| **Uma**   | `@ux-expert`     | Criador      | Experiência do usuário    |
| **Keymaker**   | `@lmas-master`   | Orquestrador | Orquestração do framework |

### Ativação de Agentes

```bash
# Ative um agente usando sintaxe @
@dev                # Ativar Neo (Desenvolvedor)
@qa                 # Ativar Oracle (QA)
@architect          # Ativar Aria (Arquiteto)
@lmas-master        # Ativar Keymaker (Orquestrador)
...
Tasks são o ponto de entrada principal no LMAS. Tudo é uma task.
...
| `brownfield-integration` | Adicionar LMAS a existente | dev, architect     |
...
Squads são equipes modulares de agentes de IA que estendem a funcionalidade do LMAS.
...
```
my-project/
├── .lmas-core/                # Configuração do framework
...
A configuração principal está em `.lmas-core/core/config/`:
...
O LMAS suporta múltiplas IDEs. A configuração é sincronizada entre:
...
O LMAS implementa quality gates em 3 camadas:
