# Guia de Hardening de Segurança do LMAS

> [EN](../../guides/security-hardening.md) | **PT** | [ES](../../es/guides/security-hardening.md)

---

> Guia completo para fortalecer a segurança de implantações do Synkra LMAS - do desenvolvimento à produção.
...
O Synkra LMAS opera em uma camada privilegiada entre modelos de IA e seu sistema. Este guia cobre estratégias de hardening específicas para ambientes de desenvolvimento orquestrados por IA.
...
### Preocupações de Segurança Específicas do LMAS
...
O LMAS implementa múltiplas camadas de proteção:
...
Chaves de API são os segredos mais críticos no LMAS. Chaves comprometidas podem levar a uso não autorizado, violações de dados e impacto financeiro significativo.
...
```bash
# ============================================================
# CONFIGURAÇÃO DE AMBIENTE DO LMAS
# ============================================================
...
### Permissões da Estrutura de Diretórios do LMAS
...
# Diretórios de configuração do LMAS
chmod 700 .lmas/              # Apenas o proprietário pode acessar
chmod 700 .lmas-core/         # Fonte do framework
chmod 700 .claude/            # Configuração do Claude

# Arquivos de configuração sensíveis
chmod 600 .env                # Variáveis de ambiente
chmod 600 .lmas/config.yaml   # Configuração principal
chmod 600 .lmas/users.json    # Banco de dados de usuários
chmod 600 .lmas/sessions.json # Sessões ativas
...
```yaml
# .lmas/config.yaml - Configuração de diretórios permitidos
security:
  allowedDirectories:
    read:
      - '${PROJECT_ROOT}'
...
echo "Verificação de Permissões de Segurança do LMAS"
echo "==============================================="
...
O LMAS usa containers Docker para isolar servidores MCP do sistema host:
...
```yaml
# .lmas/config.yaml - Configuração de auditoria
audit:
  enabled: true
...
function validateProductionSecurity() {
  ...
  if (process.env.LMAS_DEBUG === 'true') {
    errors.push('LMAS_DEBUG deve ser false em produção');
  }
...
Se você descobrir uma vulnerabilidade de segurança no Synkra LMAS, por favor siga práticas de divulgação responsável:

### Processo de Relatório

1. **NÃO** crie uma issue pública no GitHub para vulnerabilidades de segurança
2. Reporte preocupações de segurança via [GitHub Security Advisories](https://github.com/SynkraAI/lmas-core/security/advisories)
...
Atualmente, o Synkra LMAS não possui um programa formal de bug bounty. No entanto, contribuições significativas de segurança são reconhecidas e podem receber licenças LMAS Pro ou outro reconhecimento.
...
_Guia de Hardening de Segurança do Synkra LMAS v4.0_
