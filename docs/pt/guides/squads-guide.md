...
Guia completo para criar, validar, publicar e gerenciar Squads no LMAS.
...
Squads são equipes modulares de agentes de IA que estendem a funcionalidade do LMAS. Cada squad é um pacote autocontido contendo:
...
- Node.js 18+
- Projeto LMAS inicializado (`.lmas-core/` existe)
...
Tasks devem seguir [TASK-FORMAT-SPECIFICATION-V1](../../../.lmas-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Modo       | Comportamento                                   |
| ---------- | ----------------------------------------------- |
| `extend`   | Adiciona regras do squad às regras core do LMAS |
...
### Nível 2: Repositório lmas-squads (Público)
...
3. **Adicionar Campos** - `lmas.type`, `lmas.minVersion`
...
# - lmas.type: deve ser "squad"
# - lmas.minVersion: deve ser semver válido
```
...
### Qual a diferença entre um Squad e formatos legados de squad no LMAS?

**Squads** são o padrão no LMAS 2.1+ com:
...
Squads requerem LMAS 2.1.0+. Defina no manifesto:
...
_LMAS Squads: Equipes de AI agents trabalhando com você_
