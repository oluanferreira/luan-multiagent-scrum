...
> Motor de geração de documentos e substituição de variáveis para Synkra LMAS.
...
O Template Engine fornece uma forma consistente de gerar documentos (PRDs, ADRs, Stories, etc.) com substituição de variáveis, condicionais e loops. Ele alimenta todas as tarefas de geração de documentos no LMAS.
...
```javascript
const TemplateEngine = require('./.lmas-core/infrastructure/scripts/template-engine');
...
```javascript
const result = await engine.loadAndProcess('.lmas-core/product/templates/story-tmpl.md', {
  STORY_ID: '3.12',
  TITLE: 'Documentation',
});
```
...
_Synkra LMAS Template Engine v2.0_
