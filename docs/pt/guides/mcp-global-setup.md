...
# Guia de Configuração Global MCP do LMAS

> 🌐 [EN](../../guides/mcp-global-setup.md) | **PT** | [ES](../../es/guides/mcp-global-setup.md)

---

> Configure servidores MCP (Model Context Protocol) globais para Synkra LMAS.
...
O Sistema Global MCP permite que você configure servidores MCP uma única vez e os compartilhe entre todos os projetos LMAS. Isso elimina a necessidade de configurar os mesmos servidores em cada projeto.
...
- `~/.lmas/` - Diretório global do LMAS
- `~/.lmas/mcp/` - Diretório de configuração MCP
- `~/.lmas/mcp/global-config.json` - Arquivo de configuração principal
- `~/.lmas/mcp/servers/` - Configurações individuais de servidores
- `~/.lmas/mcp/cache/` - Cache de respostas
- `~/.lmas/credentials/` - Armazenamento seguro de credenciais
...
O LMAS inclui templates para servidores MCP populares:
...
Credenciais são armazenadas em `~/.lmas/credentials/` com um `.gitignore` para prevenir commits acidentais.
...
| Diretório já existe    | Use `lmas mcp setup --force` para recriar                                 |
...
_Synkra LMAS v4 Guia de Configuração Global MCP_
