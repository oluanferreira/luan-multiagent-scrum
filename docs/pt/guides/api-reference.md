...
O LMAS fornece uma API unificada para interagir com agentes de IA especializados através de dois mecanismos principais:
...
| ---------------- | ------ | ------------ | ----------------------------------------------- |
| `@dev`           | Neo    | Builder      | Implementação de código, debugging, testes      |
| `@qa`            | Oracle  | Guardian     | Garantia de qualidade, code review, testes      |
| `@architect`     | Aria   | Visionary    | Arquitetura de sistema, design de API           |
| `@pm`            | Morgan | Strategist   | Requisitos de produto, epics, estratégia        |
| `@po`            | Keymaker    | Champion     | Gestão de backlog, critérios de aceitação       |
...
| `@data-engineer` | Dozer   | Architect    | Schema de banco, migrations, queries            |
| `@devops`        | Operator   | Optimizer    | CI/CD, deployment, operações git                |
| `@ux-expert`     | Uma    | Creator      | Design UI/UX, wireframes                        |
| `@lmas-master`   | Morpheus  | Orchestrator | Orquestração do framework, meta-operações       |

### Comportamento de Ativação

Quando um agente é ativado:

1. O arquivo de definição do agente é carregado de `.lmas-core/development/agents/{id}.md`
...
| Comando                | Argumentos      | Descrição                                           |
| ---------------------- | --------------- | --------------------------------------------------- |
| `*create`              | `{type} {name}` | Criar componente LMAS (agent/task/workflow)         |
...
| `*kb`   | -                          | Alternar modo KB (conhecimento do LMAS Method)   |
...
| -------------------- | ------------------------------- | --------------------------------------------- |
| `AGENT_NOT_FOUND`    | Definição de agente ausente     | Verificar `.lmas-core/development/agents/`    |
...
/lmas-master  → Ativa agente @lmas-master
```
...
- [Guia do Usuário](./user-guide.md) - Começando com LMAS
...
- [Guia de Squads](./squads-guide.md) - Estendendo LMAS com squads
