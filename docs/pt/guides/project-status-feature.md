...
A funcionalidade de Status do Projeto exibe automaticamente seu contexto de trabalho atual quando qualquer agente LMAS é ativado. Isso inclui:
...
```
Neo (Builder) ready. Let's build something great!
...
- **Framework LMAS-FullStack** instalado
...
3. Criar arquivo de cache `.lmas/project-status.yaml`
...
1. Edite `.lmas-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Localização: `.lmas-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .lmas/project-status.yaml  # Localização do arquivo de cache
...
1. **Verifica cache** - Procura por `.lmas/project-status.yaml`
...
- **Localização do Cache:** `.lmas/project-status.yaml`
...
Todos os 11 agentes LMAS exibem status do projeto:

1. **@dev** (Neo - Builder)
2. **@po** (Keymaker - Balancer)
3. **@qa** (Oracle - Guardian)
4. **@sm** (River - Facilitator)
5. **@pm** (Morgan - Strategist)
6. **@architect** (Aria - Visionary)
7. **@analyst** (Atlas - Decoder)
8. **@devops** (Operator - Operator)
9. **@data-engineer** (Dozer - Sage)
10. **@ux-design-expert** (Uma - Empathizer)
11. **@lmas-master** (Morpheus - Orchestrator)
...
3. O arquivo `.lmas-core/infrastructure/scripts/project-status-loader.js` existe?
...
```javascript
const { loadProjectStatus, formatStatusDisplay } = require('./.lmas-core/infrastructure/scripts/project-status-loader.js');
...
const { clearCache } = require('./.lmas-core/infrastructure/scripts/project-status-loader.js');
await clearCache();
```
...
```bash
# Remove script
rm .lmas-core/infrastructure/scripts/project-status-loader.js

# Remove task
rm .lmas-core/tasks/init-project-status.md

# Remove cache
rm .lmas/project-status.yaml

# Remove tests
rm .lmas-core/infrastructure/scripts/__tests__/project-status-loader.test.js
...
R: Sim, todos os agentes usam o mesmo arquivo de cache (`.lmas/project-status.yaml`).

---

## Documentação Relacionada

- **Story:** `docs/stories/lmas migration/story-6.1.2.4-project-status-context.md`
- **Config:** `.lmas-core/core-config.yaml` (seção projectStatus)
- **Script:** `.lmas-core/infrastructure/scripts/project-status-loader.js`
- **Task de Init:** `.lmas-core/tasks/init-project-status.md`
- **Testes:** `.lmas-core/infrastructure/scripts/__tests__/project-status-loader.test.js`
