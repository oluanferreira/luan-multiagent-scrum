...
Este documento define os modos de execução para o Sistema de Verificação de Saúde do LMAS (HCS), baseado em pesquisa de melhores práticas da indústria do Kubernetes, VS Code, Terraform, npm/yarn e padrões CLI "doctor" (Flutter, Homebrew, WP-CLI).
...
| **Na instalação/bootstrap**  | `npx lmas install` | 60-120s | Esperado                       | Validação de setup      | ✅ **Obrigatório**           |
...
**Veredito:** Obrigatório. Parte do `npx lmas install` e `*bootstrap-setup`.
    ...
    steps:
      ...
      - name: Postar no Slack (em caso de falha)
        ...
        with:
          payload: |
            {
              "text": "⚠️ LMAS Health Check Falhou",
              "blocks": [
                {
                  "type": "section",
                  "text": {
