...
Este documento descreve as melhores práticas para gerenciar chaves de API usadas por servidores MCP (Model Context Protocol) no LMAS. O gerenciamento adequado de chaves de API é crítico para segurança e integridade operacional.

---

## Arquitetura MCP no LMAS

LMAS utiliza Docker MCP Toolkit como a infraestrutura principal de MCP:
...
## Governança MCP no LMAS

**IMPORTANTE:** Todo o gerenciamento de infraestrutura MCP é tratado EXCLUSIVAMENTE pelo **Agente DevOps (@devops / Operator)**.
