...
**Autor:** @architect (Architect) via @dev (Neo)
...
Este documento especifica as capacidades de auto-recuperação do Sistema de Verificação de Saúde do LMAS (HCS). A auto-recuperação permite que o sistema corrija automaticamente certos problemas enquanto mantém a segurança e o controle do usuário.
...
| ---------------------- | -------------------------------------------------------- | ------------- |
| `recreate_config`      | Recriar `.lmas/config.yaml` ausente a partir do template | Sim           |
| `clear_cache`          | Limpar arquivos de cache obsoletos em `.lmas/cache/`     | Sim           |
...
- id: PC-001
  description: '.lmas/config.yaml ausente'
  severity: CRITICAL
  tier: 1
...
✅ 3 problemas auto-corrigidos:
   • Recriado .lmas/config.yaml (backup: .lmas/backups/config.yaml.1735564800)
   • Reiniciado servidor MCP context7
   • Adicionado .lmas/cache/ ao .gitignore
```
...
- id: PC-002
  ...
  guide:
    ...
    steps:
      ...
      - 'Verificar com: npx lmas task validate deploy'
...
Passos para resolver:
  ...
  4. Verificar com: npx lmas task validate deploy
...
neverAutoFix:
  files:
    ...
    - '**/*.{json,yaml,yml}' # Config do usuário (fora de .lmas/)
...
safeToAutoFix:
  paths:
    - '.lmas/**' # Arquivos de workspace do LMAS
    - '.lmas-core/**/*.yaml' # YAML do framework (cuidado)
