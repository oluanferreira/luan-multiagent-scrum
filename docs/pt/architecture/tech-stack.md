...
LMAS é construído em JavaScript/TypeScript moderno com runtime Node.js, otimizado para desenvolvimento de CLI cross-platform com UX interativa e capacidades de orquestração de agentes.
...
LMAS integra com servidores Model Context Protocol (MCP):
...
```bash
# Configuração LMAS
LMAS_DEBUG=true                    # Habilitar logging de debug
LMAS_CONFIG_PATH=/custom/path      # Localização customizada de config
LMAS_YOLO_MODE=true               # Forçar modo yolo
...
| 1.1    | 2025-12-14 | Atualizado aviso de migração para SynkraAI/lmas-core, semantic-release para v25.0.2 [Story 6.10] | Neo (dev)        |

---

_Este é um padrão oficial do framework LMAS. Todas as escolhas de tecnologia devem estar alinhadas com este stack._
