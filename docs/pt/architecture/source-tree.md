...
# Estrutura da Árvore de Código LMAS
...
- [Estrutura Atual (lmas-core Brownfield)](#estrutura-atual-lmas-core-brownfield)
...
LMAS utiliza uma **arquitetura de camada dupla**:

1. **Framework Core** (`.lmas-core/`) - Componentes portáteis do framework
...
## Estrutura Atual (lmas-core Brownfield)

```
lmas-core/                             # Raiz (projeto brownfield)
├── .lmas-core/                        # Core do framework (portátil)
...
│
├── bin/                               # Executáveis CLI
│   ├── lmas-core.js              # Ponto de entrada principal da CLI
│   └── lmas-minimal.js                # CLI mínima
│
...
**Propósito:** Componentes portáteis do framework que funcionam em qualquer projeto LMAS.
...
Squads são extensões modulares que adicionam capacidades especializadas ao LMAS. Diferente dos Squads descontinuados, Squads seguem uma estrutura de template padronizada.
...
│   └── cli/                           # Interface CLI
│
├── .lmas-core/                        # Ativos do framework (estrutura atual)
│   ├── agents/
│   ├── tasks/
│   ├── templates/
│   └── ...
...
Localização: .lmas-core/agents/{agent-name}.md
Exemplo: .lmas-core/agents/security-expert.md

# Estou criando uma nova tarefa:
Localização: .lmas-core/tasks/{task-name}.md
Exemplo: .lmas-core/tasks/deploy-to-production.md

# Estou criando um novo workflow:
Localização: .lmas-core/workflows/{workflow-name}.yaml
Exemplo: .lmas-core/workflows/continuous-deployment.yaml

# Estou criando um novo template:
Localização: .lmas-core/product/templates/{template-name}-tmpl.{yaml|md}
Exemplo: .lmas-core/product/templates/deployment-plan-tmpl.yaml

# Estou escrevendo uma story:
Localização: docs/stories/{epic-context}/{story-file}.md
Exemplo: docs/stories/lmas migration/story-6.1.2.6.md
...
Localização: .lmas-core/utils/{utility-name}.js
Exemplo: .lmas-core/utils/performance-monitor.js
