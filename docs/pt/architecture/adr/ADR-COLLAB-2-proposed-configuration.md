...
**Data:** 2025-12-30
**Status:** Proposto
**Autor:** @devops (Operator) + @architect (Architect)

---

## Contexto

Seguindo a [Auditoria do Estado Atual](./ADR-COLLAB-1-current-state-audit.md), este documento propõe alterações de configuração específicas para habilitar contribuições externas seguras ao LMAS.
...
# Configuração CodeRabbit do LMAS
# Story: COLLAB-1
...
reviews:
  ...
  path_instructions:
    ...
    '.lmas-core/development/agents/**':
      - 'Verificar se agente segue estrutura YAML de agente LMAS (persona_profile, commands, dependencies)'
    ...
    '.lmas-core/development/tasks/**':
      - 'Verificar se tarefa segue formato de tarefa LMAS com pontos de elicitação claros'
    ...
    '.lmas-core/product/templates/**':
      - 'Garantir que template segue convenções de template LMAS'
    ...
    '**/*.js':
      ...
      - 'Garantir que código segue padrões de codificação LMAS'
...
- `@LMAS/maintainers` - Maintainers gerais (acesso de escrita)
- `@LMAS/core-team` - Desenvolvedores core do framework
- `@LMAS/devops` - CI/CD e infraestrutura
...
- [ ] Agente segue estrutura YAML de agente LMAS
...
- [ ] Tarefa segue formato de tarefa LMAS
