...
Este documento define limites claros de responsabilidade para todos os agentes LMAS, com foco particular em:
...
1. **Git Pre-Push Hook** (Aplicação Primária)
   ...
   - Verificações: Variável de ambiente `$LMAS_ACTIVE_AGENT`
   - Ação: Bloqueia push se agent != "github-devops"

2. **Variáveis de Ambiente** (Detecção em Runtime)
   ```bash
   export LMAS_ACTIVE_AGENT="github-devops"
   export LMAS_GIT_PUSH_ALLOWED="true"
   ```
...
- [ ] **Criar Git Pre-Push Hook**
  - Localização: `.git/hooks/pre-push`
  - Conteúdo: Verificar `$LMAS_ACTIVE_AGENT`, bloquear se != "github-devops"
...
- [ ] **Atualizar Scripts de Ativação de Agentes**
  - Adicionar configuração de variável de ambiente: `LMAS_ACTIVE_AGENT={agent_id}`
  - Configurar `LMAS_GIT_PUSH_ALLOWED` apropriadamente
