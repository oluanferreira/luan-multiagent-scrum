# 🏛️ LMAS Dashboard - Arquitetura Completa
...
O LMAS Dashboard é uma aplicação web Next.js que fornece uma interface visual para monitorar e gerenciar o sistema LMAS. Ele se comunica com o CLI/LMAS através de arquivos de status no filesystem e Server-Sent Events (SSE).

### Princípios Arquiteturais

1. **CLI-First**: Dashboard é complementar ao CLI, não substituto
2. **File-Based Communication**: Status via `.lmas/dashboard/status.json`
...
│   │   │   ├── status/route.ts       # LMAS status polling
...
│   │   ├── use-lmas-status.ts        # Status with SWR
...
| ---------------- | ------------------------- | -------------------------------- |
| `story-store`    | `lmas-stories`            | `storyOrder` (ordem das colunas) |
| `ui-store`       | `lmas-ui`                 | `sidebarCollapsed`, `activeView` |
| `projects-store` | `lmas-projects`           | `projects`, `activeProjectId`    |
| `settings-store` | `lmas-dashboard-settings` | Todo o objeto `settings`         |
...
```typescript
// Retorna status atual do LMAS
// Lê de: .lmas/dashboard/status.json
...
└──────┬──────┘                              └────────┬─────────┘
       ...
       │  1. Agent ativado                            │
       │  ─────────────────────────────────────────▶  │
       │     Escreve .lmas/dashboard/status.json      │
       │                                              │
       │                                              │ 2. Dashboard detecta
       │                                              │    (SSE poll 2s)
       │                                              │
...
_Documentação gerada por @architect (Architect) - LMAS Core v2.0_
