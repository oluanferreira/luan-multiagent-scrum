...
**Propósito:** Verificar se os arquivos do framework LMAS estão corretamente configurados e consistentes.

### Verificações

| ID     | Nome                           | Severidade | Nível | Modo  | Timeout | Descrição                                            |
| ------ | ------------------------------ | ---------- | ----- | ----- | ------- | ---------------------------------------------------- |
| PC-001 | Config existe                  | CRITICAL   | 1     | quick | 100ms   | `.lmas/config.yaml` existe e é YAML válido           |
...
| PC-006 | Sem arquivos órfãos            | LOW        | 3     | full  | 5s      | Todos os arquivos em `.lmas-core/` são referenciados |
...
class ConfigExistsCheck extends BaseCheck {
  ...
  async check(context) {
    ...
    if (!(await fs.pathExists(configPath))) {
      return {
        passed: false,
        message: '.lmas/config.yaml não encontrado',
...
const requiredSections = [
  'Project Overview', // ou 'LMAS-FULLSTACK Development Rules'
...
Usuários podem definir verificações específicas do projeto em `.lmas/custom-checks.yaml`:
