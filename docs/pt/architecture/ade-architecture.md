...
O **Motor de Desenvolvimento Autônomo (ADE)** é a infraestrutura do LMAS para workflows de desenvolvimento autônomo. Ele permite que agentes de IA trabalhem de forma independente através de pipelines inteligentes, loops auto-recuperáveis e aprendizado persistente.
...
Todo o estado do ADE é persistido em `.lmas/` para:
...
Arquivo de Estado: .lmas/project-status.yaml
```
...
| ------------------- | ---------------------------------- | --------------------------------- |
| Padrões de Código   | Padrões reutilizáveis da base     | .lmas/patterns/code-patterns.json |
| Gotchas             | Problemas conhecidos e soluções   | .lmas/patterns/gotchas.json       |
| Insights de Sessão  | Descobertas durante sessões       | .lmas/sessions/                   |
| Mapa de Codebase    | Análise de estrutura do projeto   | .lmas/codebase-map.json           |
...
ADE integra-se com agentes LMAS através de:
...
Todos os componentes do ADE atualizam `.lmas/status.json`:
...
Localizada em `.lmas-core/core-config.yaml`:
...
_Este é um padrão oficial do framework LMAS documentando o Motor de Desenvolvimento Autônomo._
