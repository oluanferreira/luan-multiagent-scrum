...
This document explains how to set up and use the cryptographic signing system for LMAS-Core install manifests.

## Overview

LMAS-Core uses **Ed25519 digital signatures** (via minisign format) to verify the integrity and authenticity of the `install-manifest.yaml` file. This ensures that:
...
# Output:
#   lmas-core.pub  - PUBLIC key (safe to share, will be hardcoded)
#   lmas-core.key  - SECRET key (NEVER share, store securely)
```
...
const PINNED_PUBLIC_KEY = {
  ...
  keyId: 'LMAS0001',
  // Base64 public key (the second line of lmas-core.pub)
...
- Store `lmas-core.key` in a secure location (password manager, HSM, etc.)
...
1. **Generate/Update Manifest**
   ...
   # Creates .lmas-core/install-manifest.yaml with all file hashes
   ```
