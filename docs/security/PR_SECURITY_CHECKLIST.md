...
| `bin/lmas-init.js`                               | **Modified** | Added `requireSignature` option          |
...
1. Generate a key pair: `minisign -G -p lmas-core.pub -s lmas-core.key`
2. Update `PINNED_PUBLIC_KEY` in `manifest-signature.js` with the real public key
3. Sign the manifest: `minisign -Sm install-manifest.yaml -s lmas-core.key`
