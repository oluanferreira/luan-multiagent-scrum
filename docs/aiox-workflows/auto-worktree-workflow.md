...
flowchart TB
    ...
    PF3 -->|Existe| PF4
    PF3 -->|Não existe| ERR3[Erro: LMAS incompleto]
...
graph TB
    ...
    subgraph FS["File System"]
        WT[.lmas/worktrees/{storyId}/<br/>Worktree Directory]
        LOG[.lmas/logs/merges/<br/>Merge Audit Logs]
    end

    subgraph AGENTS["Agentes"]
        DEV[@dev<br/>Operator - DevOps]
...
**Estrutura Criada:**
```
.lmas/worktrees/{storyId}/     # Diretório de trabalho
...
```bash
git worktree add .lmas/worktrees/{storyId} -b auto-claude/{storyId}
```
...
```bash
LMAS_WORKTREE=/path/to/.lmas/worktrees/{storyId}
LMAS_STORY={storyId}
```
...
| Propriedade | Valor |
|-------------|-------|
| **Localização** | `.lmas-core/development/tasks/create-worktree.md` |
| **Agente** | @devops (Operator) |
| **Versão** | 1.0 |
| **Story** | 1.3 |
...
| **LMAS Core** | Instalado | Verificar `.lmas-core/` |
...
interface WorktreeInfo {
  ...
  path: string;              // '/abs/path/.lmas/worktrees/STORY-42'
...
| `WorktreeManager not found` | LMAS incompleto | Reinstalar LMAS |
...
ls .lmas-core/infrastructure/scripts/worktree-manager.js
```
...
3. Aumentar limite (se necessário) em `.lmas/config.yaml`
...
3. Remocao manual:
   ```bash
   git worktree remove .lmas/worktrees/{storyId} --force
...
| Arquivo | Caminho |
|---------|---------|
| **Workflow Definition** | `.lmas-core/development/workflows/auto-worktree.yaml` |
| **Task Create** | `.lmas-core/development/tasks/create-worktree.md` |
| **WorktreeManager** | `.lmas-core/infrastructure/scripts/worktree-manager.js` |
...
|--------|------|-------|------------|
| 1.0 | 2026-01-28 | @architect (Architect) | Versão inicial |

---

*Documentação gerada automaticamente pelo LMAS-FULLSTACK*
