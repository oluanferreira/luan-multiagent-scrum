...
> Source: `.lmas-core/development/scripts/unified-activation-pipeline.js` (Story ACT-6)
> Previous source: `.lmas-core/development/scripts/greeting-builder.js` (949 lines)

## Overview

Every LMAS agent goes through a **single unified activation pipeline** before presenting its greeting. As of Story ACT-6, the previous two-path architecture (Path A: direct GreetingBuilder invocation, Path B: generate-greeting.js CLI wrapper) has been consolidated into one entry point.
...
Current source:
- `.lmas-core/development/scripts/activation-runtime.js`
- `.lmas-core/development/scripts/unified-activation-pipeline.js`
...
```
.lmas-core/development/agents/{agent-id}.md
```
...
sequenceDiagram
    ...
    alt Cache hit
        ...
        ACL->>FS: readFile(.lmas-core/development/agents/{id}.md)
...
**Cache file:** `.lmas/project-status.yaml`
...
Reads from `.lmas-core/core-config.yaml` path `agentIdentity.greeting.preference`.
...
await mode.load();  // Reads .lmas/config.yaml -> permissions.mode
...
1. Reads current mode from `.lmas/config.yaml`
...
```javascript
const { enforcePermission } = require('./.lmas-core/core/permissions');
...
The `environment-bootstrap` task initializes `.lmas/config.yaml` with `permissions.mode: ask` as the default. If the config file is missing or the field is absent, the system defaults to `ask` mode.
...
Each agent has specific config requirements defined in `.lmas-core/data/agent-config-requirements.yaml`:

| Agent | Config Sections | Files Loaded | Performance Target |
|-------|----------------|--------------|-------------------|
| `lmas-master` | dataLocation, registry | lmas-kb.md (lazy) | <30ms |
...
| File | Loader | Purpose |
|------|--------|---------|
| `.lmas-core/development/agents/{agent-id}.md` | AgentConfigLoader | Agent definition |
| `.lmas-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| `.lmas-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Per-agent config needs |
| `.lmas-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |

### Loaded conditionally

| File | Condition | Loader |
|------|-----------|--------|
| `.lmas/session-state.json` | Path B (CLI wrapper) or file-based session detection | ContextDetector / SessionContextLoader |
| `.lmas/project-status.yaml` | Cache check (60s TTL) | ProjectStatusLoader |
...
graph TD
    ...
    SCL --> SSF[.lmas/session-state.json]
    PSL --> GIT[git CLI commands]
    PSL --> STORIES[docs/stories/**/*.md]
    PSL --> WTM[WorktreeManager]
    PSL --> PSC[.lmas/project-status.yaml]
    GCD_R --> GIT
    WN --> WP[.lmas-core/data/workflow-patterns.yaml]
    GPM --> CC
```
...
The `user_profile` setting (`bob` or `advanced`) affects behavior across the entire LMAS pipeline. This section documents every file that references `user_profile`/`userProfile` and the behavioral difference between modes.
...
|---|------|----------|----------------|---------------------|
| 1 | `.lmas-core/core-config.yaml` | Config | `user_profile: bob` | `user_profile: advanced` |
| 2 | `.lmas-core/development/scripts/greeting-builder.js` | Greeting | Redirects non-PM agents to @pm; hides role/status sections; returns empty commands for non-PM | Full contextual greeting with all sections and commands |
| 3 | `.lmas-core/development/scripts/generate-greeting.js` | Greeting | Uses GreetingBuilder, same bob restrictions | Uses GreetingBuilder, full features |
| 4 | `.lmas-core/development/scripts/greeting-preference-manager.js` | Greeting | Forces preference to `minimal` or `named`; overrides `auto`/`archetypal` | All 4 preferences available (`auto`, `minimal`, `named`, `archetypal`) |
| 5 | `.lmas-core/infrastructure/scripts/validate-user-profile.js` | Validation | Validates `bob` as legal value; normalizes case | Validates `advanced` as legal value; normalizes case |
| 6 | `.lmas-core/core/config/config-resolver.js` | Config | `toggleUserProfile()` switches bob<->advanced; L5 user layer has priority | Same toggle; resolveConfig merges layers |
| 7 | `.lmas-core/core/config/migrate-config.js` | Config | Categorizes `user_profile` as USER_FIELD during migration | Same categorization |
| 8 | `.lmas-core/core/config/schemas/user-config.schema.json` | Schema | `enum: ["bob", "advanced"]` validation | Same validation |
| 9 | `.lmas-core/core/config/templates/user-config.yaml` | Template | Default template value: `bob` | N/A (template default is bob) |
| 10 | `.lmas-core/development/agents/pm.md` | Agent | PM becomes sole orchestrator; bob mode session detection; orchestrates other agents internally | PM operates as normal PM with standard workflow |
...
| 16 | `packages/lmas-install/src/installer.js` | Install | Sets `config.user_profile = 'bob'` in YAML | Sets `config.user_profile = 'advanced'` |
| 17 | `.lmas-core/development/tasks/environment-bootstrap.md` | Task | Documents bob selection flow | Documents advanced selection flow |
| 18 | `docs/lmas-workflows/bob-orchestrator-workflow.md` | Docs | Full bob orchestrator workflow documentation | N/A (bob-specific doc) |
...
| `@lmas-master` | 0 (uses string visibility) | Empty (redirect to @pm) | Fallback: first 12 commands |

**Note:** Agents with 0 `key` commands (`qa`, `data-engineer`, `devops`, `ux-design-expert`, `lmas-master`) lack `visibility` array metadata on their commands. In `advanced` mode `workflow` sessions, they fall back to showing first 12 commands. This is a known gap tracked for future improvement.
