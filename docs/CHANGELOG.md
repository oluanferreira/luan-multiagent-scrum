...
- **🤖 LMAS Autonomous Development Engine (ADE)**: Complete autonomous development system with 7 Epics:
...
- **Agent Sync**: All agents now synced between `.lmas-core/development/agents/` and `.claude/commands/LMAS/agents/`
...
- **Directory Structure**: Renamed `.legacy-core/` to `.lmas-core/`
...
- **IDE Configuration**: Claude Code agents moved to `.claude/commands/LMAS/agents/`
- **File Locations**:
...
- Manual installation process (use `npx lmas-core install` instead)
...
1. Directory renamed: `.legacy-core/` → `.lmas-core/`
