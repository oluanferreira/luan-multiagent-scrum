...
Reference documentation for the LMAS layered config system (ADR-PRO-002).
...
```
L1 Framework (.lmas-core/framework-config.yaml)   — Read-only, shipped with npm
  ↓ deep merge
L2 Project (.lmas-core/project-config.yaml)        — Team-shared, committed
  ↓ deep merge
Pro Extension (pro/pro-config.yaml)                 — Optional Pro submodule
  ↓ deep merge
L3 App ({appDir}/lmas-app.config.yaml)             — Monorepo app-specific
  ↓ deep merge
L4 Local (.lmas-core/local-config.yaml)            — Machine-specific, gitignored
  ↓ deep merge
L5 User (~/.lmas/user-config.yaml)                 — Cross-project user prefs
```
...
|-----|------|---------|---------------|-------------|
| `metadata.name` | string | `"Synkra LMAS"` | Never | Framework name |
| `metadata.framework_version` | string | `"4.0.0"` | Never | Framework version |
| `markdownExploder` | boolean | `true` | L2+ | Enable markdown exploder |
| `resource_locations.agents_dir` | string | `.lmas-core/development/agents` | L2+ | Agent definitions path |
| `resource_locations.tasks_dir` | string | `.lmas-core/development/tasks` | L2+ | Task definitions path |
| `resource_locations.templates_dir` | string | `.lmas-core/development/templates` | L2+ | Templates path |
| `resource_locations.checklists_dir` | string | `.lmas-core/development/checklists` | L2+ | Checklists path |
| `resource_locations.tools_dir` | string | `.lmas-core/tools` | L2+ | Tools path |
| `resource_locations.scripts.*` | string | various | L2+ | Script paths by category |
| `resource_locations.data_dir` | string | `.lmas-core/data` | L2+ | Data directory |
...
|-----|------|-------------|
| `project.type` | string | `EXISTING_LMAS`, `NEW_PROJECT`, `BROWNFIELD` |
...
### L5 User Keys (~/.lmas/user-config.yaml)
...
Schema files: `.lmas-core/core/config/schemas/`
