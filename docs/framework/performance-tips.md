...
### Why it matters for LMAS

The LMAS Unified Activation Pipeline runs `git status` during agent activation to detect modified files and project state. On large repositories, this can cause the `projectStatus` loader to timeout (~60% timeout rate observed). Enabling fsmonitor dramatically reduces this latency.
...
Run `lmas doctor` to check fsmonitor status:
...
_This is an official LMAS framework standard._
