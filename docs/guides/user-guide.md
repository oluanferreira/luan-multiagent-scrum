...
Complete guide for using LMAS - the AI-Orchestrated System for Full Stack Development.
...
Before using LMAS, ensure you have:
...
LMAS provides orchestrated structure while allowing flexibility in communication. This means:
...
LMAS includes 11 specialized agents, each with a distinct role and personality:
...
@dev                # Activate Neo (Developer)
@qa                 # Activate Oracle (QA)
@architect          # Activate Aria (Architect)
@lmas-master        # Activate Keymaker (Orchestrator)
...
Tasks are the primary entry point in LMAS. Everything is a task.
...
Squads are modular teams of AI agents that extend LMAS functionality.
...
```
my-project/
├── .lmas-core/                # Framework configuration
...
The primary configuration is in `.lmas-core/core/config/`:
...
LMAS supports multiple IDEs. Configuration is synchronized across:
...
LMAS implements 3-layer quality gates:
