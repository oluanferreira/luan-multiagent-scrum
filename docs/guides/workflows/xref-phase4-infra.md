...
**Scope:** `.lmas-core/infrastructure/` -- scripts, integrations, tools, templates, tests
...
These scripts are imported in `.lmas-core/infrastructure/index.js` and exposed as the module's public API.
...
| 7 | `scripts/status-mapper.js` | PM Integration | Maps story status between LMAS and PM tools | -- | clickup-helpers, tests/clickup, tasks | No |
...
| 9 | `scripts/template-engine.js` | Template/Generation | Template rendering engine (Handlebars-like) | `fs`, `yaml` | component-generator, template-validator, tasks, lmas-core index | No |
| 10 | `scripts/component-generator.js` | Template/Generation | LMAS component generation (agents, tasks, templates) | template-engine, yaml-validator, security-checker, transaction-manager | tasks (15+ tasks), lmas-core index | No |
| 11 | `scripts/component-metadata.js` | Template/Generation | Metadata management for LMAS components | `fs`, `yaml` | component-generator, transaction-manager, tasks/modify-agent | No |
| 12 | `scripts/component-search.js` | Template/Generation | Search across LMAS components | `fs` | lmas-core index, tasks (analyze-framework, qa-generate-tests) | No |
| 13 | `scripts/batch-creator.js` | Template/Generation | Batch creation of LMAS components | component-generator, dependency-analyzer, transaction-manager | lmas-core index, tasks/cleanup-utilities | No |
| 14 | `scripts/lmas-validator.js` | Validation | Validates LMAS component structure and schema | `fs`, `yaml` | CI (ci.yml), installer tests, README, git-workflow-guide | No |
...
| 18 | `scripts/dependency-analyzer.js` | Analysis | Analyzes component dependencies | `fs` | batch-creator, modification-validator, lmas-core index, tasks | No |
...
| 22 | `scripts/security-checker.js` | Analysis | Security validation for components | -- | component-generator, elicitation-engine, lmas-master agent | No |
...
| 42 | `scripts/usage-analytics.js` | Utilities | Analytics for LMAS feature usage | -- | tasks/analyze-framework | No |
...
| 48 | `scripts/repository-detector.js` | System | Detects repository type and structure | -- | lmas-init.js, agent devops, tasks | No |
...
| 53 | `scripts/yaml-validator.js` | Config | YAML file validation | -- | component-generator, modification-validator, lmas-master agent | No |
...
| 2 | `scripts/asset-inventory.js` | Analysis (Epic 2) | Creates comprehensive inventory of LMAS assets | `fs`, `yaml` | agent devops, many docs, install-manifest | No |
...
| # | File | Category | Purpose | Orphan? |
|---|------|----------|---------|---------|
| 1 | `templates/gitignore/gitignore-lmas-base.tmpl` | Gitignore | Base LMAS gitignore template | No |
...
| 2 | `templates/lmas-sync.yaml.template` | IDE Sync | LMAS sync configuration template | No |
...
| **Validation** | 6 | LMAS validator, template validator, output pattern, spot check, agent validator, yaml validator |
...
The infrastructure module is the **base layer** of the LMAS modular architecture:
...
- `infrastructure/` has **NO** dependencies on other LMAS modules
...
*Generated as part of LMAS cross-reference analysis, Phase 4.*
