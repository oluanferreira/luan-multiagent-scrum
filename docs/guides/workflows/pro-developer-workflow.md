# LMAS Pro Developer Workflow

This guide covers the two developer workflows for working with the LMAS Open Core architecture.

## Architecture Overview

LMAS uses a dual-repository Open Core model:

- **lmas-core** (public) — Open-source framework, available to everyone
- **lmas-pro** (private) — Premium features, available to team members and licensees

The `pro/` directory in lmas-core is a git submodule pointing to the lmas-pro repository.
...
Team members with access to the private lmas-pro repository can work on both repos.
...
2. Push `lmas-core` changes second: `cd .. && git push`

This ensures the submodule pointer in lmas-core references a valid commit in lmas-pro.
...
if (isProAvailable()) {
  console.log('LMAS Pro v' + getProVersion() + ' detected');
...
|------------|----------|-------|---------|
| **lmas-core** | Without submodules | Core-only (pro/ absent) | npm (excludes pro/) |
| **lmas-pro** | With lmas-core cloned | Integration (pro/ symlinked) | GitHub Packages |
...
- [Story PRO-5: Repository Bootstrap](../../stories/epics/epic-pro-lmas-pro-architecture/story-pro-5-repo-bootstrap.md)
