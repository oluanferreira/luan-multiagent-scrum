# LMAS Cross-Reference Phase 6: Supporting Systems Analysis

> **Generated:** 2026-02-05
> **Scope:** All supporting systems in `.lmas-core/`
...
**Location:** `.lmas-core/workflow-intelligence/`
**Total Files:** 19
**Purpose:** Provides intelligent workflow analysis, suggestions, pattern learning, and confidence scoring for the LMAS agent orchestration system.
...
| `registry/workflow-registry.js` | Central registry of all available workflows | `index.js`, `suggestion-engine.js` | Loads from `.lmas-core/development/workflows/` | No |
...
**Location:** `.lmas-core/monitor/hooks/`
**Total Files:** 10
...
**Location:** `.lmas-core/core/quality-gates/` (10 files) and `.lmas-core/quality/` (4 files)
**Total Files:** 14
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `metrics-collector.js` | Collects and aggregates quality metrics across layers | `metrics-hook.js`, CLI metrics commands | Stores to `.lmas/data/quality-metrics.json` | No |
...
| `seed-metrics.js` | Seeds initial demo metrics data | CLI `lmas metrics seed` command | Uses `MetricsCollector` | No |
...
**Location:** `.lmas-core/cli/commands/`
**Total Files:** 37 (across 8 command groups)
**Purpose:** Commander.js-based CLI commands for the `lmas` executable. Provides the CLI-first interface for all framework operations.

### Architecture

Commands are organized by domain into subdirectories, each with an `index.js` entry point. Registered in the main CLI (`cli/index.js`) and invoked via `bin/lmas.js`.
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `config/index.js` | Config hierarchy management: show, diff, migrate, validate, init-local | `cli/index.js`, `bin/lmas.js` | Uses `core/config/config-resolver.js`, `merge-utils.js`, `env-interpolator.js` | No |
...
| File | Purpose | Consumers | Cross-refs | Orphan? |
|------|---------|-----------|------------|---------|
| `validate/index.js` | Validate LMAS installation integrity | `cli/index.js` | Uses `install-manifest.yaml` | No |

### External References (20+ files reference CLI commands)

- **Main CLI:** `cli/index.js` registers all command groups
- **Entry point:** `bin/lmas.js` creates the Commander program
...
**Location:** `.lmas-core/agent-teams/`
**Total Files:** 0 (directory does not exist or is empty)
**Purpose:** Was intended to hold agent team configurations.

### Status: DOES NOT EXIST

The `agent-teams/` directory was not found in the `.lmas-core/` directory. However, references to "agent-teams" exist in:
...
The `tools/` directory exists (referenced in `core-config.yaml` as `toolsLocation: .lmas-core/tools`) but contains no files. Tool configurations may have been:
...
- `core-config.yaml`: `toolsLocation: .lmas-core/tools`
- `framework-config.yaml`: `tools_dir: ".lmas-core/tools"`
...
**Location:** `.lmas-core/manifests/`
**Total Files:** 4
**Purpose:** CSV-based registry of all agents, tasks, and workers with JSON schema validation. Acts as the central inventory of all LMAS components.
...
**Location:** `.lmas-core/core/elicitation/` (5 files) and `.lmas-core/elicitation/` (3 files)
**Total Files:** 8
...
| `session-manager.js` | Manages elicitation session persistence (save/load/resume) | `elicitation-engine.js` | Stores sessions in `.lmas-sessions/` | No |
...
- **Core config:** `core-config.yaml` has `elicitationLocation: .lmas-core/elicitation`
- **Framework config:** `framework-config.yaml` has `elicitation_dir: ".lmas-core/elicitation"`
...
**Location:** `.lmas-core/docs/standards/`
**Total Files:** 16
...
| `LMAS-LIVRO-DE-OURO-V2.1-COMPLETE.md` | **PRIMARY** -- Complete v4.0.4 framework guide (consolidated) | All agents, onboarding | Supersedes all legacy Livro docs | No |
...
| `LMAS-COLOR-PALETTE-V2.1.md` | Complete color system for LMAS branding | Dashboard, UI components | Referenced by UX agent | No |
| `LMAS-COLOR-PALETTE-QUICK-REFERENCE.md` | Quick reference for color palette | Developers, designers | Subset of full palette doc | No |
...
| `LMAS-LIVRO-DE-OURO.md` | v2.0 base document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `LMAS-LIVRO-DE-OURO-V2.1.md` | v4.0.4 delta document (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `LMAS-LIVRO-DE-OURO-V2.1-SUMMARY.md` | v4.0.4 summary (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
| `LMAS-LIVRO-DE-OURO-V2.2-SUMMARY.md` | Future v2.2 planning (DRAFT) | Planning only | Not yet active | No (Draft) |
| `LMAS-FRAMEWORK-MASTER.md` | v2.0 framework doc (DEPRECATED) | Historical reference | Superseded by V2.1-COMPLETE | Yes (Legacy) |
...
4 files are explicitly deprecated (superseded by `LMAS-LIVRO-DE-OURO-V2.1-COMPLETE.md`), 1 is an archive candidate. These should be cleaned up or moved to an `archive/` directory.

---

## 11. Processes System

**Location:** `.lmas-core/processes/`
**Total Files:** 0 (directory does not exist)
**Purpose:** Was intended to hold process definitions.

### Status: DOES NOT EXIST

No `processes/` directory was found under `.lmas-core/`. This may be:
...
No references to `.lmas-core/processes/` were found in the codebase.

---

## 12. Root Config System

**Location:** `.lmas-core/` (root-level files)
**Total Files:** 8
...
| 13 | Root Config | `.lmas-core/` root | 8 | Active | In migration (monolithic to layered) |
...
|------|--------|--------|----------------|
| `docs/standards/LMAS-LIVRO-DE-OURO.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/LMAS-LIVRO-DE-OURO-V2.1.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/LMAS-LIVRO-DE-OURO-V2.1-SUMMARY.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
| `docs/standards/LMAS-FRAMEWORK-MASTER.md` | Docs/Standards | Superseded by V2.1-COMPLETE | Move to archive/ |
...
|-----------|---------------|--------|
| `.lmas-core/agent-teams/` | install-manifest, source-tree docs, core-architecture, data/lmas-kb.md | Directory does not exist |
| `.lmas-core/tools/` (content) | core-config.yaml, framework-config.yaml | Directory exists but is empty |
| `.lmas-core/processes/` | Not referenced | Does not exist |
...
2. **Manifests are the Registry:** `manifests/workers.csv` contains 204 entries covering all LMAS components. The CLI `workers` commands are the primary consumer.
