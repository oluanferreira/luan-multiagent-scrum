# LMAS Workflow / Task / Agent Cross-Reference Analysis
...
| Workflow | @lmas-master | @analyst | @architect | @data-eng | @dev | @devops | @pm | @po | @qa | @sm | @ux-expert | @squad-creator |
|----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
| 10 | `create-doc.md` | @pm, @architect, @ux-expert, @analyst, @lmas-master | GF, GU, GS, BU, BS, SP | `*create-prd`, `*create-*-architecture`, etc. |
| 11 | `shard-doc.md` | @po, @pm, @lmas-master | GF, GU, GS, BF, BU, BS | `*shard-doc` |
| 12 | `execute-checklist.md` | @po, @architect, @data-eng, @lmas-master | GF, GU, GS, BF, BU, BS | `*execute-checklist-po`, `*execute-checklist` |
| 13 | `document-project.md` | @architect, @analyst, @lmas-master | BD, BF, BU, BS | `*document-project` |
| 14 | `correct-course.md` | @pm, @sm, @lmas-master | (any - course correction) | `*correct-course` |
...
| 17 | `create-deep-research-prompt.md` | @analyst, @architect, @data-eng, @lmas-master | GF, GU, GS | `*research {topic}` |
...
#### @architect (Architect) - Holistic System Architect
...
#### @data-engineer (Dozer) - Database Architect
...
#### @devops (Operator) - DevOps Specialist
...
sequenceDiagram
    ...
    participant ARCH as @architect (Architect)
...
sequenceDiagram
    autonumber
    participant ARCH as @architect (Architect)
    participant DE as @data-engineer (Dozer)
...
sequenceDiagram
    ...
    participant DEVOPS as @devops (Operator)
...
| `@lmas-master` (Morpheus) | META-AGENT | Orchestrates other agents. Not a workflow participant but a workflow creator/executor. |
| `@devops` (Operator) | SERVICE ROLE | Provides infrastructure (bootstrap, push, worktrees) but doesn't follow workflow sequences. |
...
| `apply-qa-fixes.md` (`applyQaFixes()` by Neo) | `dev-apply-qa-fixes.md` (`devApplyQaFixes()` by Neo) | @dev → `apply-qa-fixes.md` | `qa-loop.yaml` → `dev-apply-qa-fixes.md` | **Mismatch!** Agent def ≠ YAML. Standardize. |
| `validate-next-story.md` (`validateNextStory()` by Oracle) | `dev-validate-next-story.md` (`devValidateNextStory()` by Neo) | @po/@dev → `validate-next-story.md` | (indirect) | Different tasks for different agents. Both valid. |
...
|-----|-------------|----------------|
| @data-engineer in Greenfield | Dozer has no role in any Greenfield workflow | Consider adding DB setup step if project has database |
...
The `checklists/` directory does NOT exist at `.lmas-core/development/checklists/`. Multiple agents reference checklists:
- `@dev` → `story-dod-checklist.md`
- `@po` → `po-master-checklist.md`, `change-checklist.md`

**Impact:** If agents resolve checklists via the standard `dependencies → checklists → .lmas-core/development/checklists/{name}` path, these will fail.
...
*Analysis generated by LMAS Cross-Reference Audit*
