...
sequenceDiagram
    autonumber
    participant User
    participant Architect as @architect (Architect)
    participant PM as @pm (Trinity)
    participant PO as @po (Keymaker)
    participant SM as @sm (Niobe)
    participant Analyst as @analyst (Atlas)
    participant Dev as @dev (Neo)
    participant QA as @qa (Oracle)
...
| Atributo | Valor |
|----------|-------|
| **Agente** | @analyst (Atlas) ou @pm (Trinity) |
...
**Referencia:** `.lmas-core/data/lmas-kb.md#IDE Development Workflow`
...
graph LR
    ...
    subgraph "Agentes de Gestao"
        PO["@po<br/>Keymaker<br/>Product Owner"]
        SM["@sm<br/>River<br/>Scrum Master"]
    end

    subgraph "Agentes de Execucao"
        Dev["@dev<br/>Neo<br/>Full Stack Developer"]
        QA["@qa<br/>Oracle<br/>Test Architect"]
    end
...
| @po | Keymaker | Product Owner | Validacao de artefatos, fragmentacao de docs, retrospectiva |
| @sm | River | Scrum Master | Criacao de stories |
| @analyst | Atlas | Business Analyst | Revisao opcional de story drafts |
| @dev | Neo | Full Stack Developer | Implementacao de stories, correcao de feedback |
| @qa | Oracle | Test Architect | Revisao de implementacao, quality gates |
...
4. **Templates Disponiveis**
   - `.lmas-core/development/templates/brownfield-prd-tmpl.yaml`
   - `.lmas-core/development/templates/brownfield-architecture-tmpl.yaml`
   - `.lmas-core/development/templates/story-tmpl.yaml`

5. **Checklists Disponiveis**
   - `.lmas-core/development/checklists/po-master-checklist.md`
   - `.lmas-core/development/checklists/story-draft-checklist.md`
   - `.lmas-core/development/checklists/story-dod-checklist.md`
...
|----------|-----------------|--------------|
| PRD incompleto | @pm (Trinity) | @po (Keymaker) |
| Arquitetura inconsistente | @architect (Architect) | @pm (Trinity) |
| Story ambigua | @sm (Niobe) | @po (Keymaker) |
| Implementacao com bugs | @dev (Neo) | @qa (Oracle) |
| Quality gate falha | @qa (Oracle) | @po (Keymaker) |
| Integracao quebrada | @architect (Architect) | @devops (Operator) |
...
|---------|-----------|---------|
| Workflow Definition | Definicao YAML do workflow | `.lmas-core/development/workflows/brownfield-service.yaml` |
| PRD Template | Template para brownfield PRD | `.lmas-core/development/templates/brownfield-prd-tmpl.yaml` |
| Architecture Template | Template para arquitetura | `.lmas-core/development/templates/brownfield-architecture-tmpl.yaml` |
| Story Template | Template para stories | `.lmas-core/development/templates/story-tmpl.yaml` |
| PO Master Checklist | Checklist de validacao | `.lmas-core/development/checklists/po-master-checklist.md` |
| Story Draft Checklist | Checklist de story | `.lmas-core/development/checklists/story-draft-checklist.md` |
| Story DoD Checklist | Definition of Done | `.lmas-core/development/checklists/story-dod-checklist.md` |

### Agentes

| Agente | Arquivo | Caminho |
|--------|---------|---------|
| @architect | Aria | `.lmas-core/development/agents/architect.md` |
| @pm | Morgan | `.lmas-core/development/agents/pm.md` |
| @po | Keymaker | `.lmas-core/development/agents/po.md` |
| @sm | River | `.lmas-core/development/agents/sm.md` |
| @analyst | Atlas | `.lmas-core/development/agents/analyst.md` |
| @dev | Neo | `.lmas-core/development/agents/dev.md` |
| @qa | Oracle | `.lmas-core/development/agents/qa.md` |

### Documentacao Relacionada

- [LMAS Knowledge Base](../../../.lmas-core/data/lmas-kb.md) - Base de conhecimento do framework
- [Technical Preferences](../../../.lmas-core/development/data/technical-preferences.md) - Preferencias tecnicas do projeto
- [IDE Development Workflow](../../../.lmas-core/data/lmas-kb.md#IDE-Development-Workflow) - Fluxo de desenvolvimento no IDE
...
*Documento gerado a partir do workflow `brownfield-service.yaml` - LMAS Framework v2.2*
