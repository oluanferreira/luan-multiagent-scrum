...
**Scope:** `.lmas-core/core/` -- 130 JavaScript files across 14 subsystems
...
| File | Purpose | Internal Deps | External Consumers | Orphan? |
|------|---------|---------------|-------------------|---------|
| `elicitation-engine.js` | Core interactive elicitation engine (inquirer-based) | session-manager.js; optional: infrastructure/scripts/security-checker | core/index.js, .lmas-core/index.js, tests/core/, tests/security/ | No |
...
| `checks/project/` | 8 checks: package-json, node-version, dependencies, lmas-directory, agent-config, framework-config, task-definitions, workflow-dependencies | project/index.js | base-check.js |
...
| `global-config-manager.js` | Manages ~/.lmas/mcp/ global config | os-detector.js | config-migrator.js, CLI mcp/add, mcp/status, mcp/setup, mcp/link, tests/ | No |
...
| `skill-dispatcher.js` | Maps agent IDs to LMAS Skill invocations | None | workflow-orchestrator.js, orchestration/index.js | No |
...
| `layer2-pr-automation.js` | Layer 2: CodeRabbit + Oracle automation | base-layer.js | quality-gate-manager.js, tests/ | No |
...
| `build-registry.js` | Scans LMAS modules and builds registry JSON | None (glob) | **None found** (standalone script) | **Partial** -- used as CLI script |
...
| `context-loader.js` | Multi-agent session context loader | context-detector.js | core/index.js, .lmas-core/scripts/session-context-loader.js | No |
...
### 4.1 CLI Commands (`.lmas-core/cli/commands/`)

| CLI Command | Core Module(s) Used |
|-------------|-------------------|
| `lmas workers list/info/search-keyword/search-semantic` | `registry/registry-loader` |
| `lmas manifest validate` | `manifest/manifest-validator` |
| `lmas manifest regenerate` | `manifest/manifest-generator` |
| `lmas qa run/status` | `quality-gates/quality-gate-manager` |
| `lmas mcp add` | `mcp/global-config-manager`, `mcp/os-detector` |
| `lmas mcp status` | `mcp/global-config-manager`, `mcp/symlink-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `lmas mcp setup` | `mcp/global-config-manager`, `mcp/os-detector` |
| `lmas mcp link` | `mcp/symlink-manager`, `mcp/global-config-manager`, `mcp/os-detector`, `mcp/config-migrator` |
| `lmas config show/diff/migrate/validate/init-local` | `config/config-resolver`, `config/merge-utils`, `config/env-interpolator` |

### 4.2 Development Scripts (`.lmas-core/development/scripts/`)
...
### 4.3 Package Entry Point (`.lmas-core/index.js`)

| Entry Point | Core Module(s) Used |
|-------------|-------------------|
| `.lmas-core/index.js` (npm package) | `elicitation/elicitation-engine` |
...
| `../../infrastructure/scripts/security-checker` | elicitation/elicitation-engine.js | Exists outside core/ at `.lmas-core/infrastructure/scripts/` |
...
| elicitation | (none internally) | core/index, .lmas-core/index, development/scripts |
