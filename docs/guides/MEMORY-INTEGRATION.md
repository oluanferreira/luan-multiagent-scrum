...
The integration follows the LMAS Open Core model:
- **lmas-core:** Extension point in UnifiedActivationPipeline (this guide)
- **lmas-pro:** Memory intelligence implementation (retrieval, scoring, learning)
...
```javascript
isProAvailable() === true
// but .lmas/session-digests/ is empty
```
...
```javascript
isProAvailable() === true
// and .lmas/session-digests/ contains memory digests
```
...
const fs = require('fs');
const digests = fs.readdirSync('.lmas/session-digests');
...
**Solutions:**
1. Reduce digest count (archive old digests to `.lmas/session-digests/archive/`)
