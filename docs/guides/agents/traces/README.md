# LMAS Agent Execution Traces - Index

> **Story:** LMAS-TRACE-001 | **Traced from source code, not documentation.**

## Overview

This directory contains comprehensive runtime execution trace documentation for all 12 LMAS agents. Each document shows **exactly** what files are loaded, in what order, and why - from agent activation through every command execution.
...
| @dev | Neo (Builder) | 36 | Direct | [dev-execution-trace.md](./dev-execution-trace.md) |
| @qa | Oracle (Guardian) | 25 | Direct | [qa-execution-trace.md](./qa-execution-trace.md) |
| @lmas-master | Nova (Orchestrator) | 33 | Direct | [lmas-master-execution-trace.md](./lmas-master-execution-trace.md) |
| @devops | Operator (Pipeline) | 30 | CLI Wrapper | [devops-execution-trace.md](./devops-execution-trace.md) |
| @data-engineer | Dozer (Architect) | 27 | CLI Wrapper | [data-engineer-execution-trace.md](./data-engineer-execution-trace.md) |
| @ux-design-expert | Uma (Harmonizer) | 24 | CLI Wrapper | [ux-design-expert-execution-trace.md](./ux-design-expert-execution-trace.md) |
| @po | Keymaker (Balancer) | 17 | Direct | [po-execution-trace.md](./po-execution-trace.md) |
...
graph TD
    ...
    subgraph "Execution Layer"
        ...
        DEVOPS[@devops<br/>Operator]
    end

    subgraph "Specialist Layer"
        DE[@data-engineer<br/>Dozer]
...
|-------|----------|-----------------|--------------|-------------|
| @lmas-master | Critical | dataLocation, registry | lmas-kb.md (lazy) | <30ms |
...
Source: `.lmas-core/data/agent-config-requirements.yaml`
...
| File | Purpose | Lines |
|------|---------|-------|
| `.lmas-core/development/scripts/greeting-builder.js` | Main activation pipeline | 949 |
| `.lmas-core/development/scripts/agent-config-loader.js` | Config loading per agent | ~400 |
| `.lmas-core/development/scripts/generate-greeting.js` | CLI wrapper (3 agents) | 173 |
| `.lmas-core/core/session/context-detector.js` | Session type detection | ~100 |
| `.lmas-core/infrastructure/scripts/project-status-loader.js` | Git/project status | ~524 |
| `.lmas-core/infrastructure/scripts/git-config-detector.js` | Git config detection | ~294 |
| `.lmas-core/development/scripts/greeting-preference-manager.js` | Greeting level preference | ~146 |
| `.lmas-core/development/scripts/workflow-navigator.js` | Workflow suggestions | ~200 |
| `.lmas-core/core/permissions/index.js` | Permission mode badge | ~100 |
| `.lmas-core/data/agent-config-requirements.yaml` | Per-agent config requirements | 369 |
...
**Resolution:** The `AgentConfigLoader` resolves dependencies by searching multiple paths: `development/`, `product/`, and root `.lmas-core/` directories. The path mismatch does not cause runtime failures but is documented for accuracy.
...
| @lmas-master | add-tech-doc.md | `*add-tech-doc` command non-functional |
