# @lmas-master (Morpheus) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.lmas-core/development/agents/lmas-master.md`
...
|-------|------|--------|---------|
| 1 | `.lmas-core/development/agents/lmas-master.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.lmas-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.lmas-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: dataLocation, registry |
| 4 | `.lmas-core/data/workflow-patterns.yaml` | WorkflowNavigator._loadPatterns() | Workflow state detection |
| 5 | `.lmas/session-state.json` | ContextDetector._detectFromFile() | Session type detection (if no conversation history) |
| 6 | `.lmas/project-status.yaml` | ProjectStatusLoader.loadCache() | Cached project status (60s TTL) |

**NOT loaded on activation:**

| File | Condition | Size |
|------|-----------|------|
| `.lmas-core/data/lmas-kb.md` | ONLY when user types `*kb` | 35KB |
...
sequenceDiagram
    ...
    participant AMd as lmas-master.md
    ...
    CC->>ACL: loadAgentDefinition('lmas-master')
    ACL-->>CC: { agent: { name: 'Morpheus', icon: '👑' }, commands: [...33], ... }
...
| Data | Source | Value |
|------|--------|-------|
| Greeting level | `persona_profile.greeting_levels.archetypal` | `👑 Morpheus the Orchestrator ready to lead!` |
| Signature | `persona_profile.communication.signature_closing` | `— Morpheus, orquestrando o sistema 🎯` |
| Role | `persona.role` | Master Orchestrator, Framework Developer & LMAS Method Expert |
...
| `*list-components` | (built-in, scans .lmas-core/) | full | No |
...
**Task file:** `.lmas-core/development/tasks/kb-mode-interaction.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `kb-mode-interaction.md` | Task | EXISTS |
| `.lmas-core/data/lmas-kb.md` | Data | EXISTS (lazy, 35KB) |
...
flowchart TD
    ...
    B --> C[Load lmas-kb.md into context]
    C --> D[Toggle KB mode ON]
    D --> E[Agent now answers with full LMAS Method knowledge]
    E --> F[User types *kb again to toggle OFF]
```

**CRITICAL:** lmas-kb.md is NEVER loaded unless user explicitly types `*kb`.

---

### `*create`

**Task files:** `.lmas-core/development/tasks/create-agent.md`, `create-task.md`, `create-workflow.md`
...
| `agent-template.yaml` | Template | EXISTS (in .lmas-core/product/templates/) |
| `task-template.md` | Template | EXISTS (in .lmas-core/product/templates/) |
| `workflow-template.yaml` | Template | EXISTS (in .lmas-core/product/templates/) |
| `security-checker.js` | Util | EXISTS (in .lmas-core/infrastructure/scripts/) |
| `yaml-validator.js` | Util | EXISTS (in .lmas-core/infrastructure/scripts/) |
...
flowchart TD
    ...
    M --> N{Validation pass?}
    N -->|yes| O[Write component to .lmas-core/development/{type}/]
    N -->|no| P[Report validation errors]
    O --> Q[Update manifest if applicable]
```

**Expected output:** New LMAS component file written to appropriate directory.

---

### `*modify`

**Task files:** `.lmas-core/development/tasks/modify-agent.md`, `modify-task.md`, `modify-workflow.md`
...
**Task file:** `.lmas-core/development/tasks/update-manifest.md`
...
**Task file:** `.lmas-core/development/tasks/deprecate-component.md`
...
**Task file:** `.lmas-core/development/tasks/propose-modification.md`
...
**Task file:** `.lmas-core/development/tasks/undo-last.md`
...
**Task file:** `.lmas-core/development/tasks/validate-workflow.md`
...
| `yaml-validator.js` | Util | EXISTS |
| `.lmas-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task files:** `.lmas-core/development/tasks/run-workflow.md`, `run-workflow-engine.md`
**Template:** `.lmas-core/development/templates/subagent-step-prompt.md`
...
| `subagent-step-prompt.md` | Template | EXISTS (in .lmas-core/development/templates/) |
| `workflow-management.md` | Util | EXISTS (in .lmas-core/scripts/) |
| `.lmas-core/development/workflows/{name}.yaml` | Workflow | Dynamically loaded |
...
**Task file:** `.lmas-core/development/tasks/analyze-framework.md`
...
flowchart TD
    ...
    B --> C[Scan .lmas-core/ directory structure]
...
**Task file:** `.lmas-core/development/tasks/execute-checklist.md`

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `execute-checklist.md` | Task | EXISTS |
| `.lmas-core/product/checklists/{checklist}.md` | Checklist | Dynamically loaded |
...
flowchart TD
    ...
    B --> C{Name provided?}
    C -->|yes| D[Resolve checklist from .lmas-core/product/checklists/]
...
**Task file:** `.lmas-core/development/tasks/create-doc.md`
**Templates:** All 15 templates from `.lmas-core/product/templates/`
...
| `.lmas-core/product/data/elicitation-methods.md` | Data | EXISTS |
...
flowchart TD
    ...
    C -->|yes| D[Load template from .lmas-core/product/templates/]
...
**Task file:** `.lmas-core/development/tasks/shard-doc.md`
...
**Task file:** `.lmas-core/development/tasks/document-project.md`
...
**Task file:** `.lmas-core/development/tasks/add-tech-doc.md` (MISSING)
...
**Task file:** `.lmas-core/development/tasks/create-next-story.md`
...
**Task file:** `.lmas-core/development/tasks/advanced-elicitation.md`
...
**Task file:** `.lmas-core/development/tasks/correct-course.md`
...
**Task file:** `.lmas-core/development/tasks/index-docs.md`
...
| `*guide` | Renders the `## LMAS Master Guide` section from agent .md |
...
| `*exit` | Exits lmas-master mode, returns to base Claude Code |
| `*agent {name}` | Reads and displays info about a specialized agent (use `@` to transform into it) |
| `*list-components` | Scans .lmas-core/ and lists all framework components (agents, tasks, workflows, templates, checklists) |
...
|-------------|-----------|---------|
| @lmas-master -> @pm | Delegate | Epic/story creation (brownfield-create-epic, brownfield-create-story) |
| @lmas-master -> @analyst | Delegate | Brainstorming sessions (*brainstorm) |
| @lmas-master -> @qa | Delegate | Test suite creation (*create-suite) |
| @lmas-master -> @architect | Delegate | AI prompt generation (*generate-ai-prompt) |
| @lmas-master -> @dev | Handoff | Story implementation after story creation |
| @lmas-master -> @devops | Delegate | Git push operations, PR creation |
| @lmas-master -> any agent | Execute | Can execute any task from any agent directly |
| Any agent -> @lmas-master | Escalate | Cross-agent coordination, framework operations |
...
|------------|---------------|-------------|
| `security-checker.js` | `.lmas-core/development/utils/` | `.lmas-core/infrastructure/scripts/security-checker.js` |
| `yaml-validator.js` | `.lmas-core/development/utils/` | `.lmas-core/infrastructure/scripts/yaml-validator.js` |
| `workflow-management.md` | `.lmas-core/development/utils/` | `.lmas-core/scripts/workflow-management.md` |
| `subagent-step-prompt.md` | `.lmas-core/product/templates/` | `.lmas-core/development/templates/subagent-step-prompt.md` |
| `elicitation-methods.md` | `.lmas-core/data/` | `.lmas-core/product/data/elicitation-methods.md` |
| `brainstorming-techniques.md` | `.lmas-core/data/` | `.lmas-core/product/data/brainstorming-techniques.md` |

---

*Traced from source on 2026-02-05 | Story LMAS-TRACE-001*
