# @qa (Oracle) - Execution Trace

> Traced from source code, not documentation.
> Agent definition: `.lmas-core/development/agents/qa.md`
...
|-------|------|--------|---------|
| 1 | `.lmas-core/development/agents/qa.md` | AgentConfigLoader.loadAgentDefinition() | Agent definition (YAML block) |
| 2 | `.lmas-core/core-config.yaml` | GreetingBuilder._loadConfig() | Core configuration |
| 3 | `.lmas-core/data/agent-config-requirements.yaml` | AgentConfigLoader.loadRequirements() | Config sections: qaLocation, dataLocation, storyBacklog |
...
| 6 | `.lmas-core/product/data/test-levels-framework.md` | AgentConfigLoader.loadFile() | Test levels framework (always loaded, 8KB) |
| 7 | `.lmas-core/product/data/test-priorities-matrix.md` | AgentConfigLoader.loadFile() | Test priorities matrix (always loaded, 6KB) |
| 8 | `.lmas/session-state.json` | ContextDetector._detectFromFile() | Session type detection (if no conversation history) |
| 9 | `.lmas/project-status.yaml` | ProjectStatusLoader.loadCache() | Cached project status (60s TTL) |
...
sequenceDiagram
    ...
    ACL-->>CC: { agent: { name: 'Oracle', icon: '✅' }, commands: [...27], ... }
...
qa:
  ...
  files_loaded:
    - path: .lmas-core/data/technical-preferences.md
      lazy: false
      size: 15KB
    - path: .lmas-core/product/data/test-levels-framework.md
      lazy: false
      size: 8KB
    - path: .lmas-core/product/data/test-priorities-matrix.md
      lazy: false
      size: 6KB
  lazy_loading:
...
| Data | Source | Value |
|------|--------|-------|
| Greeting level | `persona_profile.greeting_levels.archetypal` | `✅ Oracle the Guardian ready to perfect!` |
| Signature | `persona_profile.communication.signature_closing` | `— Oracle, guardiao da qualidade 🛡️` |
...
**Task file:** `.lmas-core/development/tasks/qa-run-tests.md`
...
**Task file:** `.lmas-core/development/tasks/qa-review-story.md`
...
| `.lmas-core/product/data/test-levels-framework.md` | Data | Loaded at activation |
| `.lmas-core/product/data/test-priorities-matrix.md` | Data | Loaded at activation |
...
**Task file:** `.lmas-core/development/tasks/qa-review-build.md`
...
**Task file:** `.lmas-core/development/tasks/qa-gate.md`
**Template:** `.lmas-core/product/templates/qa-gate-tmpl.yaml` (EXISTS in product/templates)

**Dependencies loaded:**
| File | Type | Status |
|------|------|--------|
| `qa-gate.md` | Task | EXISTS |
| `qa-gate-tmpl.yaml` | Template | EXISTS (in `.lmas-core/product/templates/`) |
| `docs/stories/{storyId}` | Input | Required |
...
**Task file:** `.lmas-core/development/tasks/qa-nfr-assess.md`
...
**Task file:** `.lmas-core/development/tasks/qa-risk-profile.md`
...
**Task file:** `.lmas-core/development/tasks/qa-create-fix-request.md`
...
**Task file:** `.lmas-core/development/tasks/qa-library-validation.md`
...
**Task file:** `.lmas-core/development/tasks/qa-security-checklist.md`
...
**Task file:** `.lmas-core/development/tasks/qa-migration-validation.md`
...
**Task file:** `.lmas-core/development/tasks/qa-evidence-requirements.md`
...
**Task file:** `.lmas-core/development/tasks/qa-false-positive-detection.md`
...
**Task file:** `.lmas-core/development/tasks/qa-browser-console-check.md`
...
**Task file:** `.lmas-core/development/tasks/qa-test-design.md`
...
**Task file:** `.lmas-core/development/tasks/qa-trace-requirements.md`
...
**Task file:** `.lmas-core/development/tasks/create-suite.md`
...
**Task file:** `.lmas-core/development/tasks/spec-critique.md`
...
**Task file:** `.lmas-core/development/tasks/manage-story-backlog.md` (MISSING)
...
| `qa-gate-tmpl.yaml` | Template | `*gate` (in `dependencies.templates`) | EXISTS in `.lmas-core/product/templates/` but NOT in `.lmas-core/development/templates/` per IDE-FILE-RESOLUTION |
| `story-tmpl.yaml` | Template | (in `dependencies.templates`) | EXISTS in `.lmas-core/product/templates/` but NOT in `.lmas-core/development/templates/` per IDE-FILE-RESOLUTION |
| `test-levels-framework.md` | Data | `agent-config-requirements.yaml` | EXISTS at `.lmas-core/product/data/` (NOT in `.lmas-core/data/` where `dependencies.data` resolves) |
| `test-priorities-matrix.md` | Data | `agent-config-requirements.yaml` | EXISTS at `.lmas-core/product/data/` (NOT in `.lmas-core/data/` where `dependencies.data` resolves) |

**Path Resolution Note:** The `IDE-FILE-RESOLUTION` rule in the agent definition maps `dependencies` to `.lmas-core/development/{type}/{name}`. However, 2 templates and 2 data files exist under `.lmas-core/product/` instead. The `agent-config-requirements.yaml` correctly references the full paths under `.lmas-core/product/data/`, but the agent definition's `dependencies` block uses short names that resolve to `.lmas-core/development/templates/` and `.lmas-core/development/data/` respectively, where these files do not exist. Actual runtime loading uses `agent-config-requirements.yaml` paths, so functionality is preserved.
