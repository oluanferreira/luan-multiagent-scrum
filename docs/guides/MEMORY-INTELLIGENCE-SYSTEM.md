...
> **ARQUITETURA:** O MIS segue o modelo **Open Core** do LMAS. Toda a inteligencia de memoria reside no repositorio privado `lmas-pro` (submodule `pro/`). O `lmas-core` (open source) fornece apenas extension points e funciona 100% sem memoria inteligente — exatamente como hoje.
...
O sistema de memoria atual opera em **duas camadas desconectadas** (Claude Code nativo + LMAS Framework) com gaps criticos:
...
|--------|--------|-------------|-------|
| **Cleanup** | Remover dead code, preparar base | **lmas-core** | MIS-2 |
| **Capture** | Captura conhecimento de sessao via hooks | **lmas-pro** | MIS-3 |
| **Storage** | Armazena em Markdown com frontmatter YAML | **lmas-pro** | MIS-3, MIS-4 |
| **Retrieval** | Recupera por relevancia com progressive disclosure | **lmas-pro** + extension points em **lmas-core** | MIS-4, MIS-6 |
| **Evolution** | Aprende e evolui regras automaticamente | **lmas-pro** | MIS-5, MIS-7 |

### Principios Chave (vs Estado Atual)

| Aspecto | Estado Atual | Com MIS (lmas-pro ativo) | Sem MIS (lmas-core only) |
|---------|-------------|--------------------------|--------------------------|
...
> **lmas-core funciona 100% sem o MIS** — exatamente como funciona hoje. Quando `lmas-pro` esta presente (submodule `pro/`), o MIS se conecta automaticamente via extension points. Zero configuracao manual.

### Modelo Open Core

```mermaid
flowchart LR
    subgraph CORE["lmas-core (Open Source, MIT)"]
        direction TB
    ...
    subgraph PRO["lmas-pro (Privado, Submodule pro/)"]
        direction TB
...
O lmas-core ja possui o modulo `bin/utils/pro-detector.js` (Story PRO-5):
...
#### lmas-core (open source) — Extension Points Only

| Arquivo | Tipo | Descricao |
|---------|------|-----------|
| `bin/utils/pro-detector.js` | Existente | `isProAvailable()`, `loadProModule()` |
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | Modificacao | Adiciona `if (isProAvailable())` no Tier 2 para memory loading |
| `.lmas-core/development/scripts/greeting-builder.js` | Modificacao | Adiciona memory stats no greeting se pro disponivel |
| `.lmas-core/core/memory/gotchas-memory.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.lmas-core/core/session/context-loader.js` | Mantido | Path fix (MIS-2), funciona standalone |
| `.claude/settings.json` | Modificacao | Hook stubs que chamam scripts em `pro/` (graceful fail se ausente) |

#### lmas-pro (privado) — Toda a Inteligencia
...
Os hooks do Claude Code sao configurados em `.claude/settings.json` (lmas-core). O script apontado reside em `pro/`:
...
```json
{
  "hooks": {
    "PreCompact": [
      {
        "type": "command",
        "command": "node .lmas-core/hooks/pro-hook-runner.js PreCompact",
        "timeout": 10000
      }
    ]
  }
...
```javascript
// .lmas-core/hooks/pro-hook-runner.js (no core, ~20 linhas)
const { isProAvailable, loadProModule } = require('../bin/utils/pro-detector');
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["lmas-core (Open Source)"]
        direction TB
        EXT["Extension Points<br/>in Pipeline"]
        HOOK_RUNNER[".lmas-core/hooks/<br/>pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js<br/>(standalone)"]
        CTX_LOADER["context-loader.js<br/>(standalone)"]
    end

    subgraph PRO_BOUNDARY["lmas-pro (Privado — pro/)"]
        ...
        subgraph STORAGE["CAMADA 2: STORAGE"]
            ST1[".lmas/memories/shared/"]
            ST2[".lmas/memories/{agent}/"]
            ST3[".lmas/memories/index.json"]
        end
...
### Fluxo Sem Pro (lmas-core standalone)
...
```mermaid
flowchart LR
    subgraph CORE["lmas-core"]
        HR[".lmas-core/hooks/<br/>pro-hook-runner.js"]
        UAP["unified-activation-<br/>pipeline.js"]
        GM["gotchas-memory.js"]
        PD["bin/utils/<br/>pro-detector.js"]
    end

    subgraph PRO["lmas-pro (pro/)"]
...
> **Repositorio:** `lmas-pro` (pro/memory/)
> **Extension point no core:** `.lmas-core/hooks/pro-hook-runner.js`
...
```json
// .claude/settings.json (lmas-core — committed)
{
  "hooks": {
    "PreCompact": [
      {
    ...
    "Stop": [
      {
        "type": "command",
        "command": "node .lmas-core/hooks/pro-hook-runner.js Stop",
        "timeout": 5000
      }
    ],
    "PostToolUseFailure": [
      {
        "type": "command",
        "command": "node .lmas-core/hooks/pro-hook-runner.js PostToolUseFailure",
        "timeout": 3000
      }
    ]
  }
...
```mermaid
flowchart TD
    subgraph CORE_HOOKS["lmas-core"]
        HR["pro-hook-runner.js"]
    end

    subgraph PRO_CAPTURE["lmas-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to<br/>.lmas/memories/")]
    end
...
> **Repositorio:** `lmas-pro` (pro/memory/) gerencia o storage
> **Storage location:** `.lmas/memories/` (gitignored, criado pelo pro)
...
```
.lmas/memories/                          # Criado e gerenciado por pro/memory/
...
> **Repositorio:** `lmas-pro` (pro/memory/memory-retriever.js, memory-loader.js)
...
> **Repositorio:** `lmas-pro` (pro/memory/self-learner.js, rule-proposer.js)
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js<br/>(lmas-core)
    participant PD as pro-detector.js<br/>(lmas-core)
    participant SD as session-digest.js<br/>(lmas-pro)
    participant FS as .lmas/memories/
...
> **Repositorio:** `lmas-pro` (pro/memory/agent-memory-api.js)
...
Cada agente acessa:
1. Suas proprias memorias privadas (`.lmas/memories/{agent}/`)
2. Memorias compartilhadas (`.lmas/memories/shared/`)
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline<br/>(lmas-core)
    participant PD as pro-detector.js<br/>(lmas-core)
    participant ML as memory-loader.js<br/>(lmas-pro)
    participant MR as memory-retriever.js<br/>(lmas-pro)
    participant GB as GreetingBuilder<br/>(lmas-core)
...
### Especificacao (arquivos em .lmas/memories/)
...
# Always Use Absolute Imports in LMAS
...
> **100% em lmas-pro** — nenhum componente de self-learning no core
...
→ PROPOSTA: Adicionar a rules/: "Warning: .lmas/ permissions"
...
**Sem pro:** Apenas `[CORE]` e `[NATIVO]` existem. `[PRO]` (.lmas/memories/) nunca e criado.

---

## Mapa de Arquivos do Sistema

### lmas-core (Open Source) — Extension Points

| Arquivo | Tipo | Story | Descricao |
|---------|------|-------|-----------|
| `bin/utils/pro-detector.js` | Existente (PRO-5) | — | `isProAvailable()`, `loadProModule()` |
| `.lmas-core/hooks/pro-hook-runner.js` | **Novo** | MIS-3 | Wrapper: verifica pro, delega hook (~20 linhas) |
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | **Modificacao** | MIS-6 | Adiciona `if (isProAvailable())` no Tier 2 |
| `.lmas-core/development/scripts/greeting-builder.js` | **Modificacao** | MIS-6 | Memory stats no greeting se pro disponivel |
| `.lmas-core/core/memory/gotchas-memory.js` | **Path fix** | MIS-2 | Corrige `.lmas/error-tracking.json` |
| `.lmas-core/core/session/context-loader.js` | **Path fix** | MIS-2 | Corrige `.lmas/session-state.json` |
...
### Arquivos Removidos (MIS-2, lmas-core)

| Arquivo | Linhas | Razao |
|---------|--------|-------|
| `.lmas-core/core/memory/timeline-manager.js` | 746 | 0 consumers (orphan) |
| `.lmas-core/core/memory/file-evolution-tracker.js` | 1,003 | 0 consumers (orphan) |
| `.lmas-core/core/memory/context-snapshot.js` | 648 | 0 consumers (orphan) |
| `.lmas-core/elicitation/session-manager.js` | — | Dead code |
| `.lmas/compound-analysis/` | ~14K | 0 readers (orphan data) |

---

## Diferencas: Estado Atual vs MIS

| Aspecto | lmas-core hoje | lmas-core + lmas-pro (MIS) | lmas-core sem pro |
|---------|---------------|---------------------------|------------------|
...
|-------|--------|-------------|------------|-------|
| MIS-1 | Investigation & Architecture Design | docs (lmas-core) | — | 12h (Done) |
| MIS-2 | Dead Code Cleanup & Path Repair | **lmas-core** | MIS-1 | 4h |
| MIS-3 | Session Digest (PreCompact Hook) | **lmas-core** (hook runner) + **lmas-pro** (digest) | MIS-1 | 14h |
| MIS-4 | Progressive Memory Retrieval | **lmas-pro** | MIS-3 | 16h |
| MIS-5 | Self-Learning Engine | **lmas-pro** | MIS-3, MIS-4 | 14h |
| MIS-6 | Pipeline Integration & Agent Memory API | **lmas-core** (ext points) + **lmas-pro** (loader) | MIS-4 | 10h |
| MIS-7 | CLAUDE.md & Rules Auto-Evolution | **lmas-pro** | MIS-5 | 8h |
| **Total** | | | | **~78h** |
...
| Epic PRO Architecture | [EPIC-PRO-INDEX.md](../stories/epics/epic-pro-lmas-pro-architecture/EPIC-PRO-INDEX.md) |
...
*LMAS Memory Intelligence System — Architecture Vision v4.0.4 (Core/Pro Split)*
*Target state apos Epic MIS completo (7 stories, ~78 horas)*
*lmas-core: extension points + dead code cleanup*
*lmas-pro: toda a inteligencia de memoria (Capture, Storage, Retrieval, Evolution)*
