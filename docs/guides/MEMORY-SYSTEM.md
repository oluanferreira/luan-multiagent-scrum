...
4. [Camada 2: LMAS Framework](#camada-2-lmas-framework)
...
O sistema de memoria do LMAS opera em **duas camadas independentes** que coexistem mas **nao se comunicam entre si**:

| Camada | Gerenciado Por | Escopo |
|--------|---------------|--------|
| **Camada 1: Claude Code Nativo** | Claude Code CLI | Auto Memory, CLAUDE.md, Session Transcripts |
| **Camada 2: LMAS Framework** | Scripts JS em `.lmas-core/` | Gotchas, Session State, Context Snapshots, Timeline |
...
- **Cada camada persiste de forma independente** — `.claude/` vs `.lmas/`
...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        ...
        CLAUDE_MD_P["CLAUDE.md Projeto<br/>lmas-core/.claude/CLAUDE.md"]
    ...
    subgraph LMAS_FRAMEWORK["Camada 2: LMAS Framework"]
        direction TB
        GOTCHAS["Gotchas Memory<br/>.lmas-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot<br/>.lmas-core/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker<br/>.lmas-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager<br/>.lmas-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader<br/>.lmas-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector<br/>.lmas-core/core/session/context-detector.js"]
        SESS_STATE["Session State<br/>.lmas-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks<br/>.lmas-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks<br/>.lmas-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_LMAS["Storage LMAS (.lmas/)"]
        S_GOTCHAS[".lmas/gotchas.json + .md"]
        S_ERRORS[".lmas/error-tracking.json"]
        S_SNAPSHOTS[".lmas/snapshots/"]
        S_TIMELINE[".lmas/timeline/"]
        S_FILE_EVO[".lmas/file-evolution/"]
        S_SESSION[".lmas/session-state.json"]
    ...
    style LMAS_FRAMEWORK fill:#FFF3E0,stroke:#E65100
    style STORAGE_CLAUDE fill:#E8F5E9,stroke:#2E7D32
    style STORAGE_LMAS fill:#FCE4EC,stroke:#C62828
```
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["Memory Layer (.lmas-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["Session Layer (.lmas-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["Orchestration (.lmas-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["Activation Pipeline (.lmas-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks System (.lmas-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        ...
        LOAD_PROJECT["3. lmas-core/.claude/CLAUDE.md<br/>Instrucoes nivel projeto"]
        LOAD_RULES["4. lmas-core/.claude/rules/*.md<br/>5 arquivos de regras detalhadas"]
...
flowchart LR
    subgraph AGENT_MEM[".claude/agent-memory/"]
        AM1["lmas-architect/MEMORY.md"]
        AM2["lmas-dev/MEMORY.md"]
    ...
    NOTE["NOTA: Apenas agentes<br/>definidos em .claude/agents/<br/>usam esta memoria.<br/>Agentes LMAS (.lmas-core/<br/>development/agents/)<br/>NAO usam."]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em:<br/>.lmas/gotchas.json<br/>.lmas/gotchas.md<br/>.lmas/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em:<br/>.lmas/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em:<br/>.lmas/file-evolution/<br/>evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em:<br/>.lmas/timeline/<br/>unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD4["Le .lmas/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("Persiste em:<br/>.lmas/session-state.json<br/>TTL: 1 hora")]
...
sequenceDiagram
    ...
    Note over CC: NENHUMA memoria LMAS<br/>e carregada automaticamente<br/>no system prompt.<br/>Apenas contexto de sessao<br/>aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".lmas/session-state.json<br/>Agente atual, comandos, workflow"]
        S2[".lmas/error-tracking.json<br/>Ocorrencia do erro"]
        S3[".lmas/gotchas.json<br/>Nova gotcha auto-capturada"]
        S4[".lmas/gotchas.json<br/>Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml<br/>Phase atualizada"]
        S6["docs/stories/.session-state.yaml<br/>Story marcada done"]
        S7[".lmas/snapshots/{id}.json<br/>Contexto capturado"]
        S8[".lmas/file-evolution/<br/>Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    CL->>FS: Le .lmas/session-state.json
    ...
    CL->>FS: Escreve .lmas/session-state.json
    FS-->>CL: OK
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        TRACK --> READ_ET["Le .lmas/error-tracking.json"]
        ...
        CREATE --> SAVE_JSON["Salva .lmas/gotchas.json"]
        SAVE_JSON --> SAVE_MD["Gera .lmas/gotchas.md"]
...
classDiagram
    class GotchasStore {
        +string schema = "lmas-gotchas-memory-v1"
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker<br/>.lmas/file-evolution/"]
        SRC2["ContextSnapshot<br/>.lmas/snapshots/"]
    ...
    subgraph STORAGE["Storage"]
        ST1[(".lmas/timeline/<br/>unified-timeline.json")]
    end
...
```mermaid
flowchart LR
    subgraph LMAS_EVENTS["Eventos LMAS"]
        E1["sessionStart"]
        E2["beforeAgent"]
        E3["beforeTool"]
        E4["afterTool"]
...
|---------|--------|------------|--------|
| `.lmas-core/core/memory/gotchas-memory.js` | Memory | Epic 9, Story 9.4 | Auto-capture de erros repetidos, gotchas manuais, injecao em tasks |
| `.lmas-core/core/memory/context-snapshot.js` | Memory | Story 12.6 | Captura e restaura contexto de desenvolvimento |
| `.lmas-core/core/memory/file-evolution-tracker.js` | Memory | Gap impl | Rastreia evolucao de arquivos, detecta drift |
| `.lmas-core/core/memory/timeline-manager.js` | Memory | Gap impl | Facade unificada para timeline cross-session |
| `.lmas-core/core/session/context-loader.js` | Session | Story 2.2, 6.1.2.5 | Continuidade inter-agente, handoff de contexto |
| `.lmas-core/core/session/context-detector.js` | Session | Story 2.2 | Deteccao hibrida de tipo de sessao (new/existing/workflow) |
| `.lmas-core/core/orchestration/session-state.js` | Orchestration | Story 11.5 | Estado persistente de epic/story, crash recovery |
| `.lmas-core/core/orchestration/context-manager.js` | Orchestration | Legacy | Estado de workflow entre fases (migrado para session-state) |
| `.lmas-core/core/elicitation/session-manager.js` | Elicitation | — | Sessions de elicitacao save/load |

### Scripts de Ativacao (consomem memoria)

| Arquivo | Funcao |
|---------|--------|
| `.lmas-core/development/scripts/unified-activation-pipeline.js` | Orchestrador principal — carrega sessao no Tier 3 |
| `.lmas-core/development/scripts/greeting-builder.js` | Monta greeting com contexto de sessao/memoria |

### Hooks (persistencia de sessao)

| Arquivo | CLI | Funcao |
|---------|-----|--------|
| `.lmas-core/hooks/gemini/session-start.js` | Gemini | Carrega contexto LMAS no inicio da sessao |
| `.lmas-core/hooks/gemini/session-end.js` | Gemini | Persiste sumario da sessao em `.lmas/sessions/` |
| `.lmas-core/hooks/gemini/before-agent.js` | Gemini | Pre-processamento antes de agente |
| `.lmas-core/hooks/gemini/before-tool.js` | Ambos | Pre-processamento antes de tool |
| `.lmas-core/hooks/gemini/after-tool.js` | Ambos | Pos-processamento apos tool |
| `.lmas-core/hooks/unified/hook-interface.js` | Ambos | Classe base UnifiedHook + EVENT_MAPPING |
| `.lmas-core/hooks/unified/hook-registry.js` | Ambos | Registro central de hooks |
| `.lmas-core/hooks/unified/index.js` | Ambos | Entry point do sistema unificado |
...
| `lmas-core/.claude/CLAUDE.md` | Instrucoes projeto (carregado sempre) |
| `lmas-core/.claude/rules/*.md` | 5 arquivos de regras (carregados sempre) |
...
Os modulos LMAS (gotchas, snapshots, timeline) **nao fazem flush** automatico ao final da sessao. Dados em memoria podem se perder se o processo terminar abruptamente.
...
Claude auto-memory (`MEMORY.md`) e LMAS memory (`.lmas/`) nunca se sincronizam. Gotchas capturadas pelo LMAS nao aparecem no MEMORY.md e vice-versa.
...
| Gotchas Memory Script | `.lmas-core/core/memory/gotchas-memory.js` |
| Context Snapshot Script | `.lmas-core/core/memory/context-snapshot.js` |
| File Evolution Tracker | `.lmas-core/core/memory/file-evolution-tracker.js` |
| Timeline Manager | `.lmas-core/core/memory/timeline-manager.js` |
| Session Context Loader | `.lmas-core/core/session/context-loader.js` |
| Context Detector | `.lmas-core/core/session/context-detector.js` |
| Session State | `.lmas-core/core/orchestration/session-state.js` |
| Unified Hook Interface | `.lmas-core/hooks/unified/hook-interface.js` |
| Hook Registry | `.lmas-core/hooks/unified/hook-registry.js` |
| Gemini Session Start | `.lmas-core/hooks/gemini/session-start.js` |
| Gemini Session End | `.lmas-core/hooks/gemini/session-end.js` |
| Core Config | `.lmas-core/core-config.yaml` |
| Claude Settings | `~/.claude/settings.json` |
| Story Development Cycle | `docs/guides/workflows/STORY-DEVELOPMENT-CYCLE-WORKFLOW.md` |

---

*LMAS Memory System Architecture Guide v1.0*
*Traced from source code, not documentation.*
*@architect (Architect) — arquitetando o futuro*
