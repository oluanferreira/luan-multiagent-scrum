...
Synchronize LMAS agents, tasks, workflows, and checklists across multiple IDE configurations.

## Overview

The `*command` task automates the synchronization of LMAS components to all configured IDE directories (`.claude/`, `.cursor/`, `.gemini/`, etc.), eliminating manual copy operations.
...
Edit `.lmas-sync.yaml` to enable your IDEs:
...
│                 *command sync                        │
│                                                      │
│  1. Read .lmas-sync.yaml configuration               │
...
```
your-project/
├── .lmas-sync.yaml           # Sync configuration
...
**Solution**: Add the alias to `.lmas-sync.yaml`:
...
- [LMAS Architecture](../core-architecture.md)
