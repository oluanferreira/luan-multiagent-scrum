...
The Project Status feature automatically displays your current work context when any LMAS agent activates. This includes:
...
- **LMAS-FullStack** framework installed
...
3. Create `.lmas/project-status.yaml` cache file
...
1. Edit `.lmas-core/core-config.yaml`:
   ```yaml
   projectStatus:
     enabled: true
...
Location: `.lmas-core/core-config.yaml`
...
projectStatus:
  ...
  statusFile: .lmas/project-status.yaml  # Cache file location
...
1. **Checks cache** - Looks for `.lmas/project-status.yaml`
...
- **Cache Location:** `.lmas/project-status.yaml`
...
All 11 LMAS agents display project status:
...
3. Does `.lmas-core/infrastructure/scripts/project-status-loader.js` exist?
...
A: Yes, all agents use the same cache file (`.lmas/project-status.yaml`).
...
- **Config:** `.lmas-core/core-config.yaml` (projectStatus section)
