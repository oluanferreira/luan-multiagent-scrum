...
Complete guide for creating, validating, publishing, and managing Squads in LMAS.

> **LMAS Squads:** Equipes de AI agents trabalhando com você
...
Squads are modular teams of AI agents that extend LMAS functionality. Each squad is a self-contained package containing:
...
│  Level 2: LMAS-SQUADS  → github.com/oluanferreira (Public/Free)  │
│  Level 3: LMAS API   → api.lmas.dev      (Marketplace)  │
└─────────────────────────────────────────────────────────────┘
```
...
|-------|---------|-------------|
| [etl-squad](https://github.com/oluanferreira/luan-multiagent-scrum/tree/main/etl) | 2.0.0 | Data collection and transformation |
| [creator-squad](https://github.com/oluanferreira/luan-multiagent-scrum/tree/main/creator) | 1.0.0 | Content generation utilities |
...
- Node.js 18+
- LMAS project initialized (`.lmas-core/` exists)
...
Tasks must follow [TASK-FORMAT-SPECIFICATION-V1](../../.lmas-core/docs/standards/TASK-FORMAT-SPECIFICATION-V1.md).
...
| Mode | Behavior |
|------|----------|
| `extend` | Add squad rules to core LMAS rules |
...
### Level 2: lmas-squads Repository (Public)
...
This creates a PR to [LMAS/lmas-squads](https://github.com/oluanferreira/luan-multiagent-scrum).

### Level 3: LMAS Marketplace
...
3. **Add Fields** - `lmas.type`, `lmas.minVersion`
...
# - lmas.type: must be "squad"
# - lmas.minVersion: must be valid semver
```
...
### What's the difference between a Squad and legacy LMAS squad formats?

**Squads** are the standard format in LMAS 2.1+ with:
- Task-first architecture
...
3. Re-publish: `*publish-squad` or `*sync-squad-lmas`
...
- **Level 3**: Sync with `--private` flag: `*sync-squad-lmas my-squad --private`

### What's the minimum LMAS version for Squads?

Squads require LMAS 2.1.0+. Set in manifest:
...
- [@squad-creator Agent](../../.lmas-core/development/agents/squad-creator.md)
