...
Complete API reference for Synkra LMAS - the AI-Orchestrated System for Full Stack Development.
...
LMAS provides a unified API for interacting with specialized AI agents through two primary mechanisms:
...
| ---------------- | ------ | ------------ | ---------------------------------------- |
| `@dev`           | Neo    | Builder      | Code implementation, debugging, testing  |
| `@qa`            | Oracle  | Guardian     | Quality assurance, code review, testing  |
| `@architect`     | Aria   | Visionary    | System architecture, API design          |
| `@pm`            | Morgan | Strategist   | Product requirements, epics, strategy    |
| `@po`            | Keymaker    | Champion     | Backlog management, acceptance criteria  |
| `@sm`            | River  | Facilitator  | Sprint planning, story creation          |
| `@analyst`       | Atlas  | Explorer     | Market research, competitive analysis    |
| `@data-engineer` | Dozer   | Architect    | Database schema, migrations, queries     |
| `@devops`        | Operator   | Optimizer    | CI/CD, deployment, git operations        |
| `@ux-expert`     | Uma    | Creator      | UI/UX design, wireframes                 |
| `@lmas-master`   | Morpheus  | Orchestrator | Framework orchestration, meta-operations |

### Activation Behavior

When an agent is activated:

1. Agent definition file is loaded from `.lmas-core/development/agents/{id}.md`
...
### @lmas-master (Orchestrator)

**Framework Development:**

| Command                | Arguments       | Description                                 |
| ---------------------- | --------------- | ------------------------------------------- |
| `*create`              | `{type} {name}` | Create LMAS component (agent/task/workflow) |
...
| `*kb`   | -                          | Toggle KB mode (LMAS Method knowledge) |
...
| -------------------- | ------------------------------- | -------------------------------------- |
| `AGENT_NOT_FOUND`    | Agent definition missing        | Check `.lmas-core/development/agents/` |
...
/lmas-master  → Activates @lmas-master agent
```
...
- [User Guide](./user-guide.md) - Getting started with LMAS
...
- [Squads Guide](./squads-guide.md) - Extending LMAS with squads

---

_Synkra LMAS API Reference v4.0.4_
