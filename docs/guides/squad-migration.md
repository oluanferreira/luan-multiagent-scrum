...
How to migrate legacy squads to LMAS 2.1 format.

## Overview

LMAS 2.1 introduced a new squad format with:
...
| LMAS type field | Missing | `lmas.type: squad` |
| Min version | Missing | `lmas.minVersion: "2.1.0"` |
...
**Result:**
- Adds missing `lmas` fields to manifest
...
```javascript
const { SquadMigrator } = require('./.lmas-core/development/scripts/squad');
...
- [ ] `lmas.type` is `"squad"`
- [ ] `lmas.minVersion` is `"2.1.0"` or higher
